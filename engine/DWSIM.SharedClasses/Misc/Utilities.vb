...
Imports System.IO
Imports es = DWSIM.SharedClasses.EncryptString.StringCipher
...
Public Class Utility
    ...
    Public Shared Function GetSupportStatus() As Boolean
        ...
        Try
            sa = Assembly.LoadFile(Path.Combine(AppDomain.CurrentDomain.BaseDirectory,
                                                es.Decrypt("JjvR3q4zzr+/rlo2FnDmq7kkH375h8087Yi+uVls82K7AEygdi8CuJvU2xxjcVc9bAwpPioA+sz28JcsMGIjH9RGYp/h6dbuRTkdHUbNo2b8EDF8P82S4N7e9+Tsack1", "A3445B78-70ED-46B0-B5C8-35232FA61FBB")))
        Catch ex As Exception
        End Try
        If sa IsNot Nothing Then
            Dim tp = sa.CreateInstance(es.Decrypt("te1f3eWGaZzgWM/oHXn49FXcZvHLBC/LdLIRKj8+ttSqPe2eAXdIhiUxEf/JmGs6J7ggAg3+03Ti4XZM7Dq+3k4tOvnpJCbuZeE8Lsuk0umCoostCHTVKt746mvc5/Bo", "C6FE3F23-C0AD-4026-A680-CABA5D834EAD"))
            Return tp.GetType().GetMethod(es.Decrypt("qunLFsusJUKJGGFZo0BclD8D2d14ykCRhmpk7k210RJNkulOVXgfQDRqwwL3boc5j4YAVPSUNk4ss7EEfTi2BfbB5haIBQzo2GL/6n73fPt4dEwjpmL1/0REVd00hyyK", "14515661-4040-4395-9A2E-6C4FA1D9C002")).Invoke(tp, Nothing)
        Else
            Return False
        End If
