...
#End Region
    ...
    <System.Serializable()> Partial Public MustInherit Class PropertyPackage
...
#Region "   Property Calculation Settings"
        ...
        Public Property UseImmiscibleListForLiquid2InitialEstimates As Boolean = True
...
#Region "   Properties"
        ...
        'immiscible liquids list
        Public Property ImmiscibleLiquids As New List(Of String)
...
#Region "   Must Override or Overridable Functions"
        ...
        Public Overridable Sub PopulateCrossPlatformEditor(container As Object) Implements IPropertyPackage.PopulateCrossPlatformEditor
        ...
        ''' <summary>
        ''' Returns the generalized-cubic EOS type (Utilities.TCP.CubicCP.EOS_*) used to build the analytical
        ''' true-critical-point Hessian and cubic form, or -1 when this package is not a supported cubic EOS
        ''' (in which case the generic method falls back to finite differences). Overridden by PR / SRK.
        ''' </summary>
        Protected Overridable Function GetCubicEOSTypeForCriticalPoint() As Integer
            Return -1
        End Function
        ...
        Public Overridable Function DW_CalculateCriticalPoints() As List(Of Double())
            ...
            ' For a supported cubic EOS, feed the generic method the closed-form critical Hessian and cubic
            ' form instead of finite differences (the second-order term was a noisy finite-difference of a
            ' finite-difference). The component properties are composition-independent, so they are captured.
            Dim eosType As Integer = GetCubicEOSTypeForCriticalPoint()
            If eosType >= 0 Then
                Dim vw = RET_VW()
                Dim vkij = RET_VKij()
                gm.QMatrixAnalytic = Function(T, V, Vzi)
                                         Return Utilities.TCP.CubicCP.QMatrix(eosType, T, V, Vzi, VTc, VPc, vw, vkij)
                                     End Function
                gm.CubicFormAnalytic = Function(Dn, T, V, Vzi)
                                           Return Utilities.TCP.CubicCP.CubicForm(eosType, Dn, T, V, Vzi, VTc, VPc, vw, vkij)
                                       End Function
                gm.CalcP = Function(T, V, Vzi)
                               Return Utilities.TCP.CubicCP.PressureCubic(eosType, T, V, Vzi, VTc, VPc, vw, vkij)
                           End Function
            End If
        ...
        Public Overridable Sub DW_CalcOverallProps()
            ...
            Dim HL, HV, HS, SL, SV, SS, DL, DV, DS, CPL, CPV, CPS, KL, KV, KS, CVL, CVV, CSV, MUV, MUL, MUS As Nullable(Of Double)
            ...
            MUL = Me.CurrentMaterialStream.Phases(1).Properties.viscosity.GetValueOrDefault
            MUV = Me.CurrentMaterialStream.Phases(2).Properties.viscosity.GetValueOrDefault
            MUS = Me.CurrentMaterialStream.Phases(7).Properties.viscosity.GetValueOrDefault

            result = wl * MUL.GetValueOrDefault + wv * MUV.GetValueOrDefault + ws * MUS.GetValueOrDefault

            Me.CurrentMaterialStream.Phases(0).Properties.viscosity = result

            MUL = Me.CurrentMaterialStream.Phases(1).Properties.kinematic_viscosity.GetValueOrDefault
            MUV = Me.CurrentMaterialStream.Phases(2).Properties.kinematic_viscosity.GetValueOrDefault
            MUS = Me.CurrentMaterialStream.Phases(7).Properties.kinematic_viscosity.GetValueOrDefault

            result = wl * MUL.GetValueOrDefault + wv * MUV.GetValueOrDefault + ws * MUS.GetValueOrDefault

            Me.CurrentMaterialStream.Phases(0).Properties.kinematic_viscosity = result
                            ...
                            If Not Settings.CAPEOPENMode Then
                                ...
                                If Me.FlashBase.FlashSettings(Enums.FlashSetting.UsePhaseIdentificationAlgorithm) Then
                                    If Me.ComponentName.Contains("SRK") Or Me.ComponentName.Contains("PR") Then
                                        ...
                                        End If
                                    End If
                                ElseIf CurrentMaterialStream.PhaseEnvelopeLookup IsNot Nothing AndAlso CurrentMaterialStream.PhaseEnvelopeLookup.IsReady Then
                                    If Not Me.AUX_IS_SINGLECOMP(Phase.Mixture) Then
                                        IObj?.SetCurrent()
                                        Dim newphase As String
                                        If xv = 1.0# Or xl = 1.0# Then
                                            If xv = 1.0# Then
                                                newphase = Auxiliary.FlashAlgorithms.FlashAlgorithm.IdentifyPhaseByLookupTable(Vy, P, T, Me)
                                                If newphase = "L" Then
                                                    xv = 0.0#
                                                    xl = 1.0#
                                                    Vx = Vy
                                                End If
                                            Else
                                                newphase = Auxiliary.FlashAlgorithms.FlashAlgorithm.IdentifyPhaseByLookupTable(Vx, P, T, Me)
                                                If newphase = "V" Then
                                                    xv = 1.0#
                                                    xl = 0.0#
                                                    Vy = Vx
                                        ...
                                        Else
                                            If xl2 = 0.0# Then
                                                ...
                                                End If
                                            ElseIf xv = 0.0# Then
                                                newphase = Auxiliary.FlashAlgorithms.FlashAlgorithm.IdentifyPhaseByLookupTable(Vx2, P, T, Me)
                                                If newphase = "V" Then
                                                    xv = xl2
                                                    xl2 = 0.0#
                                                    Vy = Vx2
                ...
                Case FlashSpec.P
                    ...
                    Select Case spec2
                        ...
                        Case FlashSpec.H
                            ...
                            Dim ms_ph = TryCast(Me.CurrentMaterialStream, Streams.MaterialStream)
                            If ms_ph IsNot Nothing Then
                                Dim Test = ms_ph.EstimateTemperatureFromPH(P, H)
                                If Not Double.IsNaN(Test) AndAlso Test > 0 Then T = Test
                            End If
...
redirect:                   IObj?.SetCurrent()
                        ...
                        Case FlashSpec.S
                            ...
                            Dim ms_ps = TryCast(Me.CurrentMaterialStream, Streams.MaterialStream)
                            If ms_ps IsNot Nothing Then
                                Dim Test = ms_ps.EstimateTemperatureFromPS(P, S)
                                If Not Double.IsNaN(Test) AndAlso Test > 0 Then T = Test
                            End If
...
redirect2:                  IObj?.SetCurrent()
        ...
        Private Sub CalculateCriticalPointForEnvelope(
                ByVal n As Integer,
                ByVal Vm2() As Double, ByVal VTc2() As Double, ByVal VPc2() As Double,
                ByVal VVc2() As Double, ByVal Vw2() As Double, ByVal VKij2(,) As Double, ByVal VKij3(,) As Double,
        ...
        Public Overridable Function DW_ReturnPhaseEnvelope(ByVal peoptions As PhaseEnvelopeOptions, Optional ByVal bw As System.ComponentModel.BackgroundWorker = Nothing) As Object
            ...
            Dim PB, PO, TVB, TVD, HB, HO, SB, SO, VB, VO, TE, PE, THsI, PHsI, THsII, PHsII,
                PB1, TVB1, HB1, SB1, PB2, TVB2, HB2, SB2, VB1, VB2,
                TQ, PQ, TI, PI, TOWF, POWF, VOWF, HOWF, SOWF,
                TSLE1, PSLE1, TSLE2, PSLE2,
                TWidomCp, PWidomCp, TWidomBetaT, PWidomBetaT As New List(Of Double)
            ...
            Dim lastValidBeta As Double = 10.0#
            ...
            If eresult IsNot Nothing Then
                ...
                Else
                    ...
                    CalculateCriticalPointForEnvelope(n, Vm2, VTc2, VPc2, VVc2, Vw2, VKij2, VKij3, CP, TCR, PCR, VCR, stopAtCP, recalcCP)
            ...
            Else
                ...
                Do
                        ...
                        If options.CheckLiquidInstability Then
                            ...
                            If result.ResultException Is Nothing Then
                                If result.GetLiquidPhase2MoleFraction > 0.0# Then
                                    Dim curL1 = result.GetLiquidPhase1MoleFractions
                                    Dim curL2 = result.GetLiquidPhase2MoleFractions
                                    If prevL1Comp IsNot Nothing Then
                                        Dim distL1 = 0.0
                                        Dim distL2 = 0.0
                                        For ci = 0 To curL1.Length - 1
                                            distL1 += Math.Abs(curL1(ci) - prevL1Comp(ci))
                                            distL2 += Math.Abs(curL2(ci) - prevL1Comp(ci))
                                        Next
                                        If distL2 < distL1 Then
                                            Dim swapTmp = curL1
                                            curL1 = curL2
                                            curL2 = swapTmp
                                        End If
                                    End If
                                    prevL1Comp = curL1.Clone()
                                    Try
                                        tmp2 = Me.FlashBase.Flash_TV(curL1, T, 0.0#, P * 1.05, Me)
                                        TVB1.Add(T)
                                        PB1.Add(tmp2(4))
                                        HB1.Add(Me.DW_CalcEnthalpy(curL1, T, tmp2(4), State.Liquid))
                                        SB1.Add(Me.DW_CalcEntropy(curL1, T, tmp2(4), State.Liquid))
                                        VB1.Add(1 / Me.AUX_LIQDENS(T, curL1) * Me.AUX_MMM(curL1))
                                    Catch ex As Exception
                                        Flowsheet?.ShowMessage("Phase Envelope generation: Bubble L1 instability flash failed at T=" & T.ToString("G6") & " K: " & ex.Message, IFlowsheet.MessageType.Warning)
                                    End Try
                                    Try
                                        tmp2 = Me.FlashBase.Flash_TV(curL2, T, 0.0#, P * 1.05, Me)
                                        TVB2.Add(T)
                                        PB2.Add(tmp2(4))
                                        HB2.Add(Me.DW_CalcEnthalpy(curL2, T, tmp2(4), State.Liquid))
                                        SB2.Add(Me.DW_CalcEntropy(curL2, T, tmp2(4), State.Liquid))
                                        VB2.Add(1 / Me.AUX_LIQDENS(T, curL2) * Me.AUX_MMM(curL2))
                                    Catch ex As Exception
                                        Flowsheet?.ShowMessage("Phase Envelope generation: Bubble L2 instability flash failed at T=" & T.ToString("G6") & " K: " & ex.Message, IFlowsheet.MessageType.Warning)
                                    End Try
                                End If
                            Else
                    ...
                    Else
                        ...
                        Dim bubbleRelDistCP = Math.Max(Math.Abs(T - TCR) / TCR, Math.Abs(P - PCR) / PCR)
                        Dim bubbleNearCP = (bubbleRelDistCP < 0.15)
                        Dim bubbleUseKI = Not bubbleNearCP

                        If beta < 20 Then
                            Dim Pguess = PB(PB.Count - 1)
                            If bubbleNearCP AndAlso TVB.Count >= 3 Then
                                Dim npts = TVB.Count
                                Dim x0 = TVB(npts - 3) : Dim y0 = PB(npts - 3)
                                Dim x1 = TVB(npts - 2) : Dim y1 = PB(npts - 2)
                                Dim x2 = TVB(npts - 1) : Dim y2 = PB(npts - 1)
                                Dim L0 = ((T - x1) * (T - x2)) / ((x0 - x1) * (x0 - x2))
                                Dim L1 = ((T - x0) * (T - x2)) / ((x1 - x0) * (x1 - x2))
                                Dim L2 = ((T - x0) * (T - x1)) / ((x2 - x0) * (x2 - x1))
                                Pguess = y0 * L0 + y1 * L1 + y2 * L2
                                If Pguess <= 0 Then Pguess = PB(PB.Count - 1)
                            ElseIf bubbleNearCP AndAlso TVB.Count >= 2 Then
                                Dim slope = (PB(PB.Count - 1) - PB(PB.Count - 2)) / (TVB(TVB.Count - 1) - TVB(TVB.Count - 2))
                                Pguess = PB(PB.Count - 1) + slope * (T - TVB(TVB.Count - 1))
                                If Pguess <= 0 Then Pguess = PB(PB.Count - 1)
                            End If
                            Try
                                If bubbleUseKI Then
                                    tmp2 = Me.FlashBase.Flash_TV(Vz, T, 0, Pguess, Me, True, KI)
                                Else
                                    tmp2 = Me.FlashBase.Flash_TV(Vz, T, 0, Pguess, Me)
                                End If
                                Dim Presult = CDbl(tmp2(4))
                                Dim bubPdeviation = If(Pguess > 0, Math.Abs(Presult - Pguess) / Pguess, 0.0)
                                If bubbleNearCP AndAlso bubPdeviation > 0.03 Then
                                    Flowsheet?.ShowMessage("Phase Envelope generation: Bubble TVF point rejected (P=" & Presult.ToString("G6") & " vs expected " & Pguess.ToString("G6") & ")", IFlowsheet.MessageType.Warning)
                                    consecutiveFailures += 1
                                    If consecutiveFailures >= 10 Then Exit Do
                                    T = (T + TVB(TVB.Count - 1)) / 2.0
                                    Continue Do
                                End If
                                TVB.Add(T)
                                PB.Add(Presult)
                            ...
                            End Try
                        Else
                            Dim Tguess = TVB(TVB.Count - 1)
                            If bubbleNearCP AndAlso PB.Count >= 3 Then
                                Dim npts = PB.Count
                                Dim x0 = PB(npts - 3) : Dim y0 = TVB(npts - 3)
                                Dim x1 = PB(npts - 2) : Dim y1 = TVB(npts - 2)
                                Dim x2 = PB(npts - 1) : Dim y2 = TVB(npts - 1)
                                Dim L0 = ((P - x1) * (P - x2)) / ((x0 - x1) * (x0 - x2))
                                Dim L1 = ((P - x0) * (P - x2)) / ((x1 - x0) * (x1 - x2))
                                Dim L2 = ((P - x0) * (P - x1)) / ((x2 - x0) * (x2 - x1))
                                Tguess = y0 * L0 + y1 * L1 + y2 * L2
                                If Tguess <= 0 Then Tguess = TVB(TVB.Count - 1)
                            ElseIf bubbleNearCP AndAlso PB.Count >= 2 Then
                                Dim slope = (TVB(TVB.Count - 1) - TVB(TVB.Count - 2)) / (PB(PB.Count - 1) - PB(PB.Count - 2))
                                Tguess = TVB(TVB.Count - 1) + slope * (P - PB(PB.Count - 1))
                                If Tguess <= 0 Then Tguess = TVB(TVB.Count - 1)
                            End If
                            Try
                                If bubbleUseKI Then
                                    tmp2 = Me.FlashBase.Flash_PV(Vz, P, 0, Tguess, Me, True, KI)
                                Else
                                    tmp2 = Me.FlashBase.Flash_PV(Vz, P, 0, Tguess, Me)
                                End If
                                Dim Tresult = CDbl(tmp2(4))
                                Dim bubTdeviation = If(Tguess > 0, Math.Abs(Tresult - Tguess) / Tguess, 0.0)
                                If bubbleNearCP AndAlso bubTdeviation > 0.02 Then
                                    Flowsheet?.ShowMessage("Phase Envelope generation: Bubble PVF point rejected (T=" & Tresult.ToString("G6") & " vs expected " & Tguess.ToString("G6") & ")", IFlowsheet.MessageType.Warning)
                                    consecutiveFailures += 1
                                    If consecutiveFailures >= 10 Then Exit Do
                                    P = (P + PB(PB.Count - 1)) / 2.0
                                    Continue Do
                                End If
                                TVB.Add(Tresult)
                                PB.Add(P)
                                ...
                                consecutiveFailures = 0
                            Catch ex As Exception
                                Flowsheet?.ShowMessage("Phase Envelope generation: Bubble PVF flash failed at P=" & P.ToString("G6") & " Pa: " & ex.Message, IFlowsheet.MessageType.Warning)
                                consecutiveFailures += 1
                                If consecutiveFailures >= 10 Then Exit Do
                                Dim lastGoodP = PB(PB.Count - 1)
                                If Math.Abs(lastGoodP - PCR) / PCR < 0.2 Then
                                    P = lastGoodP + (PCR - lastGoodP) * 0.1 * consecutiveFailures
                                Else
                                    P = (P + lastGoodP) / 2.0
                                End If
                                Continue Do
                        ...
                        If stopAtCP Then
                            If (Math.Abs(T - TCR) < 2.0 AndAlso Math.Abs(P - PCR) < 10000) OrElse T > TCR Then
                                Exit Do
                            End If
                        End If

                        Dim relDistCP = Math.Max(Math.Abs(T - TCR) / TCR, Math.Abs(P - PCR) / PCR)

                        If relDistCP < 0.15 Then
                            Dim absDeltaT = If(relDistCP < 0.05, 0.5, 1.0)
                    ...
                    If Double.IsNaN(beta) Or Double.IsInfinity(beta) Then
                        beta = lastValidBeta
                    Else
                        lastValidBeta = beta
                    End If
                ...
                Do
                    ...
                    If i < 2 Then
                        ...
                        If options.DewCurveInitialFlash = "TVF" Then
                            tmp2 = Me.FlashBase.Flash_TV(Vz, T, 1, 0, Me)
                            TVD.Add(T)
                            PO.Add(tmp2(4))
                            P = PO(PO.Count - 1)
                    ...
                    Else
                        ...
                        Dim absBeta = Math.Abs(beta)
                        Dim dewRelDistCP = Math.Max(Math.Abs(T - TCR) / TCR, Math.Abs(P - PCR) / PCR)
                        Dim dewNearCP = (dewRelDistCP < 0.15)
                        Dim dewReturnCount = 0
                        If TVD.Count >= 2 Then
                            For ri = TVD.Count - 1 To 1 Step -1
                                If TVD(ri) < TVD(ri - 1) Then
                                    dewReturnCount += 1
                                Else
                                    Exit For
                                End If
                            Next
                        End If
                        Dim dewOnReturnLeg = (dewReturnCount >= 5)
                        Dim dewValidate = dewOnReturnLeg OrElse dewNearCP
                        Dim dewUseKI = Not dewNearCP

                        If absBeta < 20 Then
                            ' near cricondenbar (small slope) -> step T, iterate on P
                            Dim Pguess = PO(PO.Count - 1)
                            If dewValidate AndAlso TVD.Count >= 3 Then
                                Dim npts = TVD.Count
                                Dim x0 = TVD(npts - 3) : Dim y0 = PO(npts - 3)
                                Dim x1 = TVD(npts - 2) : Dim y1 = PO(npts - 2)
                                Dim x2 = TVD(npts - 1) : Dim y2 = PO(npts - 1)
                                Dim L0 = ((T - x1) * (T - x2)) / ((x0 - x1) * (x0 - x2))
                                Dim L1 = ((T - x0) * (T - x2)) / ((x1 - x0) * (x1 - x2))
                                Dim L2 = ((T - x0) * (T - x1)) / ((x2 - x0) * (x2 - x1))
                                Pguess = y0 * L0 + y1 * L1 + y2 * L2
                                If Pguess <= 0 Then Pguess = PO(PO.Count - 1)
                            ElseIf dewValidate AndAlso TVD.Count >= 2 Then
                                Dim slope = (PO(PO.Count - 1) - PO(PO.Count - 2)) / (TVD(TVD.Count - 1) - TVD(TVD.Count - 2))
                                Pguess = PO(PO.Count - 1) + slope * (T - TVD(TVD.Count - 1))
                                If Pguess <= 0 Then Pguess = PO(PO.Count - 1)
                            End If
                            Try
                                If dewUseKI Then
                                    tmp2 = Me.FlashBase.Flash_TV(Vz, T, 1, Pguess, Me, True, KI)
                                Else
                            ...
                            End Try
                        Else
                            ' near cricondentherm (steep slope) -> step P, iterate on T
                            Dim Tguess = TVD(TVD.Count - 1)
                            If dewValidate AndAlso PO.Count >= 3 Then
                                Dim npts = PO.Count
                                Dim x0 = PO(npts - 3) : Dim y0 = TVD(npts - 3)
                                Dim x1 = PO(npts - 2) : Dim y1 = TVD(npts - 2)
                                Dim x2 = PO(npts - 1) : Dim y2 = TVD(npts - 1)
                                Dim L0 = ((P - x1) * (P - x2)) / ((x0 - x1) * (x0 - x2))
                                Dim L1 = ((P - x0) * (P - x2)) / ((x1 - x0) * (x1 - x2))
                                Dim L2 = ((P - x0) * (P - x1)) / ((x2 - x0) * (x2 - x1))
                                Tguess = y0 * L0 + y1 * L1 + y2 * L2
                                If Tguess <= 0 Then Tguess = TVD(TVD.Count - 1)
                            ElseIf dewValidate AndAlso PO.Count >= 2 Then
                                Dim slope = (TVD(TVD.Count - 1) - TVD(TVD.Count - 2)) / (PO(PO.Count - 1) - PO(PO.Count - 2))
                                Tguess = TVD(TVD.Count - 1) + slope * (P - PO(PO.Count - 1))
                                If Tguess <= 0 Then Tguess = TVD(TVD.Count - 1)
                            End If
                            Try
                                If dewUseKI Then
                                    tmp2 = Me.FlashBase.Flash_PV(Vz, P, 1, Tguess, Me, True, KI)
                                Else
                        ...
                        dewRelDistCP = Math.Max(Math.Abs(T - TCR) / TCR, Math.Abs(P - PCR) / PCR)
                        ...
                        If pastCricondentherm AndAlso dewRelDistCP < 0.15 Then
                            ...
                            Else
                                P = P + signP * Math.Min(absDeltaP, Math.Abs(PCR - P) * 0.5)
                            End If
                        ElseIf absBeta < 20 Then
                            Dim dT = TVD(TVD.Count - 1) - TVD(TVD.Count - 2)
                            Dim signT = If(dT < 0.0, -1.0, 1.0)
                            T = T + signT * options.DewCurveDeltaT
                        Else
                            If pastCricondentherm AndAlso T > TCR Then
                                Dim signP = If(PCR > P, 1.0, -1.0)
                                P = P + signP * options.DewCurveDeltaP
                            Else
                                Dim dP = PO(PO.Count - 1) - PO(PO.Count - 2)
                                Dim signP = If(dP < 0.0, -1.0, 1.0)
                                P = P + signP * options.DewCurveDeltaP
                            End If
                        End If
                ...
                If recalcCP OrElse (Not TypeOf Me Is PengRobinsonPropertyPackage And Not TypeOf Me Is PengRobinson1978PropertyPackage And Not TypeOf Me Is SRKPropertyPackage) Then
                    ...
                    maxT = Max(TVB.ToArray.Max, TVD.ToArray.Max)
                ...
                'remove padding zeros added at intersection point

                For padding As Integer = 0 To 1
                    If TVB.Count > 1 AndAlso TVB(TVB.Count - 1) = 0.0# Then
                        TVB.RemoveAt(TVB.Count - 1)
                        PB.RemoveAt(PB.Count - 1)
                        HB.RemoveAt(HB.Count - 1)
                        SB.RemoveAt(SB.Count - 1)
                        VB.RemoveAt(VB.Count - 1)
                    End If
                Next

                For padding As Integer = 0 To 1
                    If TVD.Count > 1 AndAlso TVD(TVD.Count - 1) = 0.0# Then
                        TVD.RemoveAt(TVD.Count - 1)
                        PO.RemoveAt(PO.Count - 1)
                        HO.RemoveAt(HO.Count - 1)
                        SO.RemoveAt(SO.Count - 1)
                ...
                Do
                    If i < 2 Then
                        Try
                            ...
                            KI = tmp2(6)
                        Catch ex As Exception
                            Flowsheet?.ShowMessage("Phase Envelope generation: Quality line flash failed at P=" & P.ToString("G6") & " Pa: " & ex.Message, IFlowsheet.MessageType.Warning)
                        Finally
                            If options.QualityValue < 0.5 Then
                                P = P + options.BubbleCurveDeltaP
                            Else
                    ...
                    Else
                        If beta < 2 Then
                            ...
                            Catch ex As Exception
                                Flowsheet?.ShowMessage("Phase Envelope generation: Quality line TVF flash failed at T=" & T.ToString("G6") & " K: " & ex.Message, IFlowsheet.MessageType.Warning)
                            Finally
                        ...
                        Else
                            Try
                                ...
                                beta = (Math.Log(PQ(PQ.Count - 1) / 101325) - Math.Log(PQ(PQ.Count - 2) / 101325)) / (Math.Log(TQ(TQ.Count - 1)) - Math.Log(TQ(TQ.Count - 2)))
                            Catch ex As Exception
                                Flowsheet?.ShowMessage("Phase Envelope generation: Quality line PVF flash failed at P=" & P.ToString("G6") & " Pa: " & ex.Message, IFlowsheet.MessageType.Warning)
                            Finally
            ...
            If n > 0 And options.StabilityCurve Then
                ...
                ElseIf TypeOf Me Is PengRobinson1978PropertyPackage Then
                    If bw IsNot Nothing Then bw.ReportProgress(0, "Stability Line")
                    Dim res As ArrayList = New Utilities.TCP.Methods(Utilities.TCP.CubicCP.EOS_PR78).STABILITY_CURVE(Vm2, VTc2, VPc2, VVc2, Vw2, VKij2)
                    i = 0
                    Do
                        TE.Add(res(i)(0))
                        PE.Add(res(i)(1))
                ...
                ElseIf TypeOf Me Is SRKPropertyPackage Then
                    If bw IsNot Nothing Then bw.ReportProgress(0, "Stability Line")
                    Dim res As ArrayList = New Utilities.TCP.Methods(Utilities.TCP.CubicCP.EOS_SRK).STABILITY_CURVE(Vm2, VTc2, VPc2, VVc2, Vw2, VKij2)
                    i = 0
                    Do
                        TE.Add(res(i)(0))
                        PE.Add(res(i)(1))
            ...
            ' SLE (Solid-Liquid Equilibrium) curves
            If peoptions.SolidLiquidEquilibrium Then

                Dim nlsle As New Auxiliary.FlashAlgorithms.NestedLoopsSLE
                nlsle.SolidSolution = False

                Dim constprops As New List(Of Interfaces.ICompoundConstantProperties)
                For Each comp2 As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                    constprops.Add(comp2.ConstantProperties)
                Next
                nlsle.CompoundProperties = constprops

                ' Check that at least one compound has fusion data
                Dim hasFusionData = constprops.Any(Function(c) c.TemperatureOfFusion > 0 AndAlso c.EnthalpyOfFusionAtTf > 0)

                If hasFusionData Then

                    ' Determine pressure range from bubble curve (or use defaults)
                    ' Extend to Widom line max pressure if that option is also enabled
                    Dim Pmin = If(PB.Count > 0, PB.Min, 101325.0)
                    Dim Pmax = If(PB.Count > 0, PB.Max, 10000000.0)
                    If peoptions.WidomLine AndAlso CP.Count > 0 Then
                        Dim cpArrSle = CP(0)
                        Dim PcSle = CDbl(cpArrSle(1))
                        Pmax = Math.Max(Pmax, PcSle * 3.0)
                    End If
                    Dim nSteps = 50
                    Dim dPsle = (Pmax - Pmin) / nSteps

                    ' SLE curve 1: Sf=0.999 (nearly all solid, lower temperature curve)
                    Dim prevTref1 As Double = 0
                    For j = 0 To nSteps
                        Dim Psle = Pmin + j * dPsle
                        If bw IsNot Nothing Then
                            If bw.CancellationPending Then Exit For
                            bw.ReportProgress(0, "SLE 1 (" & j + 1 & "/" & nSteps + 1 & ")")
                        End If
                        Try
                            Dim tmpSLE = nlsle.Flash_PSF(Vz, Psle, 1, prevTref1, Me)
                            Dim Tsle = CDbl(tmpSLE(4))
                            If Not Double.IsNaN(Tsle) AndAlso Tsle > 0 Then
                                TSLE1.Add(Tsle)
                                PSLE1.Add(Psle)
                                prevTref1 = Tsle
                            End If
                        Catch ex As Exception
                            ' skip this pressure point
                        End Try
                    Next

                    ' SLE curve 2: Sf=0.001 (nearly all liquid, upper temperature curve)
                    Dim prevTref2 As Double = 0
                    For j = 0 To nSteps
                        Dim Psle = Pmin + j * dPsle
                        If bw IsNot Nothing Then
                            If bw.CancellationPending Then Exit For
                            bw.ReportProgress(0, "SLE 2 (" & j + 1 & "/" & nSteps + 1 & ")")
                        End If
                        Try
                            Dim tmpSLE = nlsle.Flash_PSF(Vz, Psle, 0, prevTref2, Me)
                            Dim Tsle = CDbl(tmpSLE(4))
                            If Not Double.IsNaN(Tsle) AndAlso Tsle > 0 Then
                                TSLE2.Add(Tsle)
                                PSLE2.Add(Psle)
            ...
            ' Hydrate equilibrium curves (structures I and II)
            If peoptions.Hydrate Then

                Dim matHyd = DirectCast(Me.CurrentMaterialStream, Streams.MaterialStream)
                Dim VzHyd = RET_VMOL(Phase.Mixture)
                Dim VnHyd = RET_VNAMES()
                Dim m_auxHyd As New NaturalGasHydrates.AuxMethods
                Dim VidsHyd = m_auxHyd.GetIDsForHydrateCalculation(VnHyd)

                Dim PminHyd = 101325.0
                Dim PmaxHyd = If(PB.Count > 0, PB.Max, 10000000.0)
                If CP.Count > 0 Then
                    PmaxHyd = Math.Max(PmaxHyd, CDbl(CP(0)(1)) * 1.3)
                End If
                Dim nStepsHyd = 50
                Dim dPHyd = (PmaxHyd - PminHyd) / nStepsHyd

                For jHyd = 0 To nStepsHyd
                    Dim PhHyd = PminHyd + jHyd * dPHyd
                    If bw IsNot Nothing Then
                        If bw.CancellationPending Then Exit For
                        bw.ReportProgress(0, "Hydrate (" & jHyd + 1 & "/" & nStepsHyd + 1 & ")")
                    End If
                    Try
                        Dim ThHyd As Object
                        Select Case peoptions.HydrateModel
                            Case 0
                                ThHyd = New NaturalGasHydrates.vdwP_PP(matHyd).HYD_vdwP2T(PhHyd, VzHyd, VidsHyd, peoptions.HydrateVaporOnly)
                            Case 1
                                ThHyd = New NaturalGasHydrates.KlaudaSandler(matHyd).HYD_KS2T(PhHyd, VzHyd, VidsHyd, peoptions.HydrateVaporOnly)
                            Case 2
                                ThHyd = New NaturalGasHydrates.ChenGuo(matHyd).HYD_CG2T(PhHyd, VzHyd, VidsHyd, peoptions.HydrateVaporOnly)
                            Case Else
                                ThHyd = New NaturalGasHydrates.KlaudaSandlerMOD(matHyd).HYD_KS2T(PhHyd, VzHyd, VidsHyd, peoptions.HydrateVaporOnly)
                        End Select
                        Dim TsI = CDbl(ThHyd(0))
                        Dim TsII = CDbl(ThHyd(1))
                        If Not Double.IsNaN(TsI) AndAlso TsI > 0 Then
                            THsI.Add(TsI)
                            PHsI.Add(PhHyd)
                        End If
                        If Not Double.IsNaN(TsII) AndAlso TsII > 0 Then
                            THsII.Add(TsII)
                            PHsII.Add(PhHyd)
                        End If
                    Catch ex As Exception
            ...
            ' Widom line: loci of Cp and κT maxima in the supercritical region
            If peoptions.WidomLine Then

                ' Need critical point to define supercritical region
                If CP.Count > 0 Then

                    Dim cpArr = CP(0)
                    Dim Tc_cp = cpArr(0) ' K
                    Dim Pc_cp = cpArr(1) ' Pa

                    Dim PmaxW = Pc_cp * 3.0
                    Dim dT = 0.1 ' K, for numerical Cp derivative

                    ' Adaptive pressure grid: dense near Pc, coarser far away
                    ' Near Pc the Cp/κT peaks are extremely sharp, so we need fine steps
                    Dim pressures As New List(Of Double)
                    ' Phase 1: very fine steps near Pc (first 10% of Pc above critical)
                    Dim P1max = Pc_cp * 1.1
                    Dim nFine = 20
                    For j = 1 To nFine
                        pressures.Add(Pc_cp + j * (P1max - Pc_cp) / nFine)
                    Next
                    ' Phase 2: medium steps (10% to 50% above Pc)
                    Dim P2max = Pc_cp * 1.5
                    Dim nMed = 15
                    For j = 1 To nMed
                        pressures.Add(P1max + j * (P2max - P1max) / nMed)
                    Next
                    ' Phase 3: coarse steps (50% to 200% above Pc)
                    Dim nCoarse = 15
                    For j = 1 To nCoarse
                        pressures.Add(P2max + j * (PmaxW - P2max) / nCoarse)
                    Next

                    ' Track previous successful temperatures to narrow search window
                    Dim prevTCp As Double = Tc_cp
                    Dim prevTBetaT As Double = Tc_cp

                    Dim gr = (Math.Sqrt(5) + 1) / 2

                    For j = 0 To pressures.Count - 1
                        Dim Pw = pressures(j)

                        If bw IsNot Nothing Then
                            If bw.CancellationPending Then Exit For
                            bw.ReportProgress(0, "Widom (" & j + 1 & "/" & pressures.Count & ")")
                        End If

                        Try
                            ' Adaptive search window centered on previous successful temperature
                            ' Near Pc: peak is very close to Tc, so window must be tight
                            ' Far from Pc: peak shifts right, window can be wider
                            Dim relP = (Pw - Pc_cp) / Pc_cp ' relative distance from Pc
                            Dim halfWindow = Tc_cp * Math.Max(0.01, Math.Min(0.3, relP * 0.5))

                            ' --- Cp maximum ---
                            Dim TlowCp = Math.Max(Tc_cp * 0.99, prevTCp - halfWindow)
                            Dim ThighCp = prevTCp + halfWindow
                            Dim nSweepCp = 40
                            Dim dTsCp = (ThighCp - TlowCp) / nSweepCp

                            Dim bestTCp As Double = prevTCp, bestCp As Double = Double.MinValue
                            For k = 0 To nSweepCp
                                Dim Tw = TlowCp + k * dTsCp
                                Dim Hplus = DW_CalcEnthalpy(Vz, Tw + dT, Pw, State.Vapor)
                                Dim Hminus = DW_CalcEnthalpy(Vz, Tw - dT, Pw, State.Vapor)
                                Dim CpVal = (Hplus - Hminus) / (2.0 * dT)
                                If CpVal > bestCp Then
                                    bestCp = CpVal
                                    bestTCp = Tw
                                End If
                            Next
                            ...
                            If bestTCp > TlowCp + dTsCp AndAlso bestTCp < ThighCp - dTsCp Then
                                Dim a = bestTCp - dTsCp, b = bestTCp + dTsCp
                                For iter = 0 To 40
                                    Dim c = b - (b - a) / gr
                                    Dim d = a + (b - a) / gr
                                    Dim Hpc = DW_CalcEnthalpy(Vz, c + dT, Pw, State.Vapor)
                                    Dim Hmc = DW_CalcEnthalpy(Vz, c - dT, Pw, State.Vapor)
                                    Dim Cpc = (Hpc - Hmc) / (2.0 * dT)
                                    Dim Hpd = DW_CalcEnthalpy(Vz, d + dT, Pw, State.Vapor)
                                    Dim Hmd = DW_CalcEnthalpy(Vz, d - dT, Pw, State.Vapor)
                                    Dim Cpd = (Hpd - Hmd) / (2.0 * dT)
                                    If Cpc < Cpd Then a = c Else b = d
                                    If Math.Abs(b - a) < 0.001 Then Exit For
                                Next
                                bestTCp = (a + b) / 2.0
                            End If

                            If bestTCp > 0 AndAlso bestCp > 0 Then
                                TWidomCp.Add(bestTCp)
                                PWidomCp.Add(Pw)
                                prevTCp = bestTCp
                            End If

                            ' --- κT maximum ---
                            Dim TlowBt = Math.Max(Tc_cp * 0.99, prevTBetaT - halfWindow)
                            Dim ThighBt = prevTBetaT + halfWindow
                            Dim nSweepBt = 40
                            Dim dTsBt = (ThighBt - TlowBt) / nSweepBt

                            Dim bestTBetaT As Double = prevTBetaT, bestBetaT As Double = Double.MinValue
                            For k = 0 To nSweepBt
                                Dim Tw = TlowBt + k * dTsBt
                                Dim betaTVal = CalcIsothermalCompressibility(Vz, Tw, Pw, PhaseName.Vapor)
                                If betaTVal > bestBetaT Then
                                    bestBetaT = betaTVal
                                    bestTBetaT = Tw
                                End If
                            Next
                            ...
                            ' Refine with golden section search
                            If bestTBetaT > TlowBt + dTsBt AndAlso bestTBetaT < ThighBt - dTsBt Then
                                Dim a = bestTBetaT - dTsBt, b = bestTBetaT + dTsBt
                                For iter = 0 To 40
                                    Dim c = b - (b - a) / gr
                                    Dim d = a + (b - a) / gr
                                    Dim btc = CalcIsothermalCompressibility(Vz, c, Pw, PhaseName.Vapor)
                                    Dim btd = CalcIsothermalCompressibility(Vz, d, Pw, PhaseName.Vapor)
                                    If btc < btd Then a = c Else b = d
                                    If Math.Abs(b - a) < 0.001 Then Exit For
                                Next
                                bestTBetaT = (a + b) / 2.0
                            End If

                            If bestTBetaT > 0 AndAlso bestBetaT > 0 Then
                                TWidomBetaT.Add(bestTBetaT)
                                PWidomBetaT.Add(Pw)
                                prevTBetaT = bestTBetaT
                            End If
                    ...
                    ' Insert critical point as the first point in both Widom lines
                    If TWidomCp.Count > 0 Then
                        TWidomCp.Insert(0, Tc_cp)
                        PWidomCp.Insert(0, Pc_cp)
                    End If
                    If TWidomBetaT.Count > 0 Then
                        TWidomBetaT.Insert(0, Tc_cp)
                        PWidomBetaT.Insert(0, Pc_cp)
                    End If
            ...
            ' Build averaged Widom line from Cp and κT lines
            Dim TWidomAvg As New List(Of Double)
            Dim PWidomAvg As New List(Of Double)
            If TWidomCp.Count > 0 AndAlso TWidomBetaT.Count > 0 Then
                ' Both lines share the same pressure grid (plus CP at index 0)
                Dim nMin = Math.Min(TWidomCp.Count, TWidomBetaT.Count)
                For j = 0 To nMin - 1
                    TWidomAvg.Add((TWidomCp(j) + TWidomBetaT(j)) / 2.0)
                    PWidomAvg.Add((PWidomCp(j) + PWidomBetaT(j)) / 2.0)
                Next
            End If

            ' Filter spurious points from all curves
            Dim Tref = TCR
            If TVB.Count > 0 Then Tref = Math.Max(Tref, TVB.Max())
            If TVD.Count > 0 Then Tref = Math.Max(Tref, TVD.Where(Function(t1) t1 < TCR * 2.0).DefaultIfEmpty(TCR).Max())
            Dim Tmax_filter = Tref * 2.0
            Dim Pmax_filter = PCR * 5.0

            ' Dew curve
            For i = TVD.Count - 1 To 0 Step -1
                If TVD(i) > Tmax_filter OrElse TVD(i) <= 0 OrElse PO(i) <= 0 OrElse PO(i) > Pmax_filter Then
                    TVD.RemoveAt(i) : PO.RemoveAt(i) : HO.RemoveAt(i) : SO.RemoveAt(i) : VO.RemoveAt(i)
                End If
            Next

            ' Bubble curve
            For i = TVB.Count - 1 To 0 Step -1
                If TVB(i) > Tmax_filter OrElse TVB(i) <= 0 OrElse PB(i) <= 0 OrElse PB(i) > Pmax_filter Then
                    TVB.RemoveAt(i) : PB.RemoveAt(i) : HB.RemoveAt(i) : SB.RemoveAt(i) : VB.RemoveAt(i)
                End If
            Next

            ' Quality line
            For i = TQ.Count - 1 To 0 Step -1
                If TQ(i) > Tmax_filter OrElse TQ(i) <= 0 OrElse PQ(i) <= 0 OrElse PQ(i) > Pmax_filter Then
                    TQ.RemoveAt(i) : PQ.RemoveAt(i)
                End If
            Next

            ' Remove self-intersecting loops from dew and bubble curves
            ' Normalizes T and P to comparable scales, then detects where a curve segment
            ' crosses an earlier segment. The points between the two crossing segments (the loop) are removed.
            Dim Tscale = If(TCR > 0, TCR, 1.0)
            Dim Pscale = If(PCR > 0, PCR, 1.0)

            ' Dew curve loop removal
            Dim loopFound = True
            While loopFound AndAlso TVD.Count > 3
                loopFound = False
                For i = 2 To TVD.Count - 1
                    If loopFound Then Exit For
                    Dim t1n = TVD(i - 1) / Tscale, p1n = PO(i - 1) / Pscale
                    Dim t2n = TVD(i) / Tscale, p2n = PO(i) / Pscale
                    For j = 0 To i - 2
                        Dim t3n = TVD(j) / Tscale, p3n = PO(j) / Pscale
                        Dim t4n = TVD(j + 1) / Tscale, p4n = PO(j + 1) / Pscale
                        ' Check if segment (t1n,p1n)-(t2n,p2n) crosses segment (t3n,p3n)-(t4n,p4n)
                        Dim d1t = t2n - t1n, d1p = p2n - p1n
                        Dim d2t = t4n - t3n, d2p = p4n - p3n
                        Dim denom = d1t * d2p - d1p * d2t
                        If Math.Abs(denom) > 0.000000000000001 Then
                            Dim dt = t3n - t1n, dp = p3n - p1n
                            Dim ua = (dt * d2p - dp * d2t) / denom
                            Dim ub = (dt * d1p - dp * d1t) / denom
                            If ua > 0.0 AndAlso ua < 1.0 AndAlso ub > 0.0 AndAlso ub < 1.0 Then
                                ' Intersection found: remove points from j+1 to i-1 (the loop)
                                For k = i - 1 To j + 1 Step -1
                                    TVD.RemoveAt(k) : PO.RemoveAt(k) : HO.RemoveAt(k) : SO.RemoveAt(k) : VO.RemoveAt(k)
                                Next
                                loopFound = True
                                Exit For
                            End If
            ...
            ' Bubble curve loop removal
            loopFound = True
            While loopFound AndAlso TVB.Count > 3
                loopFound = False
                For i = 2 To TVB.Count - 1
                    If loopFound Then Exit For
                    Dim t1n = TVB(i - 1) / Tscale, p1n = PB(i - 1) / Pscale
                    Dim t2n = TVB(i) / Tscale, p2n = PB(i) / Pscale
                    For j = 0 To i - 2
                        Dim t3n = TVB(j) / Tscale, p3n = PB(j) / Pscale
                        Dim t4n = TVB(j + 1) / Tscale, p4n = PB(j + 1) / Pscale
                        Dim d1t = t2n - t1n, d1p = p2n - p1n
                        Dim d2t = t4n - t3n, d2p = p4n - p3n
                        Dim denom = d1t * d2p - d1p * d2t
                        If Math.Abs(denom) > 0.000000000000001 Then
                            ...
                            If ua > 0.0 AndAlso ua < 1.0 AndAlso ub > 0.0 AndAlso ub < 1.0 Then
                                For k = i - 1 To j + 1 Step -1
                                    TVB.RemoveAt(k) : PB.RemoveAt(k) : HB.RemoveAt(k) : SB.RemoveAt(k) : VB.RemoveAt(k)
                                Next
                                loopFound = True
                                Exit For
                            End If
            ...
            If TVD.Count >= 3 Then
                ...
                If cricoIdx >= 0 AndAlso cricoIdx < TVD.Count - 2 Then
                    Dim di = cricoIdx + 1
                    While di < TVD.Count
                        Dim distCP = Math.Sqrt(((TVD(di) - TCR) / Tscale) ^ 2 + ((PO(di) - PCR) / Pscale) ^ 2)
                        Dim distCPprev = Math.Sqrt(((TVD(di - 1) - TCR) / Tscale) ^ 2 + ((PO(di - 1) - PCR) / Pscale) ^ 2)
                        If distCPprev < 0.1 AndAlso distCP > distCPprev * 1.01 Then
                            TVD.RemoveAt(di) : PO.RemoveAt(di) : HO.RemoveAt(di) : SO.RemoveAt(di) : VO.RemoveAt(di)
                        Else
                            di += 1
                        End If
                    End While
            ...
            If TVB.Count >= 3 Then
                ...
                If cricoIdx >= 0 AndAlso cricoIdx < TVB.Count - 2 Then
                    Dim bi = cricoIdx + 1
                    While bi < TVB.Count
                        Dim distCP = Math.Sqrt(((TVB(bi) - TCR) / Tscale) ^ 2 + ((PB(bi) - PCR) / Pscale) ^ 2)
                        Dim distCPprev = Math.Sqrt(((TVB(bi - 1) - TCR) / Tscale) ^ 2 + ((PB(bi - 1) - PCR) / Pscale) ^ 2)
                        If distCPprev < 0.1 AndAlso distCP > distCPprev * 1.01 Then
                            TVB.RemoveAt(bi) : PB.RemoveAt(bi) : HB.RemoveAt(bi) : SB.RemoveAt(bi) : VB.RemoveAt(bi)
                        Else
                            bi += 1
                        End If
                    End While
        ...
        Public Overridable Function DW_ReturnBinaryEnvelope(ByVal parameters As Object, Optional ByVal bw As System.ComponentModel.BackgroundWorker = Nothing) As Object
            ...
            Select Case tipocalc
                ...
                Case "T-x-y"
                    ...
                    Dim pxsp1, pxsp2, pysp As New ArrayList 'lle spinodal
                    Dim pxcs, pycs As New ArrayList 'lle consolute (UCST/LCST)
                    ...
                    ' Bounds of the liquid region at this pressure: the highest melting point below, the
                    ' highest saturation temperature above. Both the dome and the three-phase search use them.
                    Dim TLLEmin, TLLEmax As Double
                    TLLEmax = 0.0
                    For j As Integer = 0 To n
                        Dim tsat As Double = Me.AUX_TSATi(P, j)
                        If tsat > TLLEmax Then TLLEmax = tsat
                    Next
                    TLLEmin = MathEx.Common.Max(Me.RET_VTF())
                    If TLLEmin <= 0.0# Then TLLEmin = 0.5 * TLLEmax

                    ' The three-phase point {T3, x1_L1, x1_L2, y3}: vapour in equilibrium with both liquids.
                    ' Resolving it once up front settles the flat stretch of the bubble curve, the point the
                    ' dew curve has to reach, and the ceiling of the dome, all of which meet there.
                    Dim apkg = TryCast(Me, ActivityCoefficientPropertyPackage)
                    Dim tpp3 As Double() = Nothing
                    If apkg IsNot Nothing And n = 1 And TLLEmax > TLLEmin Then
                        Try
                            tpp3 = apkg.DW_CalculateThreePhaseTemperature(P, TLLEmin, TLLEmax)
                        Catch ex As Exception
                            tpp3 = Nothing
                        End Try
                    End If
                    ...
                    If VLE Then
                        ...
                        Do
                            ...
                            If tpp3 IsNot Nothing AndAlso x > tpp3(1) AndAlso x < tpp3(2) Then
                                ' Inside the miscibility gap the liquid splits into two phases of fixed
                                ' composition, so the mixture boils at the three-phase temperature whatever
                                ' the feed: the bubble curve is flat across the whole gap. Taking it from the
                                ' three-phase point rather than from the flash removes the dips the flash left
                                ' wherever it failed to find the split and returned the metastable
                                ' single-liquid bubble point instead, which lies below the three-phase
                                ' temperature - by up to 1.3 K on UNIFAC Methanol/Cyclohexane at 1 atm.
                                py1(py1.Count - 1) = tpp3(0)
                                unstable = True
                                ui.Add(px.Count - 1)
                                ut.Add(tpp3(0))
                            ElseIf tmp1(7) > 0 Then
                                unstable = True
                                ui.Add(px.Count - 1)
                                ut.Add(tmp1(4))
                            End If
                        ...
                        ' The dew curve reaches its minimum at the azeotropic vapour, where it meets the flat
                        ' stretch of the bubble curve: there the vapour is in equilibrium with both liquids at
                        ' once. That minimum is a cusp, not a smooth turning point - the incipient liquid
                        ' switches from one side of the gap to the other across it - so a composition grid
                        ' practically never lands on it and the straight line between the neighbouring points
                        ' cuts the corner, leaving the two curves that must touch drawn apart (0.32 K apart on
                        ' UNIFAC Methanol/Cyclohexane at 1 atm with 40 steps). Pin the exact point, which the
                        ' three-phase calculation already produced. One point serves both curves, since the
                        ' azeotropic vapour lies inside the gap, where the bubble curve is the flat stretch too.
                        If tpp3 IsNot Nothing AndAlso tpp3(3) > tpp3(1) AndAlso tpp3(3) < tpp3(2) Then
                            Dim ins As Integer = px.Count
                            For k As Integer = 0 To px.Count - 1
                                If CDbl(px(k)) > tpp3(3) Then
                                    ins = k
                                    Exit For
                                End If
                            Next
                            px.Insert(ins, tpp3(3))
                            py1.Insert(ins, tpp3(0))
                            py2.Insert(ins, tpp3(0))
                        End If
                    End If
                    ...
                    If LLE Then
                        ...
                        If apkg IsNot Nothing And n = 1 Then

                            ' Trace the dome BY TEMPERATURE. The binodal is a property of the temperature and
                            ' the model alone, so each point is solved straight from the equal-activity
                            ' condition, seeded from the spinodal, with no feed involved. The legacy path
                            ' below instead fixes one feed and sweeps temperature, harvesting flash tie-lines,
                            ' which truncates the dome by construction: that feed leaves the binodal before
                            ' the consolute point is reached, so the flash reports a single phase and the
                            ' curve stops short of its own apex. It also depends on the flash returning a
                            ' second liquid at all, which is why LLE only ever appeared for a VLLE algorithm
                            ' or a declared immiscible compound.
                            ' The dome ends where the mixture boils. Above the three-phase temperature what
                            ' the binodal would do is hypothetical, hidden under the vapour - and so is any
                            ' consolute point up there, which is why the search below is bounded by it too.
                            Dim TDomeMin As Double = TLLEmin
                            Dim TDomeMax As Double = TLLEmax
                            If tpp3 IsNot Nothing AndAlso tpp3(0) < TDomeMax Then TDomeMax = tpp3(0)

                            If TDomeMax > TDomeMin Then

                                If bw IsNot Nothing Then bw.ReportProgress(0, "LLE (consolute point)")

                                ' The gap also ends where it closes, at a consolute point. Finding that first
                                ' keeps the sweep on the dome instead of spreading it over temperatures that
                                ' have no gap at all, which left the last point short of the apex and the
                                ' branches visibly not meeting it.
                                Dim cps As New List(Of Double())
                                Try
                                    cps = apkg.DW_CalculateConsolutePoints(TDomeMin, TDomeMax)
                                Catch ex As Exception
                                    SharedClasses.ExceptionProcessing.ExceptionParser.ProcessAndDisplayException(Flowsheet, ex)
                                End Try

                                For Each cp As Double() In cps
                                    pxcs.Add(cp(1))
                                    pycs.Add(cp(0))
                                    ' An upper consolute temperature roofs the dome, a lower one floors it.
                                    ' Which one this is follows from where the gap actually is.
                                    If apkg.DW_CalculateSpinodal(cp(0) - 0.001 * (TDomeMax - TDomeMin)) IsNot Nothing Then
                                        If cp(0) < TDomeMax Then TDomeMax = cp(0)
                                    Else
                                        If cp(0) > TDomeMin Then TDomeMin = cp(0)
                                    End If
                                Next

                                If bw IsNot Nothing Then bw.ReportProgress(0, "LLE (binodal and spinodal)")

                                Try
                                    Dim dome As New List(Of Double())
                                    dome.AddRange(apkg.DW_CalculateLLEDiagram(TDomeMin, TDomeMax, StepCount))
                                    ' Both curves close on the apex: there the two binodal compositions and
                                    ' the two spinodal roots have all merged into the critical one.
                                    For Each cp As Double() In cps
                                        dome.Add(New Double() {cp(0), cp(1), cp(1), cp(1), cp(1)})
                                    Next
                                    dome.Sort(Function(u, v) u(0).CompareTo(v(0)))
                                    For Each pt As Double() In dome
                                        pysp.Add(pt(0))
                                        pxsp1.Add(pt(1))
                                        pxsp2.Add(pt(2))
                                        py3.Add(pt(0))
                    ...
                    If Critical Then
                        ...
                        Dim cpcs As New Utilities.TCP.Methods(Utilities.TCP.CubicCP.EOS_SRK)
                    ...
                    Return New Object() {px, py1, py2, px1l1, px1l2, py3, pxs1, pys1, pxs2, pys2, pxc, pyc, pxsp1, pxsp2, pysp, pxcs, pycs}
                ...
                Case "P-x-y"
                    ...
                    ' The binodal and the three-phase pressure are properties of this temperature alone, so
                    ' they are resolved once here and then serve both the bubble curve and the LLE band.
                    ' The three-phase pressure is the bubble pressure of either binodal composition: the two
                    ' liquids are in equilibrium with the same vapour, so they share it.
                    Dim apkg = TryCast(Me, ActivityCoefficientPropertyPackage)
                    Dim binodal As Double() = Nothing
                    Dim P3 As Double = 0.0#
                    Dim y3 As Double = -1.0#
                    If apkg IsNot Nothing And n = 1 Then
                        Try
                            ' One call settles the whole three-phase state at this temperature: the two
                            ' liquid compositions, the pressure, and the vapour that stands in equilibrium
                            ' with both liquids at once.
                            Dim tpp As Double() = apkg.DW_CalculateThreePhasePressure(T)
                            If tpp IsNot Nothing Then
                                P3 = tpp(0)
                                y3 = tpp(1)
                                binodal = New Double() {tpp(2), tpp(3)}
                                If P3 <= 0.0# Then binodal = Nothing
                            End If
                        Catch ex As Exception
                            binodal = Nothing
                        End Try
                    ...
                    If Not TypeOf Me.FlashBase Is Auxiliary.FlashAlgorithms.NestedLoopsSLE Then
                        ...
                        Do
                            ...
                            Try
                                ...
                                py2.Add(y2)
                                'check if liquid phase is stable.
                                If binodal IsNot Nothing AndAlso x > binodal(0) AndAlso x < binodal(1) Then
                                    ' Inside the miscibility gap the liquid splits into two phases of fixed
                                    ' composition, so the mixture boils at the three-phase pressure whatever
                                    ' the feed: the bubble curve is flat across the whole gap. Reading it from
                                    ' the binodal replaces the stability probe below, which had to flash at
                                    ' every composition, missed some inside the gap, and set the plateau from
                                    ' whatever split the first detected composition happened to converge on -
                                    ' 7.5% high on UNIFAC Methanol/Cyclohexane at 300 K.
                                    py1(py1.Count - 1) = P3
                                    unstable = True
                                    ui.Add(px.Count - 1)
                                    If up.Count = 0 Then up.Add(P3)
                                Else
                        ...
                        ' The dew curve peaks exactly at the azeotropic vapour, where it meets the bubble
                        ' curve's three-phase plateau: there the vapour is in equilibrium with both liquids
                        ' at once, so a drop of each appears together. That peak is a cusp rather than a
                        ' smooth maximum - the incipient liquid switches from one side of the gap to the
                        ' other across it - so a composition grid practically never lands on it, and the
                        ' straight line drawn between the points either side cuts the corner and leaves the
                        ' dew curve stopping short of the plateau it should touch (460 Pa short on UNIFAC
                        ' Methanol/Cyclohexane at 300 K with 40 steps). Pin the exact point instead; the
                        ' three-phase calculation above already produced it. One point serves both curves,
                        ' because the azeotropic vapour necessarily lies inside the gap, where the bubble
                        ' curve is the plateau too.
                        If binodal IsNot Nothing AndAlso y3 > binodal(0) AndAlso y3 < binodal(1) Then
                            Dim ins As Integer = px.Count
                            For k As Integer = 0 To px.Count - 1
                                If CDbl(px(k)) > y3 Then
                                    ins = k
                                    Exit For
                                End If
                            Next
                        ...
                        If LLE And binodal IsNot Nothing Then

                            ' On a P-x-y diagram the two-liquid region is a vertical BAND, not a curve. These
                            ' activity coefficients depend on temperature and composition only, and the Psat/P
                            ' and Poynting factors are shared by both liquid phases and cancel out of the
                            ' equal-activity condition, so at fixed T the binodal does not move with pressure
                            ' at all. Sweeping pressure and flashing, as the branch below does, therefore
                            ' spends its effort recomputing one answer.
                            ' The band stands on the three-phase pressure, where vapour coexists with both
                            ' liquids. That floor is flat and needs no separate calculation: the two binodal
                            ' compositions are in equilibrium with the same vapour, so they share one bubble
                            ' pressure, and either one gives it.
                            If bw IsNot Nothing Then bw.ReportProgress(0, "LLE (binodal and spinodal)")

                            Try
                                ' Two points are all a vertical line needs. The band is drawn to twice the
                                ' three-phase pressure only so that it is visible: the liquid region runs
                                ' upwards from there indefinitely, since pressure does not move the binodal,
                                ' and the bubble curve itself is capped by the heteroazeotrope at exactly
                                ' that same pressure, so the band stands right on the plateau.
                                Dim sp As Double() = apkg.DW_CalculateSpinodal(T)
                                px1l1.Add(binodal(0)) : px1l2.Add(binodal(1)) : py3.Add(P3)
                                px1l1.Add(binodal(0)) : px1l2.Add(binodal(1)) : py3.Add(2.0 * P3)
                                If sp IsNot Nothing Then
                                    pxsp1.Add(sp(0)) : pxsp2.Add(sp(1)) : pysp.Add(P3)
                                    pxsp1.Add(sp(0)) : pxsp2.Add(sp(1)) : pysp.Add(2.0 * P3)
                                End If
                            Catch ex As Exception
                                SharedClasses.ExceptionProcessing.ExceptionParser.ProcessAndDisplayException(Flowsheet, ex)
                            End Try
                    ...
                    Return New Object() {px, py1, py2, px1l1, px1l2, py3, pxsp1, pxsp2, pysp}
...
Final3:
           ...
           cprop.OriginalDB = "KDB" Then
            ...
            ElseIf cprop.OriginalDB = "ChemSep" Or
                ...
                If cprop.VaporizationEnthalpyEquation <> "" And cprop.LiquidHeatCapacityEquation <> "0" And Not cprop.IsIon And Not cprop.IsSalt Then
                    ...
                    End If
                Else
                    If cprop.HVap_A = 0.0# Or Not cprop.HVap_A.IsValidDouble() Then
                        cprop.HVap_A = New Utilities.Hypos.Methods.HYP().DHvb_Vetere(cprop.Critical_Temperature, cprop.Critical_Pressure, cprop.Normal_Boiling_Point)
                        cprop.HVap_A /= cprop.Molar_Weight
                    End If
                    result = cprop.HVap_A * ((1 - Tr) / (1 - tr1)) ^ 0.375
        ...
        Private Shared ReadOnly _costaldVStarWarned As New HashSet(Of String)
        Private Shared ReadOnly _costaldOmegaSrkWarned As New HashSet(Of String)
        ...
        Public Overridable Function RET_VCSTLDCV() As Double()
            ...
            For Each subst In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                ...
                If val(i) = 0.0 Then
                    val(i) = subst.ConstantProperties.Critical_Volume
                    SyncLock _costaldVStarWarned
                        If _costaldVStarWarned.Add(subst.ConstantProperties.Name) Then
                            Flowsheet?.ShowMessage(String.Format("Compound '{0}' is missing its COSTALD characteristic volume (V*); falling back to Critical Volume. COSTALD liquid density results will be approximate.", subst.ConstantProperties.Name), IFlowsheet.MessageType.Warning)
                        End If
                    End SyncLock
                End If
                i += 1
        ...
        Public Overridable Function RET_VCSTLDAF() As Double()
            ...
            For Each subst In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                ...
                If val(i) = 0.0 Then
                    ...
                    SyncLock _costaldOmegaSrkWarned
                        If _costaldOmegaSrkWarned.Add(subst.ConstantProperties.Name) Then
                            Flowsheet?.ShowMessage(String.Format("Compound '{0}' is missing its COSTALD SRK acentric factor; falling back to the standard Acentric Factor. COSTALD liquid density results will be approximate.", subst.ConstantProperties.Name), IFlowsheet.MessageType.Warning)
                        End If
                    End SyncLock
                End If
                i += 1
...
#Region "   CAPE-OPEN ICapeUtilities Implementation"
        ...
        ''' <summary>
        ''' Opens the CAPE-OPEN parameter editor. Does nothing where the editors are not built.
        ''' </summary>
        Partial Private Sub ShowCapeOpenEditor()
        End Sub
...
#Region "   XML data persistence"
        ...
        Public Overridable Function LoadData(data As System.Collections.Generic.List(Of System.Xml.Linq.XElement)) As Boolean Implements Interfaces.ICustomXMLSerialization.LoadData
            ...
            e1 = (From el As XElement In data Select el Where el.Name = "UseImmiscibleListForLiquid2InitialEstimates").FirstOrDefault
            If e1 IsNot Nothing Then UseImmiscibleListForLiquid2InitialEstimates = e1.Value
            ...
            If Not elf Is Nothing Then
                ...
                End If
                If Not FlashSettings.ContainsKey(Interfaces.Enums.FlashSetting.NestedLoops_v2) Then
                    FlashSettings.Add(Interfaces.Enums.FlashSetting.NestedLoops_v2, False)
                End If
            End If
        ...
        Public Overridable Function SaveData() As System.Collections.Generic.List(Of System.Xml.Linq.XElement) Implements Interfaces.ICustomXMLSerialization.SaveData
            ...
            With elements
                ...
                .Add(New XElement("UseImmiscibleListForLiquid2InitialEstimates", UseImmiscibleListForLiquid2InitialEstimates))
...
#Region "   DWSIM IPropertyPackage implementation"
        ...
        ''' <summary>
        ''' Opens the grouped editor of this package. Does nothing where the editors are not built.
        ''' </summary>
        Partial Private Sub ShowGroupedEditor()
        End Sub
        ...
        ''' <summary>
        ''' Opens the property overrides editor. Leaves the argument alone where the editors are
        ''' not built.
        ''' </summary>
        Partial Private Sub BuildAdvancedEditor(ByRef editor As Object)
        End Sub
        ...
        Public Overridable ReadOnly Property ImplementsCrossPlatformEditor As Boolean = False Implements IPropertyPackage.ImplementsCrossPlatformEditor
...
#Region "   Derivatives"
        ...
        Private Shared _analyticDerivsDisabled As Integer = -1

        ''' <summary>
        ''' Global runtime kill-switch for analytical thermodynamic derivatives. When the environment
        ''' variable DWSIM_DISABLE_ANALYTIC_DERIVS is set to "1", packages that implement analytical
        ''' derivatives must fall back to the finite-difference base implementations. This allows working
        ''' around a field issue without a rebuild even when a package advertises analytical derivatives.
        ''' </summary>
        Protected ReadOnly Property AnalyticalDerivativesDisabled As Boolean
            Get
                If _analyticDerivsDisabled = -1 Then
                    _analyticDerivsDisabled = If(Environment.GetEnvironmentVariable("DWSIM_DISABLE_ANALYTIC_DERIVS") = "1", 1, 0)
                End If
                Return _analyticDerivsDisabled = 1
            End Get
        End Property
        ...
        ''' <summary>
        ''' Temperature derivative of the natural logarithm of the fugacity coefficients, d(ln phi_i)/dT,
        ''' for the given phase. Base implementation is a forward finite difference; EOS/activity packages
        ''' override this with a closed-form expression. Distinct from DW_CalcdFugCoeffdT, which returns
        ''' d(phi)/dT (not the logarithm).
        ''' </summary>
        Public Overridable Function DW_CalcdLnFugCoeffdT(ByVal Vx As Double(), ByVal T As Double, ByVal P As Double, st As State) As Double()
            ...
            For i As Integer = 0 To n
                deriv(i) = (Math.Log(F2(i)) - Math.Log(F1(i))) / 0.01
            Next
        ...
        ''' <summary>
        ''' Composition (mole-number) derivative of the natural logarithm of the fugacity coefficients,
        ''' d(ln phi_i)/dn_j, for the given phase, evaluated on a total-moles = 1 basis. Returns an
        ''' (nc x nc) matrix. Base implementation is a finite difference in mole numbers; EOS/activity
        ''' packages override this with the closed-form partial-molar expression.
        ''' </summary>
        Public Overridable Function DW_CalcdLnFugCoeffdn(ByVal Vx As Double(), ByVal T As Double, ByVal P As Double, st As State) As Double(,)

            Dim n As Integer = Vx.Length - 1
            Dim deltan As Double = 0.0000001
            Dim deriv(n, n) As Double

            Dim lnphi1(n) As Double
            Dim F1 As Double() = DW_CalcFugCoeff(Vx, T, P, st)
            For i As Integer = 0 To n
                lnphi1(i) = Math.Log(F1(i))
            Next

            For j As Integer = 0 To n
                Dim newn(n) As Double
                For k As Integer = 0 To n
                    newn(k) = Vx(k)
                Next
                newn(j) += deltan
                Dim newx As Double() = newn.NormalizeY()
                Dim F2 As Double() = DW_CalcFugCoeff(newx, T, P, st)
                For i As Integer = 0 To n
                    deriv(i, j) = (Math.Log(F2(i)) - lnphi1(i)) / deltan
                Next
            Next
        ...
        ''' <summary>
        ''' Composition (mole-number) derivative of the equilibrium K-values, d(K_i)/dn_j, with respect to
        ''' the mole numbers of the phase indicated by <paramref name="withRespectTo"/> (State.Liquid perturbs
        ''' Vx, State.Vapor perturbs Vy), on a total-moles = 1 basis. Returns an (nc x nc) matrix. Base
        ''' implementation is a finite difference; EOS/activity packages override it using the analytical
        ''' d(ln phi)/dn (or d(ln gamma)/dx) building blocks.
        ''' </summary>
        Public Overridable Function DW_CalcdKdComposition(ByVal Vx As Double(), ByVal Vy As Double(), ByVal T As Double, ByVal P As Double, ByVal withRespectTo As State, Optional ByVal type As String = "LV") As Double(,)

            Dim n As Integer = Vx.Length - 1
            Dim deltan As Double = 0.0000001
            Dim deriv(n, n) As Double

            Dim K1 As Double() = DW_CalcKvalue(Vx, Vy, T, P, type)

            For j As Integer = 0 To n
                Dim nx As Double() = Vx.Clone()
                Dim ny As Double() = Vy.Clone()
                ...
                Else
                    ...
                    ny = ny.NormalizeY()
                End If
                Dim K2 As Double() = DW_CalcKvalue(nx, ny, T, P, type)
                For i As Integer = 0 To n
                    deriv(i, j) = (K2(i) - K1(i)) / deltan
                Next
            Next
