...
Namespace PropertyPackages
    ...
    <System.Runtime.InteropServices.Guid(PengRobinson1978PropertyPackage.ClassId)>
    <System.Serializable()> Public Partial Class PengRobinson1978PropertyPackage
...
#Region "    DWSIM Functions"
        ...
        Public Overrides Function DW_CalculateCriticalPoints() As List(Of Double())
            ...
            ' A component at exactly zero mole fraction makes the critical Hessian singular (its
            ' ln(n) / 1/n terms blow up) and the solver returns a garbage point. A trace amount has no
            ' measurable effect on the critical point, so floor the zero fractions and renormalize.
            ...
            Dim cproutine = New Utilities.TCP.Methods(Utilities.TCP.CubicCP.EOS_PR78)
...
#Region "   Analytical Derivatives"
        ...
        Public Overrides Function DW_CalcdLnFugCoeffdT(ByVal Vx As Double(), ByVal T As Double, ByVal P As Double, st As State) As Double()
            ...
            Dim phase As Integer = If(st = State.Liquid, 0, 1)
            Dim res = ThermoPlugs.CubicEOSDerivatives.Calc(ThermoPlugs.CubicEOSDerivatives.EOS_PR78, T, P, Vx, RET_VKij, RET_VTC, RET_VPC, RET_VW, phase)
        ...
        Public Overrides Function DW_CalcdLnFugCoeffdn(ByVal Vx As Double(), ByVal T As Double, ByVal P As Double, st As State) As Double(,)
            ...
            Dim phase As Integer = If(st = State.Liquid, 0, 1)
            Dim res = ThermoPlugs.CubicEOSDerivatives.Calc(ThermoPlugs.CubicEOSDerivatives.EOS_PR78, T, P, Vx, RET_VKij, RET_VTC, RET_VPC, RET_VW, phase)
            Return DirectCast(res(2), Double(,))
        End Function
        ...
        Public Overrides Function DW_CalcdKdT(ByVal Vx As Double(), ByVal Vy As Double(), ByVal T As Double, ByVal P As Double, Optional ByVal type As String = "LV") As Double()
            ...
            Dim resL = ThermoPlugs.CubicEOSDerivatives.Calc(ThermoPlugs.CubicEOSDerivatives.EOS_PR78, T, P, Vx, RET_VKij, RET_VTC, RET_VPC, RET_VW, 0)
            Dim resV = ThermoPlugs.CubicEOSDerivatives.Calc(ThermoPlugs.CubicEOSDerivatives.EOS_PR78, T, P, Vy, RET_VKij, RET_VTC, RET_VPC, RET_VW, 1)
        ...
        Public Overrides Function DW_CalcdKdComposition(ByVal Vx As Double(), ByVal Vy As Double(), ByVal T As Double, ByVal P As Double, ByVal withRespectTo As State, Optional ByVal type As String = "LV") As Double(,)
            If AnalyticalDerivativesDisabled OrElse type <> "LV" Then Return MyBase.DW_CalcdKdComposition(Vx, Vy, T, P, withRespectTo, type)
            Dim n As Integer = Vx.Length - 1
            Dim K As Double() = DW_CalcKvalue(Vx, Vy, T, P, type)
            Dim deriv(n, n) As Double
            If withRespectTo = State.Liquid Then
                Dim resL = ThermoPlugs.CubicEOSDerivatives.Calc(ThermoPlugs.CubicEOSDerivatives.EOS_PR78, T, P, Vx, RET_VKij, RET_VTC, RET_VPC, RET_VW, 0)
                Dim dn = DirectCast(resL(2), Double(,))
                For i As Integer = 0 To n
            ...
            Else
                Dim resV = ThermoPlugs.CubicEOSDerivatives.Calc(ThermoPlugs.CubicEOSDerivatives.EOS_PR78, T, P, Vy, RET_VKij, RET_VTC, RET_VPC, RET_VW, 1)
                Dim dn = DirectCast(resV(2), Double(,))
                For i As Integer = 0 To n
                    For j As Integer = 0 To n
                        deriv(i, j) = -K(i) * dn(i, j)
                    Next
