...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class LeeKeslerPlocker
        ...
        Function ESTIMAR_Vr2(ByVal TIPO As String, ByVal Pr As Double, ByVal Tr As Double, ByVal B As Double, ByVal C As Double, ByVal D As Double, ByVal c4 As Double, ByVal beta As Double, ByVal gamma As Double)
            ...
            i = 0
            If TIPO <> "L" Then
                'The vapour root satisfies Vr = Z Tr / Pr with Z < 2, so the scan from Vr = 1001 down
                'finds no sign change above max(10, 2 Tr / Pr). Start on the first grid point below that,
                'on the same grid, with the same total step budget: same bracket, same root.
...
Final3:
        ...
        ''' <summary>
        ''' Temperature derivatives d(z)/dTr and d(ln fug)/dTr of one Lee-Kesler reference fluid, obtained by
        ''' implicit differentiation of the BWR equation (Vr is the reduced volume already solved for z_ref).
        ''' Returns {dz/dTr, dlnfug/dTr}.
        ''' </summary>
        Private Function BWR_dTr(ByVal Tr As Double, ByVal Pr As Double, ByVal z_ref As Double,
                                 ByVal b1 As Double, ByVal b2 As Double, ByVal b3 As Double, ByVal b4 As Double,
                                 ByVal c1 As Double, ByVal c2 As Double, ByVal c3 As Double, ByVal c4 As Double,
                                 ByVal d1 As Double, ByVal d2 As Double, ByVal beta As Double, ByVal gamma As Double) As Double()

            Dim Vr As Double = z_ref * Tr / Pr
            Dim B As Double = b1 - b2 / Tr - b3 / Tr ^ 2 - b4 / Tr ^ 3
            Dim C As Double = c1 - c2 / Tr + c3 / Tr ^ 3
            Dim D As Double = d1 + d2 / Tr
            Dim dB As Double = b2 / Tr ^ 2 + 2 * b3 / Tr ^ 3 + 3 * b4 / Tr ^ 4
            Dim dC As Double = c2 / Tr ^ 2 - 3 * c3 / Tr ^ 4
            Dim dD As Double = -d2 / Tr ^ 2

            Dim w2 As Double = 1.0 / Vr ^ 2
            Dim exg As Double = Math.Exp(-gamma * w2)
            Dim BWRc4 As Double = c4 / Tr ^ 3 * (beta + gamma * w2) * exg * w2
            Dim dQ_dw2 As Double = exg * (gamma * w2 + (beta + gamma * w2) * (1.0 - gamma * w2))
            Dim dBWRc4_dVr As Double = c4 / Tr ^ 3 * dQ_dw2 * (-2.0 / Vr ^ 3)
            Dim dBWRc4_dTr As Double = -3.0 / Tr * BWRc4

            ' R(Vr,Tr) = Pr*Vr/Tr - 1 - B/Vr - C/Vr^2 - D/Vr^5 - BWRc4 = 0  (at fixed Pr)
            Dim dR_dVr As Double = Pr / Tr + B / Vr ^ 2 + 2 * C / Vr ^ 3 + 5 * D / Vr ^ 6 - dBWRc4_dVr
            Dim dR_dTr As Double = -Pr * Vr / Tr ^ 2 - dB / Vr - dC / Vr ^ 2 - dD / Vr ^ 5 - dBWRc4_dTr
            Dim dVr As Double = -dR_dTr / dR_dVr

            Dim dz As Double = Pr / Tr * dVr - Pr * Vr / Tr ^ 2

            ' lnfug = z - 1 - ln(z) + B/Vr + C/(2Vr^2) + D/(5Vr^5) + E
            ' E = c4/(2 Tr^3 gamma) * (beta+1 - (beta+1+gamma/Vr^2) exp(-gamma/Vr^2))
            Dim u As Double = gamma * w2
            Dim Ebr As Double = beta + 1.0 - (beta + 1.0 + u) * exg
            Dim E As Double = c4 / (2 * Tr ^ 3 * gamma) * Ebr
            Dim dEbr_dVr As Double = exg * (beta + u) * (-2.0 * gamma / Vr ^ 3)
            Dim dE As Double = -3.0 / Tr * E + c4 / (2 * Tr ^ 3 * gamma) * dEbr_dVr * dVr

            Dim dlnfug As Double = dz * (1.0 - 1.0 / z_ref) +
                                   (dB / Vr - B / Vr ^ 2 * dVr) +
                                   (dC / (2 * Vr ^ 2) - C / Vr ^ 3 * dVr) +
                                   (dD / (5 * Vr ^ 5) - D / Vr ^ 6 * dVr) + dE

            Return New Double() {dz, dlnfug}

        End Function

        ''' <summary>
        ''' Pressure derivatives dz/dPr and d(ln fug)/dPr of one Lee-Kesler reference fluid at fixed Tr
        ''' (B, C, D and the c4 term have no explicit Pr dependence; only Vr moves with Pr). Returns
        ''' {dz/dPr, dlnfug/dPr}.
        ''' </summary>
        Private Function BWR_dPr(ByVal Tr As Double, ByVal Pr As Double, ByVal z_ref As Double,
                                 ByVal b1 As Double, ByVal b2 As Double, ByVal b3 As Double, ByVal b4 As Double,
                                 ByVal c1 As Double, ByVal c2 As Double, ByVal c3 As Double, ByVal c4 As Double,
                                 ByVal d1 As Double, ByVal d2 As Double, ByVal beta As Double, ByVal gamma As Double) As Double()
            ...
            Dim dR_dVr As Double = Pr / Tr + B / Vr ^ 2 + 2 * C / Vr ^ 3 + 5 * D / Vr ^ 6 - dBWRc4_dVr
            ' dR/dPr (explicit) = Vr/Tr
            Dim dVr As Double = -(Vr / Tr) / dR_dVr
            Dim dz As Double = Vr / Tr + Pr / Tr * dVr

            Dim u As Double = gamma * w2
            Dim dEbr_dVr As Double = exg * (beta + u) * (-2.0 * gamma / Vr ^ 3)
            Dim dE As Double = c4 / (2 * Tr ^ 3 * gamma) * dEbr_dVr * dVr
        ...
        ''' <summary>
        ''' Analytical d(ln fugacity coefficient)/dT for the Lee-Kesler-Plocker mixture. Mirrors CalcLnFugCPU
        ''' but differentiates the corresponding-states expression through Tr = T/Tcm (Pr = P/Pcm is constant
        ''' in T). The fugacity part is fully analytical (via BWR_dTr); the enthalpy-departure T-derivative
        ''' uses a cheap central finite difference of H_LK_MIX. Validated against finite differences.
        ''' </summary>
        Function CalcLnFugDT(ByVal TIPO As String, ByVal T As Double, ByVal P As Double, ByVal Vz As Double(), ByVal VKij As Double(,), ByVal VTc As Double(), ByVal VPc As Double(), ByVal Vw As Double(), ByVal VMM As Double(), ByVal VVc As Double()) As Double()

            Dim obj = Me.MixCritProp_LK(Vz, VTc, VPc, Vw, VVc, VKij)
            Dim Tcm As Double = obj(0), Pcm As Double = obj(1), Vcm As Double = obj(2), wm As Double = obj(3)
            Dim Tr As Double = T / Tcm, Pr As Double = P / Pcm
            Dim zcm As Double = Pcm * Vcm / (8.314 * Tcm)

            Dim n As Integer = Vz.Length - 1
            Dim i, j, l As Integer

            Dim MMm As Double = 0.0
            For i = 0 To n : MMm += Vz(i) * VMM(i) : Next

            Dim dHmRTcm As Double = Me.H_LK_MIX(TIPO, T, P, Vz, VKij, VTc, VPc, Vw, VMM, 0, VVc) / (8.314 * Tcm) * MMm
            Dim eH As Double = 0.01
            Dim Hp As Double = Me.H_LK_MIX(TIPO, T + eH, P, Vz, VKij, VTc, VPc, Vw, VMM, 0, VVc)
            Dim Hm As Double = Me.H_LK_MIX(TIPO, T - eH, P, Vz, VKij, VTc, VPc, Vw, VMM, 0, VVc)
            Dim dHmRTcm_dT As Double = (Hp - Hm) / (2 * eH) / (8.314 * Tcm) * MMm

            ' composition-only weights (identical to CalcLnFugCPU)
            Dim vcjk(n, n), tcjk(n, n) As Double
            For i = 0 To n
                For j = 0 To n
                    vcjk(i, j) = 1 / 8 * (VVc(i) ^ (1 / 3) + VVc(j) ^ (1 / 3)) ^ 3 / 1000
            ...
            ' mixture z and its Tr-derivative
            Dim tmp = Me.Z_LK(TIPO, Tr, Pr, wm)
            Dim zm As Double = tmp(0), zs As Double = tmp(1), zh As Double = tmp(2)

            Dim ds = BWR_dTr(Tr, Pr, zs, 0.1181193, 0.265728, 0.15479, 0.030323, 0.0236744, 0.0186984, 0.0, 0.042724, 0.0000155488, 0.0000623689, 0.65392, 0.060167)
            Dim dh = BWR_dTr(Tr, Pr, zh, 0.2026579, 0.331511, 0.027655, 0.203488, 0.0313385, 0.0503618, 0.016901, 0.041577, 0.000048736, 0.00000740336, 1.226, 0.03754)
            Dim wh As Double = 0.3978
            Dim dzm_dTr As Double = ds(0) + wm / wh * (dh(0) - ds(0))
            Dim dlnfugm_dTr As Double = ds(1) + wm / wh * (dh(1) - ds(1))
            Dim ddlnfidwm_dTr As Double = 1 / wh * (dh(1) - ds(1))

            Dim dlnfi(n) As Double
            For i = 0 To n
                dlnfi(i) = dlnfugm_dTr / Tcm +
                           suma(i) * ((1.0 / T ^ 2) * dHmRTcm - (1.0 / T) * dHmRTcm_dT) +
                           (dzm_dTr / Tcm) / Pcm * sumb(i) -
                           (ddlnfidwm_dTr / Tcm) * sumc(i)
            Next
        ...
        ''' <summary>
        ''' The composition-dependent "correction" quantities of the LKP fugacity expression (see
        ''' CalcLnFugCPU): suma(i)=sum_j!=i x_j dTcmdx(i,j), sumb(i)=sum_j!=i x_j dPcmdx(i,j), and the scalar
        ''' dHmRTcm = H_LK_MIX/(R Tcm)*MMm. These are cheap (mixing-rule sums + one enthalpy call, no per
        ''' component BWR solve) so their composition derivatives are taken by finite difference in CalcLnFugDN.
        ''' Returns Object(){ suma(), sumb(), dHmRTcm }.
        ''' </summary>
        Private Function CorrectionTerms(ByVal TIPO As String, ByVal T As Double, ByVal P As Double, ByVal Vz As Double(), ByVal VKij As Double(,), ByVal VTc As Double(), ByVal VPc As Double(), ByVal Vw As Double(), ByVal VMM As Double(), ByVal VVc As Double()) As Object

            Dim n As Integer = Vz.Length - 1
            Dim obj = Me.MixCritProp_LK(Vz, VTc, VPc, Vw, VVc, VKij)
            Dim Tcm As Double = obj(0), Pcm As Double = obj(1), Vcm As Double = obj(2)
            Dim zcm As Double = Pcm * Vcm / (8.314 * Tcm)
            Dim i, j, l As Integer
            ...
            For i = 0 To n
                ...
                Next
            Next
            Dim suma(n), sumb(n) As Double
            For i = 0 To n
                For j = 0 To n
                    If j <> i Then
                        Dim dVcmij As Double = 2 * sum2(i, j)
                        Dim dTcmij As Double = (2 * sum1(i, j) - 0.25 * Vcm ^ (0.25 - 1) * dVcmij * Tcm) / Vcm ^ 0.25
                        Dim dPcmij As Double = Pcm * (-0.085 * (Vw(j) - Vw(i)) / zcm + dTcmij / Tcm - dVcmij / Vcm)
                        suma(i) += Vz(j) * dTcmij
                        sumb(i) += Vz(j) * dPcmij
                    End If
                Next
            ...
            Return New Object() {suma, sumb, dHmRTcm}

        End Function

        ''' <summary>
        ''' Analytical composition (mole-number, total-moles = 1 basis) derivative d(ln phi_i)/dn_m of the
        ''' Lee-Kesler-Plocker mixture. The corresponding-states fugacity/z sensitivities to the mixture
        ''' reduced conditions (through Tcm, Pcm, wm) are analytical (BWR_dTr / BWR_dPr + the first
        ''' composition derivatives of the Plocker mixing rules); the cheap "correction" terms suma/sumb and
        ''' the enthalpy departure dHmRTcm are differentiated by finite difference. Returns an (nc x nc) matrix.
        ''' Validated against finite differences of CalcLnFugCPU.
        ''' </summary>
        Function CalcLnFugDN(ByVal TIPO As String, ByVal T As Double, ByVal P As Double, ByVal Vz As Double(), ByVal VKij As Double(,), ByVal VTc As Double(), ByVal VPc As Double(), ByVal Vw As Double(), ByVal VMM As Double(), ByVal VVc As Double()) As Double(,)
            ...
            Dim tmp = Me.Z_LK(TIPO, Tr, Pr, wm)
            Dim zm As Double = tmp(0), zs As Double = tmp(1), zh As Double = tmp(2)
            Dim res = Me.LnFugM(TIPO, Tr, Pr, wm)
            Dim dlnfidwm As Double = 1 / wh * (res(2) - res(1))

            Dim dsT = BWR_dTr(Tr, Pr, zs, 0.1181193, 0.265728, 0.15479, 0.030323, 0.0236744, 0.0186984, 0.0, 0.042724, 0.0000155488, 0.0000623689, 0.65392, 0.060167)
            Dim dhT = BWR_dTr(Tr, Pr, zh, 0.2026579, 0.331511, 0.027655, 0.203488, 0.0313385, 0.0503618, 0.016901, 0.041577, 0.000048736, 0.00000740336, 1.226, 0.03754)
            Dim dsP = BWR_dPr(Tr, Pr, zs, 0.1181193, 0.265728, 0.15479, 0.030323, 0.0236744, 0.0186984, 0.0, 0.042724, 0.0000155488, 0.0000623689, 0.65392, 0.060167)
            Dim dhP = BWR_dPr(Tr, Pr, zh, 0.2026579, 0.331511, 0.027655, 0.203488, 0.0313385, 0.0503618, 0.016901, 0.041577, 0.000048736, 0.00000740336, 1.226, 0.03754)

            Dim dlnfugm_dTr As Double = dsT(1) + wm / wh * (dhT(1) - dsT(1))
            Dim dlnfugm_dPr As Double = dsP(1) + wm / wh * (dhP(1) - dsP(1))
            Dim dzm_dTr As Double = dsT(0) + wm / wh * (dhT(0) - dsT(0))
            Dim dzm_dPr As Double = dsP(0) + wm / wh * (dhP(0) - dsP(0))
            Dim dzm_dwm As Double = (zh - zs) / wh

            ' cross terms for the first composition derivatives of Vcm and Tcm's numerator S
            Dim vcjk(n, n), tcjk(n, n) As Double
            For i = 0 To n
                For j = 0 To n
                    vcjk(i, j) = 1 / 8 * (VVc(i) ^ (1 / 3) + VVc(j) ^ (1 / 3)) ^ 3 / 1000
            ...
            ' base correction terms and sumc
            Dim baseCT = DirectCast(Me.CorrectionTerms(TIPO, T, P, Vz, VKij, VTc, VPc, Vw, VMM, VVc), Object())
            Dim sumaB = DirectCast(baseCT(0), Double()), sumbB = DirectCast(baseCT(1), Double())
            Dim dHmRTcmB As Double = CDbl(baseCT(2))
            Dim sumcB(n) As Double
            For i = 0 To n
                For j = 0 To n
                    If j <> i Then sumcB(i) += Vz(j) * (Vw(j) - Vw(i))
                Next
            Next

            ' unconstrained d(ln fi)/dx_m
            Dim dxu(n, n) As Double
            Dim dstep As Double = 0.000001
            For m As Integer = 0 To n
                ' first composition derivatives of the mixing rules (unconstrained)
                Dim dVcm As Double = 0.0, dS As Double = 0.0
                For j = 0 To n
                    dVcm += Vz(j) * vcjk(m, j)
                    dS += Vz(j) * (vcjk(m, j) ^ 0.25 * tcjk(m, j))
                Next
                dVcm *= 2 : dS *= 2
                Dim dTcm As Double = -0.25 * Tcm / Vcm * dVcm + Vcm ^ (-0.25) * dS
                Dim dwmx As Double = Vw(m)
                Dim dPcm As Double = 8.314 * ((-0.085 * dwmx) * Tcm / Vcm + (0.2905 - 0.085 * wm) * (dTcm / Vcm - Tcm * dVcm / Vcm ^ 2))
                Dim dTrx As Double = -Tr / Tcm * dTcm
                Dim dPrx As Double = -Pr / Pcm * dPcm

                Dim dlnfugm_dxm As Double = dlnfugm_dTr * dTrx + dlnfugm_dPr * dPrx + dlnfidwm * dwmx
                Dim dzm_dxm As Double = dzm_dTr * dTrx + dzm_dPr * dPrx + dzm_dwm * dwmx
                Dim dlnfugs_dxm As Double = dsT(1) * dTrx + dsP(1) * dPrx
                Dim dlnfugh_dxm As Double = dhT(1) * dTrx + dhP(1) * dPrx
                Dim ddlnfidwm_dxm As Double = 1 / wh * (dlnfugh_dxm - dlnfugs_dxm)

                ' finite-difference the correction terms wrt x_m
                Dim Vzp As Double() = DirectCast(Vz.Clone(), Double())
                Vzp(m) += dstep
                Dim ctP = DirectCast(Me.CorrectionTerms(TIPO, T, P, Vzp, VKij, VTc, VPc, Vw, VMM, VVc), Object())
                Dim sumaP = DirectCast(ctP(0), Double()), sumbP = DirectCast(ctP(1), Double())
                Dim ddHm As Double = (CDbl(ctP(2)) - dHmRTcmB) / dstep

                For i = 0 To n
                    Dim dsuma As Double = (sumaP(i) - sumaB(i)) / dstep
                    Dim dsumb As Double = (sumbP(i) - sumbB(i)) / dstep
                    Dim dsumc As Double = If(m <> i, Vw(m) - Vw(i), 0.0)
                    dxu(i, m) = dlnfugm_dxm -
                                (1.0 / T) * (ddHm * sumaB(i) + dHmRTcmB * dsuma) +
                                (dzm_dxm / Pcm * sumbB(i) - (zm - 1) / Pcm ^ 2 * dPcm * sumbB(i) + (zm - 1) / Pcm * dsumb) -
                                (ddlnfidwm_dxm * sumcB(i) + dlnfidwm * dsumc)
                Next
            Next

            ' project to mole-number (total moles = 1) basis
            Dim deriv(n, n) As Double
            For i = 0 To n
                Dim proj As Double = 0.0
                For m As Integer = 0 To n : proj += Vz(m) * dxu(i, m) : Next
                For m As Integer = 0 To n : deriv(i, m) = dxu(i, m) - proj : Next
            Next
