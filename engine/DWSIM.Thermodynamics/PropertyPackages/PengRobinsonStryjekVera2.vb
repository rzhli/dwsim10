...
Namespace PropertyPackages
    ...
    <System.Serializable()> Public Partial Class PRSV2PropertyPackage
...
#End Region
        ...
        ''' <summary>
        ''' True (mixture) critical points. PRSV2 is a cubic, but its composition-dependent Stryjek-Vera /
        ''' Margules mixing rule rules out the closed-form CubicCP Hessian used by PR/SRK, so the critical
        ''' matrix and cubic form are supplied by PRSV2CP, which differentiates the exact ln-fugacity by
        ''' automatic differentiation (the Margules rule is therefore handled exactly).
        ''' </summary>
        Public Overrides Function DW_CalculateCriticalPoints() As List(Of Double())

            Dim Vz = RET_VMOL(Phase.Mixture)

            ' A component at exactly zero mole fraction makes the critical Hessian singular and the
            ' solver returns a garbage point; a trace amount has no measurable effect on the critical
            ' point, so floor the zero fractions and renormalize.
            ...
            gm.QMatrixAnalytic = Function(T, V, Vzi)
                                     Return Utilities.TCP.PRSV2CP.QMatrix(T, V, Vzi, VTc, VPc, Vw, k1, k2, k3, kij, kij2)
                                 End Function
            gm.CubicFormAnalytic = Function(Dn, T, V, Vzi)
                                       Return Utilities.TCP.PRSV2CP.CubicForm(Dn, T, V, Vzi, VTc, VPc, Vw, k1, k2, k3, kij, kij2)
                                   End Function
            gm.CalcP = Function(T, V, Vzi)
                           Return Utilities.TCP.PRSV2CP.PressureTV(T, V, Vzi, VTc, VPc, Vw, k1, k2, k3, kij, kij2)
                       End Function
...
#Region "   Analytical Derivatives"
        ...
        ' Only the temperature derivative is analytical for PRSV2 so far (the composition-dependent
        ' Stryjek-Vera mixing rule makes d(lnphi)/dn substantially harder - it stays finite-difference).
        Public Overrides ReadOnly Property ImplementsAnalyticalDerivatives As Boolean
            Get
                Return Not AnalyticalDerivativesDisabled
            End Get
        End Property
        ...
        Public Overrides Function DW_CalcdLnFugCoeffdT(ByVal Vx As Double(), ByVal T As Double, ByVal P As Double, st As State) As Double()
            ...
            Return DirectCast(m_pr.CalcLnFugDT(T, P, Vx, RET_VKij, RET_VKij2, RET_KAPPA1, RET_KAPPA2, RET_KAPPA3, RET_VTC, RET_VPC, RET_VW, TIPO), Double())
        End Function
        ...
        Public Overrides Function DW_CalcdKdT(ByVal Vx As Double(), ByVal Vy As Double(), ByVal T As Double, ByVal P As Double, Optional ByVal type As String = "LV") As Double()
            ...
            Dim dLdT = DirectCast(m_pr.CalcLnFugDT(T, P, Vx, RET_VKij, RET_VKij2, RET_KAPPA1, RET_KAPPA2, RET_KAPPA3, RET_VTC, RET_VPC, RET_VW, "L"), Double())
            Dim dVdT = DirectCast(m_pr.CalcLnFugDT(T, P, Vy, RET_VKij, RET_VKij2, RET_KAPPA1, RET_KAPPA2, RET_KAPPA3, RET_VTC, RET_VPC, RET_VW, "V"), Double())
