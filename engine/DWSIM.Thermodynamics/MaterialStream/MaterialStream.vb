...
''' <summary>
''' Contains the material stream class representing process streams that carry multicomponent,
''' multiphase flow data between unit operations in a DWSIM flowsheet.
''' </summary>
Namespace Streams
    ...
    <System.Serializable()> Public Partial Class MaterialStream
        ...
        Private _phaseEnvelopeLookup As PropertyPackages.PhaseEnvelopeLookupTable
        ...
        Public Property PhaseEnvelopeLookup As PropertyPackages.PhaseEnvelopeLookupTable
            ...
            End Get
            Set(value As PropertyPackages.PhaseEnvelopeLookupTable)
                _phaseEnvelopeLookup = value
            End Set
        End Property
        ...
        Public Sub BuildPhaseEnvelopeLookupAsync(Optional mode As PropertyPackages.PhaseEnvelopeLookupMode = PropertyPackages.PhaseEnvelopeLookupMode.WidomOnly)
            ...
            Dim Vz = PropertyPackage.RET_VMOL(PropertyPackages.Phase.Mixture)

            Dim ppClone = DirectCast(Me.PropertyPackage.Clone(), PropertyPackages.PropertyPackage)
            ppClone.CurrentMaterialStream = Me

            _lookupBuildTask = Task.Run(Sub()
                                            Dim newTable As New PropertyPackages.PhaseEnvelopeLookupTable()
                                            newTable.CompositionHash = DirectCast(Vz.Clone(), Double())
                                            newTable.Mode = mode
                                            newTable.IsBuilding = True

                                            Try
                                                Dim opts As New PropertyPackages.PhaseEnvelopeOptions()
                                                opts.WidomLine = True
                                                opts.OperatingPoint = False
                                                opts.QualityLine = False
                                                opts.PhaseIdentificationCurve = False
                                                opts.StabilityCurve = False
                                                If mode = PropertyPackages.PhaseEnvelopeLookupMode.FullEnvelope Then
                                                    opts.SolidLiquidEquilibrium = True
                                                End If

                                                Dim result = ppClone.DW_ReturnPhaseEnvelope(opts, Nothing)
                                                newTable.BuildFromEnvelopeResult(result, ppClone.AUX_TCM(PropertyPackages.Phase.Mixture), ppClone.AUX_PCM(PropertyPackages.Phase.Mixture))
                                            Catch ex As Exception
                                                newTable.IsReady = False
                                                newTable.IsBuilding = False
                                            End Try

                                            SyncLock _lookupLock
                                                _phaseEnvelopeLookup = newTable
                                            End SyncLock
                                        End Sub)
        End Sub
        ...
        Public Sub InvalidatePhaseEnvelopeLookup()
            SyncLock _lookupLock
                If _phaseEnvelopeLookup IsNot Nothing Then
                    _phaseEnvelopeLookup.IsReady = False
                End If
            End SyncLock
        End Sub
        ...
        Public Function EstimateTemperatureFromPS(P As Double, S As Double) As Double
            Dim tbl = _phaseEnvelopeLookup
            If tbl Is Nothing OrElse Not tbl.IsReady Then Return Double.NaN
            Return tbl.EstimateTemperaturePS(P, S)
        End Function
        ...
        ''' <summary>
        ''' Gets or sets the composition basis used in the floating table display.
        ''' </summary>
        ...
        ''' <summary>
        ''' Gets or sets which flow variable (mass, mole or volumetric) is used as the defined specification for this stream.
        ''' </summary>
        Public Property DefinedFlow As FlowSpec = FlowSpec.Mass Implements IMaterialStream.DefinedFlow

        ''' <summary>
        ''' Gets or sets the forced phase setting, which overrides the flash calculation result.
        ''' </summary>
...
#Region "    XML serialization"
        ...
        ''' <summary>
        ''' Restores stream state from a list of XML elements previously produced by <see cref="SaveData"/>.
        ''' </summary>
        ''' <param name="data">XML elements containing serialized stream data.</param>
        ...
        ''' </summary>
        ''' <returns>List of XElement objects representing the serialized stream.</returns>
...
#Region "    DWSIM Specific"
        ...
        ''' </summary>
        ''' <returns>Debug report as a plain-text string.</returns>
        ...
        ''' <summary>
        ''' Validates the stream specification values and throws an <see cref="ArgumentException"/> if any are invalid.
        ''' </summary>
        ...
        ''' <summary>
        ''' Returns the property package instance associated with this stream.
        ''' </summary>
        ''' <returns>The current <see cref="PropertyPackage"/> object.</returns>
        ...
        ''' </summary>
        ''' <returns>A cloned <see cref="PropertyPackage"/> object.</returns>
        ...
        ''' </summary>
        ''' <param name="PP">The property package instance to assign.</param>
        ...
        ''' </summary>
        ''' <returns>True if a property package instance was cleared; False if none was set.</returns>
        ...
        ''' <summary>
        ''' Initializes a new instance of <see cref="MaterialStream"/> with default phase collection and stream conditions.
        ''' </summary>
        Public Sub New()
        ...
        ''' <summary>
        ''' Initializes a new instance of <see cref="MaterialStream"/> attached to a flowsheet and property package.
        ''' </summary>
        ''' <param name="name">Display name of the stream.</param>
        ''' <param name="description">Description of the stream.</param>
        ''' <param name="flowsheet">The parent flowsheet.</param>
        ''' <param name="proppack">The property package to use for calculations.</param>
        ...
        ''' <summary>
        ''' Initializes a new instance of <see cref="MaterialStream"/> with a name and description, without a flowsheet.
        ''' </summary>
        ''' <param name="name">Display name of the stream.</param>
        ''' <param name="description">Description of the stream.</param>
        ...
        ''' <summary>
        ''' Returns the <see cref="BaseClasses.Phase"/> object corresponding to the given phase name.
        ''' </summary>
        ''' <param name="phasename">Phase name (e.g. "Vapor", "Liquid1", "Solid", "Mixture").</param>
        ''' <returns>The matching phase, or the Mixture phase if the name is not recognised.</returns>
        ...
        ''' </summary>
        ''' <param name="phasename">Phase name (e.g. "Vapor", "Liquid1", "Solid").</param>
        ''' <returns>Molar fraction of the requested phase.</returns>
        Public Function GetPhaseMolarFraction(phasename As String) As Double
            Select Case phasename
                Case "Vapor"
                    Return Phases(2).Properties.molarfraction.GetValueOrDefault()
                Case "LiquidMixture", "OverallLiquid", "Líquido (Mistura)"
                    Return Phases(1).Properties.molarfraction.GetValueOrDefault()
                Case "Liquid1", "Líquido 1"
                    Return Phases(3).Properties.molarfraction.GetValueOrDefault()
                Case "Liquid2", "Líquido 2"
                    Return Phases(4).Properties.molarfraction.GetValueOrDefault()
                Case "Liquid3", "Líquido 3"
                    Return Phases(5).Properties.molarfraction.GetValueOrDefault()
                Case "Aqueous", "Phase Aquosa"
                    Return Phases(6).Properties.molarfraction.GetValueOrDefault()
                Case "Solid", "Sólida"
                    Return Phases(7).Properties.molarfraction.GetValueOrDefault()
                Case "Mixture", "Mistura"
                    Return Phases(0).Properties.molarfraction.GetValueOrDefault()
                Case Else
                    Return Phases(0).Properties.molarfraction.GetValueOrDefault()
            End Select
        End Function
        ...
        ''' </summary>
        ''' <param name="phasename">Phase name (e.g. "Vapor", "Liquid1", "Solid").</param>
        ''' <returns>Mass fraction of the requested phase.</returns>
        Public Function GetPhaseMassFraction(phasename As String)
            Select Case phasename
                Case "Vapor"
                    Return Phases(2).Properties.massfraction.GetValueOrDefault()
                Case "LiquidMixture", "OverallLiquid", "Líquido (Mistura)"
                    Return Phases(1).Properties.massfraction.GetValueOrDefault()
                Case "Liquid1", "Líquido 1"
                    Return Phases(3).Properties.massfraction.GetValueOrDefault()
                Case "Liquid2", "Líquido 2"
                    Return Phases(4).Properties.massfraction.GetValueOrDefault()
                Case "Liquid3", "Líquido 3"
                    Return Phases(5).Properties.massfraction.GetValueOrDefault()
                Case "Aqueous", "Phase Aquosa"
                    Return Phases(6).Properties.massfraction.GetValueOrDefault()
                Case "Solid", "Sólida"
                    Return Phases(7).Properties.massfraction.GetValueOrDefault()
                Case "Mixture", "Mistura"
                    Return Phases(0).Properties.massfraction.GetValueOrDefault()
                Case Else
                    Return Phases(0).Properties.massfraction.GetValueOrDefault()
            End Select
        End Function
        ...
        ''' </summary>
        ''' <param name="phasename">Phase name (e.g. "Vapor", "Liquid1", "Solid").</param>
        ''' <returns>Molar flow of the requested phase in mol/s.</returns>
        Public Function GetPhaseMolarFlow(phasename As String)
            Select Case phasename
                Case "Vapor"
                    Return Phases(2).Properties.molarflow.GetValueOrDefault()
                Case "LiquidMixture", "OverallLiquid", "Líquido (Mistura)"
                    Return Phases(1).Properties.molarflow.GetValueOrDefault()
                Case "Liquid1", "Líquido 1"
                    Return Phases(3).Properties.molarflow.GetValueOrDefault()
                Case "Liquid2", "Líquido 2"
                    Return Phases(4).Properties.molarflow.GetValueOrDefault()
                Case "Liquid3", "Líquido 3"
                    Return Phases(5).Properties.molarflow.GetValueOrDefault()
                Case "Aqueous", "Phase Aquosa"
                    Return Phases(6).Properties.molarflow.GetValueOrDefault()
                Case "Solid", "Sólida"
                    Return Phases(7).Properties.molarflow.GetValueOrDefault()
                Case "Mixture", "Mistura"
        ...
        ''' </summary>
        ''' <param name="phasename">Phase name (e.g. "Vapor", "Liquid1", "Solid").</param>
        ''' <returns>Mass flow of the requested phase in kg/s.</returns>
        Public Function GetPhaseMassFlow(phasename As String)
            Select Case phasename
                Case "Vapor"
                    Return Phases(2).Properties.massflow.GetValueOrDefault()
                Case "LiquidMixture", "OverallLiquid", "Líquido (Mistura)"
                    Return Phases(1).Properties.massflow.GetValueOrDefault()
                Case "Liquid1", "Líquido 1"
                    Return Phases(3).Properties.massflow.GetValueOrDefault()
                Case "Liquid2", "Líquido 2"
                    Return Phases(4).Properties.massflow.GetValueOrDefault()
                Case "Liquid3", "Líquido 3"
                    Return Phases(5).Properties.massflow.GetValueOrDefault()
                Case "Aqueous", "Phase Aquosa"
                    Return Phases(6).Properties.massflow.GetValueOrDefault()
                Case "Solid", "Sólida"
                    Return Phases(7).Properties.massflow.GetValueOrDefault()
                Case "Mixture", "Mistura"
    ...
    <Xml.Serialization.XmlIgnore> Public Property EquilibriumCalculatedExternally As Boolean = False
        ...
        Public Overloads Sub Calculate(equilibrium As Boolean, properties As Boolean)
            ...
            With Me.PropertyPackage
                ...
                ' Build phase envelope lookup table synchronously before equilibrium calculation
                If GeneratePhaseEnvelopeLookup AndAlso comp > 0.0# Then
                    Try
                        Dim Vz = .RET_VMOL(PropertyPackages.Phase.Mixture)
                        If _phaseEnvelopeLookup Is Nothing OrElse Not _phaseEnvelopeLookup.IsReady OrElse Not _phaseEnvelopeLookup.CompositionMatches(Vz) Then
                            Dim ppClone = DirectCast(Me.PropertyPackage.Clone(), PropertyPackages.PropertyPackage)
                            ppClone.CurrentMaterialStream = Me

                            Dim newTable As New PropertyPackages.PhaseEnvelopeLookupTable()
                            newTable.CompositionHash = DirectCast(Vz.Clone(), Double())
                            newTable.Mode = PhaseEnvelopeLookupMode
                            newTable.IsBuilding = True
                            ...
                            opts.PhaseIdentificationCurve = False
                            opts.StabilityCurve = False
                            If PhaseEnvelopeLookupMode = PropertyPackages.PhaseEnvelopeLookupMode.FullEnvelope Then
                                opts.SolidLiquidEquilibrium = True
                            End If
                        ...
                        End If
                    Catch ex As Exception
                        If _phaseEnvelopeLookup IsNot Nothing Then _phaseEnvelopeLookup.IsReady = False
                    End Try
                End If
                ...
                If properties And comp > 0.0 Then
        ...
        ''' </summary>
        ''' <param name="Vx">Array of mole fractions (one per compound, in compound order).</param>
        ...
        ''' </summary>
        ''' <param name="Vx">Array of mass fractions (one per compound, in compound order).</param>
        ...
        ''' <summary>
        ''' Sets the molar composition of a phase by index. Delegates to <see cref="SetPhaseComposition(Double(), PropertyPackages.Phase)"/>.
        ''' </summary>
        ...
        ''' <summary>
        ''' Returns an array with the overall mass composition.
        ''' </summary>
        ''' <returns>Array of mass fractions.</returns>
        ...
        ''' </summary>
        ''' <returns>Array of compound name strings.</returns>
        ...
        ''' <param name="su">Unit system to convert the result to; defaults to SI if Nothing.</param>
        ...
        ''' </summary>
        ''' <param name="proptype">The category of properties to enumerate.</param>
        ''' <returns>Array of property identifier strings.</returns>
        Public Overloads Overrides Function GetProperties(ByVal proptype As Interfaces.Enums.PropertyType) As String()
            ...
            Dim complist = Phases(0).Compounds.Values.ToList()

            If FlowSheet IsNot Nothing Then
                Select Case FlowSheet.Options.CompoundOrderingMode
            ...
            Dim comps = complist.Select(Function(c) c.Name).ToArray()
        ...
        ''' </summary>
        ''' <param name="prop">Property identifier string.</param>
        ''' <param name="propval">New property value.</param>
        ''' <param name="su">Unit system for the supplied value; defaults to SI if Nothing.</param>
        ...
        ''' </summary>
        ''' <param name="prop">Property identifier string.</param>
        ''' <param name="su">Unit system; defaults to SI if Nothing.</param>
        ''' <returns>Unit string for the property.</returns>
...
#Region "    CAPE-OPEN 1.0 Methods and Properties"
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0: Returns the list of properties that can be calculated. Delegates to <see cref="GetPropList"/>.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0: Checks whether a list of properties can be calculated. Delegates to <see cref="PropCheck"/>.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0: Checks the validity of a calculation. Delegates to <see cref="ValidityCheck"/>.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoEquilibriumServer: Calculates equilibrium. Delegates to the associated property package.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoEquilibriumServer: Validity check. Not implemented in DWSIM.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoPropertyPackage: Calculates properties. Delegates to the associated property package.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoPropertyPackage: Returns compound constant properties. Delegates to the associated property package.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoPropertyPackage: Returns the list of phase labels. Delegates to the associated property package.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoPropertyPackage: Returns the list of supported properties. Delegates to the associated property package.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoPropertyPackage: Checks calculation validity. Delegates to the associated property package.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoMaterialTemplate: Creates a new material object from this template. Delegates to <see cref="CreateMaterial"/>.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN 1.0 ICapeThermoMaterialTemplate: Sets a property on the template. Not implemented in DWSIM.
        ''' </summary>
...
#Region "    CAPE-OPEN 1.1 Thermo & Properties"
        ...
        ''' </summary>
        ''' <param name="property">CAPE-OPEN property identifier.</param>
        ''' <param name="basis">Basis (Mole or Mass).</param>
        ''' <returns>Unit string (e.g. "J/mol", "kg/m3").</returns>
        ...
        Public Sub GetTwoPhaseProp(ByVal [property] As String, ByVal phaseLabels As Object, ByVal basis As String, ByRef results As Object) Implements ICapeThermoMaterial.GetTwoPhaseProp
            ...
            Select Case [property].ToLower
                ...
                Case "logkvalue"
                    ...
                    Next
                Case "surfacetension"
                    Dim st = Phases(0).Properties.surfaceTension.GetValueOrDefault()
                    If Double.IsNaN(st) Then st = 0.0
                    res.Add(st)
                Case Else
...
#Region "    CAPE-OPEN Error Interfaces"
        ...
        ''' <summary>
        ''' CAPE-OPEN ECapeRoot: Returns the name of this component.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN ECapeUser: Returns the error code.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN ECapeUser: Returns the error description.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN ECapeUser: Returns the interface name where the error occurred.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN ECapeUser: Returns additional error information.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN ECapeUser: Returns the operation that caused the error.
        ''' </summary>
        ...
        ''' <summary>
        ''' CAPE-OPEN ECapeUser: Returns the scope (call stack context) of the error.
        ''' </summary>
...
#End Region
        ...
        ''' <summary>
        ''' Gets or sets whether this stream is an electrolyte stream.
        ''' </summary>
        Public Property IsElectrolyteStream As Boolean = False Implements IMaterialStream.IsElectrolyteStream

        ''' <summary>
        ''' Gets or sets the name of the reference solvent compound for electrolyte calculations.
        ''' </summary>
        Public Property ReferenceSolvent As String = "" Implements IMaterialStream.ReferenceSolvent

        ''' <summary>
        ''' Gets or sets the flash specification type (e.g. T+P, P+H, P+S, etc.) for this stream.
        ''' </summary>
        ...
        ''' <summary>
        ''' Creates and returns a shallow clone of this stream as an <see cref="IMaterialStream"/>.
        ''' </summary>
        ...
        ''' <summary>
        ''' Creates a new <see cref="MaterialStream"/> with the same compounds, properties and settings as this one.
        ''' </summary>
        ''' <returns>A shallow copy of this stream.</returns>
        ...
        ''' <summary>
        ''' Gets the parent flowsheet that this stream belongs to.
        ''' </summary>
        ...
        ''' <summary>
        ''' Validates the stream specification. Delegates to <see cref="Validate"/>.
        ''' </summary>
        ...
        ''' </summary>
        ''' <returns>Array of property identifier strings for the process flowsheet display.</returns>
        ...
        ''' </summary>
        ''' <param name="dwp">The phase for which mass fractions are calculated.</param>
        ...
        ''' </summary>
        ''' <returns>A new <see cref="MaterialStream"/> with all properties copied.</returns>
        ...
        ''' <summary>
        ''' Returns all phases as an array.
        ''' </summary>
        ...
        ''' <summary>
        ''' Gets or sets additional solid phase properties (e.g. bulk density, angle of repose) for this stream.
        ''' </summary>
        ...
        ''' <summary>
        ''' Gets or sets the stream topology type (Feed, Product, Inner, Recycle, etc.).
        ''' </summary>
        Public Property StreamType As StreamType = StreamType.Undefined Implements IMaterialStream.StreamType

        ''' <summary>
        ''' Inspects the graphic object connector state and updates <see cref="StreamType"/> accordingly.
        ''' </summary>
        ...
        ''' </summary>
        ''' <param name="su">Unit system to use for property values.</param>
        ''' <param name="ci">Culture info for number formatting.</param>
        ''' <param name="numberformat">Number format string (e.g. "0.####").</param>
        ...
        ''' </summary>
        ''' <returns>List of report tuples containing item type and string array.</returns>
        ...
        ''' <summary>
        ''' Helper that calls <see cref="GetSinglePhaseProp"/> and returns the result as a typed list.
        ''' </summary>
        ''' <param name="prop">Property identifier.</param>
        ''' <param name="basis">Basis (Mole or Mass).</param>
        ''' <param name="phaseLabel">Phase label.</param>
        ''' <returns>List of Double values for the requested property.</returns>
        ...
        ''' <param name="prop">Property identifier.</param>
        ''' <param name="basis">Basis (Mole or Mass).</param>
        ''' <param name="su">Unit system.</param>
        ''' <returns>Unit string for the property.</returns>
        ...
        ''' </summary>
        ''' <param name="s">CAPE-OPEN property identifier.</param>
        ''' <returns>Human-readable display name for the property.</returns>
        ...
        ''' </summary>
        ''' <param name="p">Property identifier string.</param>
        ''' <returns>Localised description text.</returns>
        ...
        Public Function GetMassEnthalpyAt(T As Double, Optional calc_eq As Boolean = True) As Double
            ...
            cl.PropertyPackage = PropertyPackage
            cl.AssignSelfToPP()

            cl.SetTemperature(T)
            cl.Calculate(calc_eq, True)

            Dim Hnew = cl.GetMassEnthalpy()
        ...
        ''' </summary>
        ''' <returns>Flash specification code string.</returns>
        ...
        ''' </summary>
        ''' <param name="massflow">Mass flow to subtract, in kg/s.</param>
        ''' <param name="p">Phase from which to subtract flow.</param>
        ''' <param name="calceq">If True, recalculates equilibrium after the subtraction.</param>
        Public Sub SubtractFromPhase(massflow As Double, p As PropertyPackages.Phase, calceq As Boolean)
            ...
            Select Case p
                Case PropertyPackages.Phase.Mixture
                    pi = 0
                    ratio = (Mixture.Properties.massflow.GetValueOrDefault() - massflow) / Mixture.Properties.massflow.GetValueOrDefault()
                Case PropertyPackages.Phase.Vapor
                    pi = 2
                    ratio = (Vapor.Properties.massflow.GetValueOrDefault() - massflow) / Vapor.Properties.massflow.GetValueOrDefault()
                Case PropertyPackages.Phase.Liquid1
                    pi = 3
                    ratio = (Liquid1.Properties.massflow.GetValueOrDefault() - massflow) / Liquid1.Properties.massflow.GetValueOrDefault()
                Case PropertyPackages.Phase.Liquid2
                    pi = 4
                    ratio = (Liquid2.Properties.massflow.GetValueOrDefault() - massflow) / Liquid2.Properties.massflow.GetValueOrDefault()
                Case PropertyPackages.Phase.Solid
                    pi = 7
                    ratio = (Solid.Properties.massflow.GetValueOrDefault() - massflow) / Solid.Properties.massflow.GetValueOrDefault()
            End Select

            For Each comp In Phases(pi).Compounds.Values
                Phases(0).Compounds(comp.Name).MassFlow -= (1 - ratio) * comp.MassFlow
                Phases(0).Compounds(comp.Name).MolarFlow -= (1 - ratio) * comp.MolarFlow
                comp.MassFlow *= ratio
                comp.MolarFlow *= ratio
            Next

            Dim W, M As Double

            For Each comp In Mixture.Compounds.Values
                comp.MassFlow = Vapor.Compounds(comp.Name).MassFlow + Liquid1.Compounds(comp.Name).MassFlow + Liquid2.Compounds(comp.Name).MassFlow + Solid.Compounds(comp.Name).MassFlow
                comp.MolarFlow = Vapor.Compounds(comp.Name).MolarFlow + Liquid1.Compounds(comp.Name).MolarFlow + Liquid2.Compounds(comp.Name).MolarFlow + Solid.Compounds(comp.Name).MolarFlow
                W += comp.MassFlow
                M += comp.MolarFlow
            Next

            Mixture.Properties.massflow = W
            Mixture.Properties.molarflow = M
        ...
        ''' </summary>
        ''' <param name="source">Source stream to copy solid data from.</param>
        ...
        ''' <returns>Property value as Double.</returns>
        ...
        ''' <summary>
        ''' Returns the total energy flow associated with this stream, in kW.
        ''' </summary>
    ...
    Public Class CalculationResults
        ...
        ''' <summary>Gets or sets the map of property names to lists of numerical values.</summary>
        Public Property Data As Dictionary(Of String, List(Of Double))
        ''' <summary>Gets or sets the map of property names to their unit strings.</summary>
        Public Property DataUnits As Dictionary(Of String, String)
        ''' <summary>Gets or sets additional per-compound data.</summary>
        Public Property CompoundData As Dictionary(Of String, Object)
        ''' <summary>Gets or sets the plain-text output generated for this result.</summary>
        Public Property TextOutput As String = ""
        ''' <summary>Gets or sets the unit system used for this result.</summary>
        Public Property Units As SystemsOfUnits.Units
        ''' <summary>Gets or sets any exception that occurred during the calculation.</summary>
        Public Property ExceptionResult As Exception
        ''' <summary>Gets or sets the number format string (default "0.####").</summary>
        Public Property NumberFormat As String = "0.####"
        ''' <summary>Gets or sets the language/locale code used for text output.</summary>
    ...
    Public Class MaterialStreamInputData
        ...
        ''' <summary>Temperature (K), Pressure (Pa), mass/molar/volumetric flows (SI), enthalpy (kJ/kg), entropy (kJ/[kg.K]) and vapor fraction at the time of the last solution.</summary>
        Public Temperature, Pressure, MassFlow, MolarFlow, VolumetricFlow, Enthalpy, Entropy, VaporFraction As Double

        ''' <summary>Overall molar composition at the time of the last solution.</summary>
...
''' <summary>
''' Provides editor support classes for material stream property inspection and UI state persistence.
''' </summary>
Namespace Streams.Editors
    ...
    Public Class MaterialStreamEditorState
        ...
        ''' <summary>Index of the currently selected main tab.</summary>
        Public Property MainSelectedTab As Integer = 0
        ''' <summary>Index of the composition basis selection for stream input.</summary>
        Public Property InputCompositionBasis As Integer = 0
        ''' <summary>Index of the currently selected compounds tab.</summary>
        Public Property CompoundsSelectedTab As Integer = 0
        ''' <summary>Index of the compound amount basis selection.</summary>
        Public Property CompoundsAmountBasis As Integer = 0
        ''' <summary>Index of the selected compound property to display.</summary>
        Public Property CompoundsProperty As Integer = 0
        ''' <summary>Index of the selected amount sub-tab in the compounds section.</summary>
        Public Property CompoundsAmountSelectedTab As Integer = 0
        ''' <summary>Index of the selected property sub-tab in the compounds section.</summary>
        Public Property CompoundsPropertySelectedTab As Integer = 0
        ''' <summary>Index of the selected phase properties tab.</summary>
        Public Property PhasePropsSelectedTab As Integer = 0
        ''' <summary>Index of the secondary main tab (alternate view).</summary>
        Public Property MainSelectedTab0 As Integer = 0
        ''' <summary>0 = show as fractions, non-zero = show as percentages.</summary>
