...
''' <summary>
''' Contains the core thermodynamic data classes used throughout DWSIM, including compound
''' properties, phases, reactions, and petroleum fractions.
''' </summary>
Namespace BaseClasses
    ...
    <System.Serializable()> Public Class Compound
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="Compound"/> class with the specified name and description.
        ''' </summary>
        ''' <param name="name">The compound name.</param>
        ''' <param name="description">A description of the compound.</param>
        ...
        ''' </summary>
        ''' <returns>A formatted string describing the compound's key flow properties.</returns>
        ...
        ''' <summary>
        ''' Loads compound data from a list of XML elements.
        ''' </summary>
        ''' <param name="data">A list of XML elements containing the serialized compound data.</param>
        ...
        ''' </summary>
        ''' <returns>A list of XML elements representing the serialized compound data.</returns>
        ...
        ''' <summary>Gets or sets the diffusion coefficient of this compound in the mixture (m2/s).</summary>
        Public Property DiffusionCoefficient As Double? Implements Interfaces.ICompound.DiffusionCoefficient

        ''' <summary>Gets or sets extra dynamic properties for this compound.</summary>
        Public Property ExtraProperties As New ExpandoObject Implements ICompound.ExtraProperties

        ''' <summary>Gets or sets the molar departure enthalpy of formation at 25 C relative to ideal gas (kJ/mol).</summary>
        Public Property EnthalpyF_Dmol As Double? = 0.0 Implements Interfaces.ICompound.EnthalpyF_Dmol

        ''' <summary>Gets or sets the molar departure entropy of formation at 25 C relative to ideal gas (kJ/[mol.K]).</summary>
    ...
    <System.Serializable()> Public Class Phase
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="Phase"/> class with the specified name and description.
        ''' </summary>
        ''' <param name="name">The phase name.</param>
        ''' <param name="description">A description of the phase.</param>
        Public Sub New(ByVal name As String, ByVal description As String)
        ...
        ''' </summary>
        ''' <returns>The phase name or the base ToString representation.</returns>
        Public Overrides Function ToString() As String
            If Name <> "" Then
                Return Name
            Else
        ...
        ''' <summary>
        ''' Loads phase data, including compound compositions and phase properties, from a list of XML elements.
        ''' </summary>
        ''' <param name="data">A list of XML elements containing the serialized phase data.</param>
        ''' <returns>True if the data was loaded successfully.</returns>
        Public Function LoadData(data As System.Collections.Generic.List(Of System.Xml.Linq.XElement)) As Boolean Implements Interfaces.ICustomXMLSerialization.LoadData
        ...
        ''' <summary>
        ''' Saves phase data, including compound compositions and phase properties, to a list of XML elements.
        ''' </summary>
        ''' <returns>A list of XML elements representing the serialized phase data.</returns>
        Public Function SaveData() As System.Collections.Generic.List(Of System.Xml.Linq.XElement) Implements Interfaces.ICustomXMLSerialization.SaveData
        ...
        ''' <summary>Gets or sets the CAPE-OPEN component description for this phase.</summary>
        Public Property ComponentDescription As String = "" Implements Interfaces.IPhase.ComponentDescription

        ''' <summary>Gets or sets the CAPE-OPEN component name for this phase.</summary>
        Public Property ComponentName As String = "" Implements Interfaces.IPhase.ComponentName

        ''' <summary>Gets or sets the dictionary of compounds present in this phase, keyed by compound name.</summary>
        ...
        ''' <summary>Gets or sets the name of this phase.</summary>
        Public Property Name As String = "" Implements Interfaces.IPhase.Name

        ''' <summary>Gets the thermodynamic properties of this phase.</summary>
        ''' <returns>The phase properties interface implementation.</returns>
...
#Region "    DWSIM Specific"
        ...
        ''' <param name="pp">The property package used for thermodynamic calculations (e.g., Gibbs-based K).</param>
        ''' <returns>The equilibrium constant K at temperature T.</returns>
            ...
            Select Case KExprType
                ...
                Case KOpt.Expression
                    ...
                    If _ExpressionChanged Then
                        KFunc = CompileKExpression(MEngine)
                        _ExpressionChanged = False
                    End If

                    Dim raw As Object = KFunc.Invoke(New Object() {T})

                    If raw Is Nothing OrElse Not TypeOf raw Is Double Then
                        Throw New ArgumentException(String.Format(
                            "Reaction '{0}': the equilibrium constant expression '{1}' did not evaluate to a number at T = {2:F2} K.",
                            Name, Expression, T))
                    End If

                    Dim lnK As Double = DirectCast(raw, Double)

                    If Double.IsNaN(lnK) OrElse Double.IsInfinity(lnK) Then
                        Throw New ArgumentException(String.Format(
                            "Reaction '{0}': the equilibrium constant expression '{1}' evaluated to {2} at T = {3:F2} K.",
                            Name, Expression, lnK, T))
                    End If
        ...
        ''' <summary>
        ''' Matches a bare identifier. The lookbehind keeps the exponent letter of a
        ''' numeric literal (1E-5, 2.5e-3) from being read as a name of its own.
        ''' </summary>
        ...
        ''' </summary>
        Private Shared ReadOnly LegacyMathNames As New Dictionary(Of String, String)(StringComparer.Ordinal) From {
            {"Abs", "abs"}, {"Acos", "arccos"}, {"Asin", "arcsin"}, {"Atan", "arctan"},
            {"Ceiling", "ceil"}, {"Cos", "cos"}, {"Cosh", "cosh"}, {"E", "e"}, {"Exp", "exp"},
            {"Floor", "floor"}, {"Log", "log"}, {"Log10", "log10"}, {"Max", "max"}, {"Min", "min"},
            {"PI", "pi"}, {"Pow", "pow"}, {"Round", "round"}, {"Sign", "sign"}, {"Sin", "sin"},
            {"Sinh", "sinh"}, {"Sqrt", "sqrt"}, {"Tan", "tan"}, {"Tanh", "tanh"}}

        ''' <summary>
        ''' Names the lambda parameter plus the ones MAGES resolves in the parser
        ''' rather than through Globals, so they must be allowed explicitly.
        ''' </summary>
        Private Shared ReadOnly ExtraKnownNames As New HashSet(Of String)(StringComparer.Ordinal) From {"T", "pi"}

        ''' <summary>
        ''' Compiles <see cref="Expression"/> into a MAGES function of T.
        ''' </summary>
        ''' <remarks>
        ''' An unknown identifier does not make MAGES fail: it collapses the whole
        ''' expression to null, which reads back as 0 and turns into K = exp(0) = 1.
        ''' Every name is therefore resolved against the engine before the expression
        ''' is accepted, so a name it cannot evaluate is reported instead of silently
        ''' yielding an equilibrium constant of one.
        ''' </remarks>
        Private Function CompileKExpression(engine As Mages.Core.Engine) As Mages.Core.Function

            If String.IsNullOrWhiteSpace(Expression) Then
                Throw New ArgumentException(String.Format(
                    "Reaction '{0}': the equilibrium constant expression is empty.", Name))
            End If

            Dim translated As String = IdentifierRegex.Replace(Expression,
                Function(m)
                    Dim mapped As String = Nothing
                    If LegacyMathNames.TryGetValue(m.Value, mapped) Then Return mapped
                    Return m.Value
                End Function)

            Dim unknown = IdentifierRegex.Matches(translated).Cast(Of Match)().
                Select(Function(m) m.Value).
                Where(Function(n) Not ExtraKnownNames.Contains(n) AndAlso Not engine.Globals.ContainsKey(n)).
                Distinct().ToList()

            If unknown.Count > 0 Then
                Throw New ArgumentException(String.Format(
                    "Reaction '{0}': the equilibrium constant expression '{1}' uses unknown name(s): {2}. " &
                    "Use the temperature 'T' (K) and the lowercase built-ins (log, log10, log2, exp, sqrt, abs, pow, min, max, sin, cos, tan, pi, e).",
                    Name, Expression, String.Join(", ", unknown)))
            End If

            Return engine.Interpret("(T) => " + translated)
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="Reaction"/> class with default values.
        ''' </summary>
        Public Sub New()
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="Reaction"/> class with a name and identifier.
        ''' </summary>
        ''' <param name="Name">The reaction name.</param>
        ''' <param name="Id">The unique reaction identifier (GUID string).</param>
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="Reaction"/> class with a name, identifier, and description.
        ''' </summary>
        ''' <param name="Name">The reaction name.</param>
        ''' <param name="Id">The unique reaction identifier (GUID string).</param>
        ''' <param name="Description">A text description of the reaction.</param>
        ...
        ''' <summary>
        ''' Creates a deep copy of this reaction with a new unique ID.
        ''' </summary>
        ''' <returns>A cloned <see cref="Reaction"/> object with a new GUID.</returns>
        Public Function Clone() As Object Implements System.ICloneable.Clone

            Dim rxn As New Reaction()
            rxn.LoadData(SaveData())
...
#End Region
        ...
        ''' </summary>
        ''' <returns>The reaction name or the base ToString representation.</returns>
        Public Overrides Function ToString() As String
            If Name <> "" Then
                Return Name
            Else
        ...
        ''' <summary>
        ''' Loads reaction data from a list of XML elements, including stoichiometric component data.
        ''' </summary>
        ''' <param name="data">A list of XML elements containing the serialized reaction data.</param>
        ''' <returns>True if the data was loaded successfully.</returns>
        Public Function LoadData(data As System.Collections.Generic.List(Of System.Xml.Linq.XElement)) As Boolean Implements Interfaces.ICustomXMLSerialization.LoadData
        ...
        ''' <summary>
        ''' Saves reaction data, including stoichiometric component data, to a list of XML elements.
        ''' </summary>
        ''' <returns>A list of XML elements representing the serialized reaction data.</returns>
        Public Function SaveData() As System.Collections.Generic.List(Of System.Xml.Linq.XElement) Implements Interfaces.ICustomXMLSerialization.SaveData
        ...
        ''' <summary>Gets or sets the name of the base reactant compound for this reaction.</summary>
        Public Property BaseReactant As String = "" Implements Interfaces.IReaction.BaseReactant

        ''' <summary>Gets the dictionary of stoichiometric components participating in this reaction, keyed by compound name.</summary>
        ''' <returns>A dictionary of reaction stoichiometry bases.</returns>
        ...
        ''' <summary>Gets or sets the text description of this reaction.</summary>
        Public Property Description As String = "" Implements Interfaces.IReaction.Description

        ''' <summary>Gets or sets the stoichiometric equation string for this reaction.</summary>
        Public Property Equation As String = "" Implements Interfaces.IReaction.Equation

        ''' <summary>Gets or sets the unique identifier (GUID string) for this reaction.</summary>
        Public Property ID As String = "" Implements Interfaces.IReaction.ID

        ''' <summary>Gets or sets the name of this reaction.</summary>
        Public Property Name As String = "" Implements Interfaces.IReaction.Name

        ''' <summary>Gets or sets the concentration basis used for the reaction rate expression.</summary>
        ...
        ''' <summary>Gets or sets the heat of reaction (kJ/mol) at standard conditions.</summary>
        Public Property ReactionHeat As Double Implements Interfaces.IReaction.ReactionHeat

        ''' <summary>Gets or sets the heat of reaction (kJ/mol) as used by CAPE-OPEN interfaces.</summary>
        Public Property ReactionHeatCO As Double Implements Interfaces.IReaction.ReactionHeatCO

        ''' <summary>Gets or sets the phase in which this reaction takes place (Vapor, Liquid, or Overall).</summary>
        Public Property ReactionPhase As Interfaces.Enums.ReactionPhase Implements Interfaces.IReaction.ReactionPhase

        ''' <summary>Gets or sets the type of this reaction (Kinetic, Equilibrium, Conversion, or Heterogeneous Catalytic).</summary>
        Public Property ReactionType As Interfaces.Enums.ReactionType Implements Interfaces.IReaction.ReactionType

        ''' <summary>Gets or sets the stoichiometric balance check value for this reaction.</summary>
        Public Property StoichBalance As Double Implements Interfaces.IReaction.StoichBalance

        ''' <summary>Gets or sets the pre-exponential factor for the forward Arrhenius rate constant.</summary>
        Public Property A_Forward As Double Implements Interfaces.IReaction.A_Forward

        ''' <summary>Gets or sets the pre-exponential factor for the reverse Arrhenius rate constant.</summary>
        Public Property A_Reverse As Double Implements Interfaces.IReaction.A_Reverse

        ''' <summary>Gets or sets the reaction approach-to-equilibrium fraction (for conversion reactions).</summary>
        Public Property Approach As Double Implements Interfaces.IReaction.Approach

        ''' <summary>Gets or sets the concentration unit string for the rate expression.</summary>
        Public Property ConcUnit As String = "" Implements Interfaces.IReaction.ConcUnit

        ''' <summary>Gets or sets the constant equilibrium K value used when the K expression type is set to Constant.</summary>
        Public Property ConstantKeqValue As Double Implements Interfaces.IReaction.ConstantKeqValue

        ''' <summary>Gets or sets the activation energy for the forward reaction (J/mol).</summary>
        Public Property E_Forward As Double Implements Interfaces.IReaction.E_Forward

        ''' <summary>Gets or sets the activation energy for the reverse reaction (J/mol).</summary>
        ...
        Public Property Expression As String Implements Interfaces.IReaction.Expression
            ...
            End Set
        End Property
        ''' <summary>Gets or sets the equilibrium constant expression type (Constant, Expression, or Gibbs).</summary>
        Public Property KExprType As Interfaces.Enums.KOpt Implements Interfaces.IReaction.KExprType

        ''' <summary>Gets or sets the current calculated equilibrium constant value for this reaction.</summary>
        Public Property Kvalue As Double Implements Interfaces.IReaction.Kvalue

        ''' <summary>Gets or sets the current calculated reaction rate.</summary>
        Public Property Rate As Double Implements Interfaces.IReaction.Rate

        ''' <summary>Gets or sets the denominator expression string for LHHW-type rate equations.</summary>
        Public Property RateEquationDenominator As String = "" Implements Interfaces.IReaction.RateEquationDenominator

        ''' <summary>Gets or sets the numerator expression string for LHHW-type rate equations.</summary>
        Public Property RateEquationNumerator As String = "" Implements Interfaces.IReaction.RateEquationNumerator

        ''' <summary>Gets or sets the Gibbs energy change of reaction (kJ/mol).</summary>
        Public Property ReactionGibbsEnergy As Double Implements Interfaces.IReaction.ReactionGibbsEnergy

        ''' <summary>Gets or sets the maximum temperature for this reaction (K).</summary>
        Public Property Tmax As Double = 2000.0 Implements Interfaces.IReaction.Tmax

        ''' <summary>Gets or sets the minimum temperature for this reaction (K).</summary>
        Public Property Tmin As Double = 0.0 Implements Interfaces.IReaction.Tmin

        ''' <summary>Gets or sets the velocity (rate) unit string for this reaction.</summary>
        Public Property VelUnit As String = "" Implements Interfaces.IReaction.VelUnit

        ''' <summary>Gets or sets the forward kinetic rate type (Arrhenius or expression-based).</summary>
        ...
        ''' <summary>Gets or sets the reverse kinetic rate type (Arrhenius or expression-based).</summary>
        ...
        ''' <summary>Gets or sets the user-defined expression string for the forward kinetic rate.</summary>
        Public Property ReactionKinFwdExpression As String = "" Implements IReaction.ReactionKinFwdExpression

        ''' <summary>Gets or sets the user-defined expression string for the reverse kinetic rate.</summary>
        Public Property ReactionKinRevExpression As String = "" Implements IReaction.ReactionKinRevExpression

        ''' <summary>Gets or sets the unit of the forward activation energy (e.g., "J/mol").</summary>
        Public Property E_Forward_Unit As String = "J/mol" Implements IReaction.E_Forward_Unit

        ''' <summary>Gets or sets the unit of the reverse activation energy (e.g., "J/mol").</summary>
        Public Property E_Reverse_Unit As String = "J/mol" Implements IReaction.E_Reverse_Unit

        ''' <summary>Gets or sets the kinetics formulation type for this reaction (Expression or Script).</summary>
        Public Property ReactionKinetics As ReactionKinetics = ReactionKinetics.Expression Implements IReaction.ReactionKinetics

        ''' <summary>Gets or sets the title of the script used for kinetic evaluation when using Script kinetics.</summary>
        Public Property ScriptTitle As String = "" Implements IReaction.ScriptTitle

        ''' <summary>Gets or sets the units string for the equilibrium constant basis (e.g., "Pa").</summary>
        ...
        ''' </summary>
        ''' <param name="T">Temperature in Kelvin.</param>
        ''' <param name="PP">The property package interface used for thermodynamic calculations.</param>
        ''' <returns>The equilibrium constant K at temperature T.</returns>
        ...
        ''' </summary>
        ''' <param name="prop">The property identifier string (e.g., "Kinetic_A_Forward").</param>
        ''' <returns>The numeric value of the requested property.</returns>
...
#Region "    DWSIM Specific"
        ...
        ''' </summary>
        ''' <param name="reactname">The name of the reaction to look up.</param>
        ''' <returns>The unique ID string of the matching reaction, or an empty string if not found.</returns>
        ...
        ''' <summary>Gets the dictionary of reaction set entries, keyed by reaction ID.</summary>
        ''' <returns>A dictionary mapping reaction IDs to their reaction set base objects.</returns>
        ...
        ''' <summary>Gets or sets the unique identifier for this reaction set.</summary>
        Public Property ID() As String = "" Implements Interfaces.IReactionSet.ID

        ''' <summary>Gets or sets the name of this reaction set.</summary>
        Public Property Name() As String = "" Implements Interfaces.IReactionSet.Name

        ''' <summary>Gets or sets the text description for this reaction set.</summary>
        Public Property Description() As String = "" Implements Interfaces.IReactionSet.Description

        ''' <summary>
        ''' Initializes a new empty instance of the <see cref="ReactionSet"/> class.
        ''' </summary>
        Sub New()
            MyBase.New()
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="ReactionSet"/> class with the specified ID, name, and description.
        ''' </summary>
        ''' <param name="id">The unique identifier for the reaction set.</param>
        ''' <param name="name">The name of the reaction set.</param>
        ''' <param name="description">A text description of the reaction set.</param>
        ...
        ''' </summary>
        ''' <returns>The reaction set name or the base ToString representation.</returns>
        Public Overrides Function ToString() As String
            If Name <> "" Then
                Return Name
            Else
        ...
        ''' </summary>
        ''' <returns>A cloned <see cref="ReactionSet"/> object with a new GUID.</returns>
        Public Function Clone() As Object Implements System.ICloneable.Clone

            Dim rxs As New ReactionSet()
            rxs.LoadData(SaveData())
...
#Region "    CAPE-OPEN Reaction Package Methods and Properties"
        ...
        ''' </summary>
        ''' <param name="index">A 1-based integer index or a string parameter name.</param>
        ''' <returns>The matching parameter object.</returns>
        ...
        ''' <summary>
        ''' Opens the CAPE-OPEN editor UI for this reaction package. Not implemented.
        ''' </summary>
        ...
        ''' <summary>
        ''' Initializes the CAPE-OPEN parameter collection for this reaction package.
        ''' </summary>
        ...
        ''' <summary>Gets the CAPE-OPEN parameter collection associated with this reaction package.</summary>
        ''' <returns>The parameter collection object.</returns>
        ...
        ''' <summary>Sets the CAPE-OPEN simulation context (flowsheet) for this reaction package.</summary>
        ...
        ''' <summary>
        ''' Terminates and cleans up the CAPE-OPEN reaction package. No action required.
        ''' </summary>
        ...
        ''' <summary>
        ''' Sets the kinetic reaction object used during CAPE-OPEN rate calculations.
        ''' </summary>
        ''' <param name="reactionsObject">The reaction object to associate with this context.</param>
        ...
        ''' </summary>
        ''' <param name="materialObject">The material object (material stream or COM material object) to associate.</param>
        ...
        ''' <summary>Gets or sets the CAPE-OPEN component description for this reaction set.</summary>
        Public Property ComponentDescription() As String = "" Implements CapeOpen.ICapeIdentification.ComponentDescription

        ''' <summary>Gets or sets the CAPE-OPEN component name for this reaction set.</summary>
        Public Property ComponentName() As String = "" Implements CapeOpen.ICapeIdentification.ComponentName

        ''' <summary>
        ''' Associates a material stream with this reaction package via the ICapeThermoMaterialContext interface.
        ''' </summary>
        ''' <param name="material">The material object (material stream or COM material object) to associate.</param>
        ...
        ''' <summary>
        ''' Dissociates the current material stream from this reaction package.
        ''' </summary>
...
#End Region
        ...
        ''' </summary>
        ''' <param name="data">A list of XML elements containing the serialized reaction set data.</param>
        ...
        ''' <summary>
        ''' Saves reaction set data to a list of XML elements for serialization.
        ''' </summary>
        ''' <returns>A list of XML elements representing the serialized reaction set data.</returns>
        Public Function SaveData() As System.Collections.Generic.List(Of System.Xml.Linq.XElement) Implements Interfaces.ICustomXMLSerialization.SaveData
        ...
        ''' </summary>
        ''' <param name="reactionID">The unique identifier of the reaction to update.</param>
        ''' <param name="rank">The new rank value for the reaction.</param>
        ...
        ''' </summary>
        ''' <param name="reactionID">The unique identifier of the reaction to enable.</param>
        ...
        ''' </summary>
        ''' <param name="reactionID">The unique identifier of the reaction to disable.</param>
    ...
    <System.Serializable()> Public Class ReactionSetBase
        ...
        ''' <summary>
        ''' Initializes a new empty instance of the <see cref="ReactionSetBase"/> class.
        ''' </summary>
        Sub New()
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="ReactionSetBase"/> class with the specified values.
        ''' </summary>
        ''' <param name="id">The reaction ID to reference in this set entry.</param>
        ''' <param name="rank">The priority rank of the reaction within the set.</param>
        ''' <param name="isactive">Whether the reaction is active in this set.</param>
        ...
        ''' <summary>Gets or sets whether this reaction is active in the reaction set.</summary>
        Public Property IsActive As Boolean Implements Interfaces.IReactionSetBase.IsActive

        ''' <summary>Gets or sets the priority rank of this reaction within the reaction set.</summary>
        Public Property Rank As Integer Implements Interfaces.IReactionSetBase.Rank

        ''' <summary>Gets or sets the reaction ID this entry refers to.</summary>
    ...
    <System.Serializable()> Public Class ReactionStoichBase
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="ReactionStoichBase"/> class with stoichiometric data.
        ''' </summary>
        ''' <param name="name">The compound name participating in the reaction.</param>
        ''' <param name="stoichcoeff">The stoichiometric coefficient (positive = product, negative = reactant).</param>
        ''' <param name="isbasereactant">Whether this compound is the base reactant for conversion calculations.</param>
        ''' <param name="directorder">The reaction order with respect to this compound for the forward rate.</param>
        ''' <param name="reversorder">The reaction order with respect to this compound for the reverse rate.</param>
        ...
        ''' <summary>Gets or sets the name of the compound in this stoichiometric entry.</summary>
        Public Property CompName As String = "" Implements Interfaces.IReactionStoichBase.CompName

        ''' <summary>Gets or sets the reaction order for the forward rate with respect to this compound.</summary>
        Public Property DirectOrder As Double Implements Interfaces.IReactionStoichBase.DirectOrder

        ''' <summary>Gets or sets whether this compound is the base reactant for conversion-based calculations.</summary>
        Public Property IsBaseReactant As Boolean Implements Interfaces.IReactionStoichBase.IsBaseReactant

        ''' <summary>Gets or sets the reaction order for the reverse rate with respect to this compound.</summary>
        Public Property ReverseOrder As Double Implements Interfaces.IReactionStoichBase.ReverseOrder

        ''' <summary>Gets or sets the stoichiometric coefficient for this compound (negative for reactants, positive for products).</summary>
...
#Region "Subclasses"
    ...
    <System.Serializable()> Public Class InteractionParameter
        ...
        Implements ICloneable, Interfaces.ICustomXMLSerialization, Interfaces.IInteractionParameter
        Public Property Comp1 As String = "" Implements Interfaces.IInteractionParameter.Comp1
        Public Property Comp2 As String = "" Implements Interfaces.IInteractionParameter.Comp2
        Public Property Model As String = "" Implements Interfaces.IInteractionParameter.Model
        Public DataType As String = ""
        Public Description As String = ""
        Public RegressionFile As String = ""
        Public Property Parameters As Dictionary(Of String, Object) Implements Interfaces.IInteractionParameter.Parameters
        ...
        Public Function Clone() As Object Implements System.ICloneable.Clone
            ...
            Dim copy As New InteractionParameter With {
                .Comp1 = Comp1,
                .Comp2 = Comp2,
                .Model = Model,
                .DataType = DataType,
            ...
            If Parameters IsNot Nothing Then
                For Each item In Parameters
                    copy.Parameters(item.Key) = item.Value
                Next
            End If
    ...
    <System.Serializable()> Public Class ConstantProperties
        ...
        Public Function Clone() As Object Implements System.ICloneable.Clone
            ...
            Dim comp = DirectCast(MemberwiseClone(), ConstantProperties)

            ' MemberwiseClone copies the scalars, which is all this class has apart from the
            ' members below. They are shared with the original until copied as well.

            comp.Elements = CopyOf(Elements)
            comp.MODFACGroups = CopyOf(MODFACGroups)
            comp.NISTMODFACGroups = CopyOf(NISTMODFACGroups)
            comp.UNIFACGroups = CopyOf(UNIFACGroups)

            comp.Vapor_Pressure_Tabular_Data = CopyOf(Vapor_Pressure_Tabular_Data)
            comp.Ideal_Gas_Heat_Capacity_Tabular_Data = CopyOf(Ideal_Gas_Heat_Capacity_Tabular_Data)
            comp.Liquid_Viscosity_Tabular_Data = CopyOf(Liquid_Viscosity_Tabular_Data)
            comp.Liquid_Density_Tabular_Data = CopyOf(Liquid_Density_Tabular_Data)
            comp.Liquid_Heat_Capacity_Tabular_Data = CopyOf(Liquid_Heat_Capacity_Tabular_Data)
            comp.Liquid_Thermal_Conductivity_Tabular_Data = CopyOf(Liquid_Thermal_Conductivity_Tabular_Data)
            comp.Vapor_Thermal_Conductivity_Tabular_Data = CopyOf(Vapor_Thermal_Conductivity_Tabular_Data)
            comp.Vapor_Viscosity_Tabular_Data = CopyOf(Vapor_Viscosity_Tabular_Data)
            comp.Solid_Density_Tabular_Data = CopyOf(Solid_Density_Tabular_Data)
            comp.Surface_Tension_Tabular_Data = CopyOf(Surface_Tension_Tabular_Data)
        ...
        Private Shared Function CopyOf(list As SortedList) As SortedList

            If list Is Nothing Then Return New SortedList()

            Return DirectCast(list.Clone(), SortedList)

        End Function

        Private Shared Function CopyOf(data As ITabularData) As ITabularData

            If data Is Nothing Then Return New TabularData()

            Return DirectCast(data, TabularData).Clone()
        ...
        Public Property PF_xP As Double? Implements Interfaces.ICompoundConstantProperties.PF_xP

        Public Property PF_xN As Double? Implements Interfaces.ICompoundConstantProperties.PF_xN

        Public Property PF_xA As Double? Implements Interfaces.ICompoundConstantProperties.PF_xA

        Public Property PF_n20 As Double? Implements Interfaces.ICompoundConstantProperties.PF_n20

        Public Property PF_Ri As Double? Implements Interfaces.ICompoundConstantProperties.PF_Ri
