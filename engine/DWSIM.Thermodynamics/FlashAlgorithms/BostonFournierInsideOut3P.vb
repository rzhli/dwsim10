...
'    Copyright 2011-2026 Daniel Wagner O. de Medeiros
'
...
Namespace PropertyPackages.Auxiliary.FlashAlgorithms
    ...
    <System.Serializable()> Public Class BostonFournierInsideOut3P
        ...
        Private Function EnergyBalance(ByVal R As Double) As Double
            ...
            For i = 0 To n
                Dim denom = R + (1 - R + S) / (2 * Kb0 * Exp(ui1(i))) + (1 - R - S) / (2 * Kb0 * Exp(ui2(i)))
                If Math.Abs(denom) < 1.0E-30 Then denom = 1.0E-30
                pi(i) = fi(i) / denom
            Next
            ...
            Next
            If Math.Abs(sumpi) < 1.0E-30 Then sumpi = 1.0E-30
            If Math.Abs(sumeuipi1) < 1.0E-30 Then sumeuipi1 = 1.0E-30
            If Math.Abs(sumeuipi2) < 1.0E-30 Then sumeuipi2 = 1.0E-30
            For i = 0 To n
            ...
            L2 = 1 - L1 - V
            Dim sumL = L1 + L2 : If Math.Abs(sumL) < 1.0E-30 Then sumL = 1.0E-30
            beta = L1 / sumL
            L = L1 + L2

            Dim eberror As Double

            If Math.Abs(B) > 1.0E-30 AndAlso Kb > 0.0 Then
                T = 1 / T_ + (Log(Kb) - A) / B
            Else
                T = 1 / T_
            End If
            If Math.Abs(T) > 1.0E-30 Then T = 1 / T Else T = 1.0E+4
        ...
        Private Function EntropyBalance(ByVal R As Double) As Double
            ...
            L1 = 0.5 * (S * V * (Kb / Kb0 - 1) + (1 + S) - V)
            L2 = 1 - L1 - V
            Dim sumL_s1 = L1 + L2 : If Math.Abs(sumL_s1) < 1.0E-30 Then sumL_s1 = 1.0E-30
            beta = L1 / sumL_s1
        ...
        Private Function EntropyBalanceAbs(ByVal R As Double) As Double
            ...
            L1 = 0.5 * (S * V * (Kb / Kb0 - 1) + (1 + S) - V)
            L2 = 1 - L1 - V
            Dim sumL_s2 = L1 + L2 : If Math.Abs(sumL_s2) < 1.0E-30 Then sumL_s2 = 1.0E-30
            beta = L1 / sumL_s2
        ...
        Public Function Flash_PH_3P(ByVal Vz() As Double, ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST() As Double, ByVal Vx1EST() As Double, ByVal Vx2EST() As Double, ByVal P As Double, ByVal H As Double, ByVal Tref As Double, ByVal PP As PropertyPackage) As Object
            ...
            If Kb <= 0.0 Then Kb = 1.0E-20
            If Kb_ <= 0.0 Then Kb_ = 1.0E-20
            Kb0 = Kb_

            Dim invTdiff = 1 / T_ - 1 / T
            If Math.Abs(invTdiff) > 1.0E-30 AndAlso Kb_ > 0.0 AndAlso Kb > 0.0 Then
                B = Log(Kb_ / Kb) / invTdiff
            Else
                B = 0.0
            End If
            A = Log(Kb) - B * (1 / T - 1 / T_)
            ...
            For i = 0 To n
                If Ki1(i) > 0.0 AndAlso Kb > 0.0 Then ui1(i) = Log(Ki1(i) / Kb) Else ui1(i) = -50.0
                If Ki2(i) > 0.0 AndAlso Kb > 0.0 Then ui2(i) = Log(Ki2(i) / Kb) Else ui2(i) = -50.0
            Next
            ...
            L2 = L2est
            L = L1 + L2
            Dim betaDenom_ph = L2 : If Math.Abs(betaDenom_ph) < 1.0E-30 Then betaDenom_ph = 1.0E-30
            beta = L1 / betaDenom_ph
...
restart:    Do
                ...
                For i = 0 To n
                    If Ki1(i) > 0.0 AndAlso Kb > 0.0 Then
                        uic1(i) = Log(Ki1(i) / Kb)
                    Else
                        uic1(i) = ui1(i)
                    End If
                    If Ki2(i) > 0.0 AndAlso Kb > 0.0 Then
                        uic2(i) = Log(Ki2(i) / Kb)
                    Else
                        uic2(i) = ui2(i)
                    End If
                ...
                Dim invTdiff_h = 1 / T_ - 1 / T
                If Math.Abs(invTdiff_h) > 1.0E-30 AndAlso Kb_ > 0.0 AndAlso Kb > 0.0 Then
                    Bc = Log(Kb_ / Kb) / invTdiff_h
                Else
                    Bc = B
                End If
            ...
            If Abs(fr) > itol Then
                ...
                Else
                    ...
                    Do
                        ...
                        If Math.Abs(dfdx2) < 1.0E-30 Then Exit Do
                        Dim dT_sp = fx2 / dfdx2
                        If Math.Abs(dT_sp) > 50.0 Then dT_sp = Math.Sign(dT_sp) * 50.0
                        x1 = x1 - dT_sp
        ...
        Public Function Flash_PS_3P(ByVal Vz() As Double, ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST() As Double, ByVal Vx1EST() As Double, ByVal Vx2EST() As Double, ByVal P As Double, ByVal S As Double, ByVal Tref As Double, ByVal PP As PropertyPackage) As Object
            ...
            Dim invTdiff_ps = 1 / T_ - 1 / T
            If Math.Abs(invTdiff_ps) > 1.0E-30 AndAlso Kb_ > 0.0 AndAlso Kb > 0.0 Then
                B = Log(Kb_ / Kb) / invTdiff_ps
            Else
                B = 0.0
            End If
            A = Log(Kb) - B * (1 / T - 1 / T_)
            ...
            L2 = L2est
            L = L1 + L2
            Dim betaDenom_ps = L2 : If Math.Abs(betaDenom_ps) < 1.0E-30 Then betaDenom_ps = 1.0E-30
            beta = L1 / betaDenom_ps
...
restart:    Do
                ...
                Dim invTdiff_ps9 = 1 / T_ - 1 / T
                If Math.Abs(invTdiff_ps9) > 1.0E-30 AndAlso Kb_ > 0.0 AndAlso Kb > 0.0 Then
                    Bc = Log(Kb_ / Kb) / invTdiff_ps9
                Else
                    Bc = B
                End If
                Ac = Log(Kb) - Bc * (1 / T - 1 / T_)
            ...
            If Abs(fr) > itol Then
                ...
                Else
                    ...
                    Do
                        ...
                        If Math.Abs(dfdx2) < 1.0E-30 Then Exit Do
                        Dim dT_sp2 = fx2 / dfdx2
                        If Math.Abs(dT_sp2) > 50.0 Then dT_sp2 = Math.Sign(dT_sp2) * 50.0
                        x1 = x1 - dT_sp2
                        If x1 < 10.0 Then x1 = 10.0
                        ecount += 1
                    Loop Until ecount > maxit_e Or Double.IsNaN(x1)
                ...
                Try
                    res = Me.Flash_PT(Vz, P, T, PP, False, Nothing)
                Catch ex As Exception
                End Try
                If res Is Nothing Then GoTo skipconfirm_ps
...
skipconfirm_ps:
        ...
        Public Function Flash_PT_3P(ByVal Vz As Double(), ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST As Double(), ByVal Vx1EST As Double(), ByVal Vx2EST As Double(), ByVal P As Double, ByVal T As Double, ByVal PP As PropertyPackages.PropertyPackage) As Object
            ...
            Do
                ...
                i = i + 1
            Loop Until i = n + 1
            If Math.Abs(soma_x1) < 1.0E-30 Then soma_x1 = 1.0E-30
            If Math.Abs(soma_x2) < 1.0E-30 Then soma_x2 = 1.0E-30
            If Math.Abs(soma_y) < 1.0E-30 Then soma_y = 1.0E-30
            i = 0
            Do
                Vx1(i) = Vx1(i) / soma_x1
                Vx2(i) = Vx2(i) / soma_x2
            ...
            For i = 0 To n
                If Ki1(i) > 0.0 Then ui1(i) = Log(Ki1(i)) Else ui1(i) = -50.0
                If Ki2(i) > 0.0 Then ui2(i) = Log(Ki2(i)) Else ui2(i) = -50.0
            Next
            ...
            L2 = L2est
            L = L1 + L2
            Dim betaDenom_pt = L2 : If Math.Abs(betaDenom_pt) < 1.0E-30 Then betaDenom_pt = 1.0E-30
            beta = L1 / betaDenom_pt
            ...
            Do
                ...
                For i = 0 To n
                    If Ki1(i) > 0.0 Then uic1(i) = Log(Ki1(i)) Else uic1(i) = -50.0
                    If Ki2(i) > 0.0 Then uic2(i) = Log(Ki2(i)) Else uic2(i) = -50.0
                Next
...
out:
        ...
        Private Function TPErrorFunc(ByVal Rtv As Double) As Double
            ...
            For i = 0 To n
                Dim denom_tp = Rt + (1 - Rt + S) / (2 * Kb0 * Exp(ui1(i))) + (1 - Rt - S) / (2 * Kb0 * Exp(ui2(i)))
                If Math.Abs(denom_tp) < 1.0E-30 Then denom_tp = 1.0E-30
                pi(i) = fi(i) / denom_tp
            Next
            ...
            L1 = 0.5 * (S * V * (Kb / Kb0 - 1) + (1 + S) - V)
            L2 = 1 - L1 - V
            Dim sumL_tp = L1 + L2 : If Math.Abs(sumL_tp) < 1.0E-30 Then sumL_tp = 1.0E-30
            beta = L1 / sumL_tp
        ...
        Private Function SErrorFuncAbs(ByVal S0 As Double) As Double
            ...
            For i = 0 To n
                Dim denom_se = Rt + (1 - Rt + S0) / (2 * Kb0 * Exp(ui1(i))) + (1 - Rt - S0) / (2 * Kb0 * Exp(ui2(i)))
                If Math.Abs(denom_se) < 1.0E-30 Then denom_se = 1.0E-30
                errfunc += fi(i) * (1 / Exp(ui1(i)) - 1 / Exp(ui2(i))) / denom_se
            Next
            Return errfunc ^ 2
        ...
        Private Function SErrorFunc(ByVal S0 As Double, ByVal Rt As Double) As Double
            ...
            Dim errfunc As Double = 0
            For i = 0 To n
                Dim denom_sf = Rt + (1 - Rt + S0) / (2 * Kb0 * Exp(ui1(i))) + (1 - Rt - S0) / (2 * Kb0 * Exp(ui2(i)))
                If Math.Abs(denom_sf) < 1.0E-30 Then denom_sf = 1.0E-30
                errfunc += fi(i) * (1 / Exp(ui1(i)) - 1 / Exp(ui2(i))) / denom_sf
            Next
            Return errfunc
        ...
        Public Function Flash_PV_3P(ByVal Vz() As Double, ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST As Double(), ByVal Vx1EST As Double(), ByVal Vx2EST As Double(), ByVal P As Double, ByVal V As Double, ByVal Tref As Double, ByVal PP As PropertyPackage, Optional ByVal ReuseKI As Boolean = False, Optional ByVal PrevKi() As Double = Nothing) As Object
            ...
            Dim invTdiff_pv = 1 / T_ - 1 / T
            If Math.Abs(invTdiff_pv) > 1.0E-30 AndAlso Kb_ > 0.0 AndAlso Kb > 0.0 Then
                B = Log(Kb_ / Kb) / invTdiff_pv
            Else
                B = 0.0
            End If
            A = Log(Kb) - B * (1 / T - 1 / T0)
            ...
            L2 = L2est
            L = L1 + L2
            Dim betaDenom_pv = L2 : If Math.Abs(betaDenom_pv) < 1.0E-30 Then betaDenom_pv = 1.0E-30
            beta = L1 / betaDenom_pv
            ...
            Do
                ...
                If RLoop Then
                    ...
                    Do
                        ...
                        ElseIf (R - fr / dfr) < 0.0# Or (R - fr / dfr) > 1.0# Then
                            If (R + 0.1) < 1.0# Then R += 0.1 Else R -= 0.1
                        Else
                            R = R - fr / dfr
                        End If
                ...
                For i = 0 To n
                    If Ki1(i) > 0.0 AndAlso Kb > 0.0 Then uic1(i) = Log(Ki1(i) / Kb) Else uic1(i) = ui1(i)
                    If Ki2(i) > 0.0 AndAlso Kb > 0.0 Then uic2(i) = Log(Ki2(i) / Kb) Else uic2(i) = ui2(i)
                Next

                Dim invTdiff_pv9 = 1 / T_ - 1 / T
                If Math.Abs(invTdiff_pv9) > 1.0E-30 AndAlso Kb_ > 0.0 AndAlso Kb > 0.0 Then
                    Bc = Log(Kb_ / Kb) / invTdiff_pv9
                Else
                    Bc = B
                End If
        ...
        Public Function Flash_TV_3P(ByVal Vz() As Double, ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST As Double(), ByVal Vx1EST As Double(), ByVal Vx2EST As Double(), ByVal T As Double, ByVal V As Double, ByVal Pref As Double, ByVal PP As PropertyPackage) As Object
            ...
            If Kb <= 0.0 Then Kb = 1.0E-20
            If Kb_ <= 0.0 Then Kb_ = 1.0E-20
            If Kb0 <= 0.0 Then Kb0 = 1.0E-20

            Dim logPratio = Log(P_ / P0)
            If Math.Abs(logPratio) > 1.0E-30 AndAlso Kb_ * P_ > 0.0 AndAlso Kb0 * P0 > 0.0 Then
                B = Log(Kb_ * P_ / (Kb0 * P0)) / logPratio
            Else
                B = 0.0
            End If
            ...
            L2 = L2est
            L = L1 + L2
            Dim betaDenom_tv = L2 : If Math.Abs(betaDenom_tv) < 1.0E-30 Then betaDenom_tv = 1.0E-30
            beta = L1 / betaDenom_tv
            ...
            Do
                ...
                Dim logPratio_tv9 = Log(P_ / P0)
                If Math.Abs(logPratio_tv9) > 1.0E-30 AndAlso Kb_ * P_ > 0.0 AndAlso Kb0 * P0 > 0.0 Then
                    Bc = Log(Kb_ * P_ / (Kb0 * P0)) / logPratio_tv9
                Else
                    Bc = B
                End If
        ...
        Private Function LiquidFractionBalance(ByVal R As Double) As Double
            ...
            For i = 0 To n
                Dim denom_lfb = R + (1 - R + S) / (2 * Kb0 * Exp(ui1(i))) + (1 - R - S) / (2 * Kb0 * Exp(ui2(i)))
                If Math.Abs(denom_lfb) < 1.0E-30 Then denom_lfb = 1.0E-30
                pi(i) = fi(i) / denom_lfb
            Next
            ...
            L1 = 0.5 * (S * V * (Kb / Kb0 - 1) + (1 + S) - V)
            L2 = 1 - L1 - V
            Dim sumL_lfb = L1 + L2 : If Math.Abs(sumL_lfb) < 1.0E-30 Then sumL_lfb = 1.0E-30
            beta = L1 / sumL_lfb
        ...
        Private Function LiquidFractionBalanceP(ByVal R As Double) As Double
            ...
            For i = 0 To n
                Dim denom_lfbp = R + (1 - R + S) / (2 * Kb0 * Exp(ui1(i))) + (1 - R - S) / (2 * Kb0 * Exp(ui2(i)))
                If Math.Abs(denom_lfbp) < 1.0E-30 Then denom_lfbp = 1.0E-30
                pi(i) = fi(i) / denom_lfbp
            Next
            ...
            L1 = 0.5 * (S * V * (Kb / Kb0 - 1) + (1 + S) - V)
            L2 = 1 - L1 - V
            Dim sumL_lfbp = L1 + L2 : If Math.Abs(sumL_lfbp) < 1.0E-30 Then sumL_lfbp = 1.0E-30
            beta = L1 / sumL_lfbp

            If Math.Abs(1 - B) > 1.0E-30 AndAlso Kb > 0.0 AndAlso P0 > 0.0 Then
                P = Exp((A - Log(Kb) - B * Log(P0)) / (1 - B))
            End If
            If P < 1.0 Then P = 1.0
            If Double.IsNaN(P) OrElse Double.IsInfinity(P) Then P = Pf
