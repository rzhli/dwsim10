        ...
        Public Function Flash_PT_3P(ByVal Vz As Double(), ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST As Double(), ByVal Vx1EST As Double(), ByVal Vx2EST As Double(), ByVal P As Double, ByVal T As Double, ByVal PP As PropertyPackages.PropertyPackage) As Object
            ...
            Do
                ...
                Else
                    ...
                    Dim df = 0.7 ' * Math.Min(Math.Min(df1, df2), 0.1)
...
out:
        ...
        Public Function Flash_PV_3P(ByVal Vz() As Double, ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST As Double(), ByVal Vx1EST As Double(), ByVal Vx2EST As Double(), ByVal P As Double, ByVal V As Double, ByVal Tref As Double, ByVal PP As PropertyPackage, Optional ByVal ReuseKI As Boolean = False, Optional ByVal PrevKi() As Double = Nothing) As Object
            ...
            Do
                ...
                ' NOTE: there used to be an "If Abs(dTP) < itol * 100 Then Exit Do" here. dTP is the last
                ' step of the inner temperature loop, so it measures whether THAT loop converged, not
                ' whether this one has. It is smallest exactly when the temperature is already right - and
                ' with three phases present the phase rule pins the temperature from the first iteration, so
                ' it fired immediately and left the compositions and the material balance wherever they
                ' happened to be. See the loop condition below for the real test.
...
out:        L1 = L1 * (1 - V) 'calculate global phase fractions
        ...
        Public Function Flash_TV_3P(ByVal Vz() As Double, ByVal Vest As Double, ByVal L1est As Double, ByVal L2est As Double, ByVal VyEST As Double(), ByVal Vx1EST As Double(), ByVal Vx2EST As Double(), ByVal T As Double, ByVal V As Double, ByVal Pref As Double, ByVal PP As PropertyPackage) As Object
            ...
            Do
                ...
                ecount += 1
                If ecount > maxit_e Then Throw New Exception(Calculator.GetLocalString("PropPack_FlashMaxIt"))
            ' The pressure is held to the same relative tolerance the compositions are held to. It used
            ' to be an absolute 1 Pa, which means quite different things at different pressures: about 1e-5
            ' of an atmosphere, but 5e-7 at 2 MPa, far tighter than the etol the compositions themselves are
            ' asked for and tighter than this loop can deliver, so it spent passes chasing digits nobody
            ' asked for. The pressure is not iterated independently anyway - it is recomputed from the
            ' composition each pass, so it settles when the composition does.
            Loop Until (e1 + e2 + e3 + e4) < etol And Math.Abs(P - Pant) < etol * P
...
out:        L1 = L1 * (1 - V) 'calculate global phase fractions
            ...
            ' Element 4 is the PRESSURE: this is a TV flash, so T is an input and P is what was solved for.
            ' It used to return T here, copied from Flash_PV_3P where T is indeed the unknown. The contract
            ' is stated at BaseFlashAlgorithm.CalculateEquilibrium ('TV {L1, V, Vx1, Vy, P, ecount, ...}'),
            ' which assigns element 4 straight to CalculatedPressure, and the sibling implementation in
            ' BostonFournierInsideOut3P.Flash_TV_3P returns P here.
            If gl1 < gl2 Then
                prevres = New PreviousResults With {.L1 = L1, .L2 = L2, .V = V, .Vy = Vy, .Vx1 = Vx1, .Vx2 = Vx2}
                Return New Object() {L1, V, Vx1, Vy, P, ecount, Ki1, L2, Vx2, 0.0#, PP.RET_NullVector}
            Else
                prevres = New PreviousResults With {.L1 = L2, .L2 = L1, .V = V, .Vy = Vy, .Vx1 = Vx2, .Vx2 = Vx1}
                Return New Object() {L2, V, Vx2, Vy, P, ecount, Ki1, L1, Vx1, 0.0#, PP.RET_NullVector}
            End If
