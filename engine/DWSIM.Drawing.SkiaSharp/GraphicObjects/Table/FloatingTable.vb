...
#End Region
        ...
        Public Overrides Sub Draw(ByVal g As Object)
            ...
            Try
                ...
                Using bpaint As New SKPaint()
                Using bpaint2 As New SKPaint()
                Using spaint As New SKPaint()
                Using tbpaint As New SKPaint()
                ...
                Else
                    ...
                    Dim GradientColors = {New SKColor(220, 240, 255, 230), New SKColor(200, 220, 235, 255)}
                    Dim GradientWeights = {0.0F, 1.0F}
                    Dim Gradient = SKShader.CreateLinearGradient(New SKPoint(X, Y), New SKPoint(X, Y + Height), GradientColors, GradientWeights, SKShaderTileMode.Clamp)
                    ...
                    If Me.Owner.GetFlowsheet IsNot Nothing Then
                        ...
                        If Not Owner.GraphicObject.ObjectType = Enums.GraphicObjects.ObjectType.MaterialStream Then
                            ...
                            If ncols <= 1.0 Then
                                ...
                                Height = sizes.Count * maxH + heading1size.Height + heading2size.Height + 6 * Padding
                                ...
                                Dim y0 = YD + 5 * Padding + heading1size.Height + heading2size.Height
                                Dim i = 0
                                For Each prop In props
                                    propstring = Owner.GetFlowsheet.GetTranslatedString(prop)
                                    pval0 = Owner.GetPropertyValue(prop, Owner.GetFlowsheet.FlowsheetOptions.SelectedUnitSystem)
                                    ...
                                    End If
                                    propunit = Owner.GetPropertyUnit(prop, Owner.GetFlowsheet.FlowsheetOptions.SelectedUnitSystem)
                                    canvas.DrawText(propstring, XD + Padding + margin, y0 + maxH - Padding, tbpaint)
                                    canvas.DrawText(propval, (maxL2 - MeasureString(propval, tpaint).Width) + XD + maxL1 + 4 * Padding, y0 + maxH - Padding, tpaint)
                                    canvas.DrawText(propunit, XD + maxL1 + maxL2 + 5 * Padding + margin, y0 + maxH - Padding, tbpaint)
                                    y0 += maxH
                                    i += 1
                                Next
                        ...
                        Else
                            ...
                            Dim maxPhaseCol = Math.Max(maxL5, Math.Max(maxL6, Math.Max(maxL7, Math.Max(maxL8, maxL9))))
                            maxL5 = maxPhaseCol
                            maxL6 = maxPhaseCol
                            maxL7 = maxPhaseCol
                            maxL8 = maxPhaseCol
                            ...
                            If MSObj.Flowsheet.FlowsheetOptions.DisplayFloatingTableCompoundAmounts And (YD + DeltaY) > 0 Then
                                ...
                                If DrawMode = 2 Then
                                    ...
                                    Dim GradientColors = {New SKColor(220, 240, 255, 230), New SKColor(175, 205, 225, 255)}
                                    Dim GradientWeights = {0.0F, 1.0F}
                                    Dim Gradient = SKShader.CreateLinearGradient(New SKPoint(X2, Y + DeltaY - size.Height), New SKPoint(X2, Y + DeltaY - size.Height + Height2), GradientColors, GradientWeights, SKShaderTileMode.Clamp)
