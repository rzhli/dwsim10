...
#End Region
                ...
                Else
                    ...
                    Else
                        ...
                        Math.Abs(Flowsheet.FlowsheetOptions.CurrentFlowsheetObjectZoomLevel - 1.0) > 0.000001 Then
                            ...
                            Dim zl = Flowsheet.FlowsheetOptions.CurrentFlowsheetObjectZoomLevel

                            DrawingCanvas.Scale(zl, zl, dobj.X + dobj.Width / 2, dobj.Y + dobj.Height / 2)
    ...
    Private Sub DrawInstructions(canvas As SKCanvas)
        ...
        text.AppendLine("(If you configured the simulation with the Setup Wizard, skip to Step 3)")
        text.AppendLine("")
        text.AppendLine("1. Select the compounds (Simulation Settings > Compounds).")
        text.AppendLine("2. Add at least one Property Package and Flash Algorithm (Simulation Settings > Thermodynamics).")
        text.AppendLine("3. Add unit operations and streams: drag them from the Objects palette on the right onto the flowsheet, or double-click a palette item.")
        text.AppendLine("4. Select an object on the flowsheet to open its editor in the Editor panel on the left.")
        text.AppendLine("5. Connect the blocks to streams from the object's editor (Connections tab).")
        text.AppendLine("6. Set the feed Material Streams and the unit operation inputs in the editor (Input Data).")
        text.AppendLine("7. Run the simulation (press F5, or the green Play button).")
        text.AppendLine("8. View results in the object's editor (Results tab), or in the Results and Material Streams panels.")
        text.AppendLine("")
        text.AppendLine("Useful shortcuts")
        text.AppendLine("")
        text.AppendLine("Double-click a palette item (or drag it): add the object to the flowsheet")
        text.AppendLine("Double-click an object: open its editor")
        text.AppendLine("Alt + Double-click an object: view its results")
        text.AppendLine("Shift + Double-click an object: edit its connections")
        text.AppendLine("Ctrl + Double-click an object: debug the object")
        text.AppendLine("Double-click an empty area: fit the flowsheet to the window")
...
#Else
            If MultiSelectMode Then
                SelectRectangle = Not GlobalSettings.KeyboardState.IsShiftDown
            Else
                SelectRectangle = GlobalSettings.KeyboardState.IsShiftDown
            End If
#End If
...
#If LINUX Then
                        ...
                        justselected = False
#Else
                    If GlobalSettings.KeyboardState.IsCtrlDown And MultiSelectMode Then
                        If Not Me.SelectedObjects.ContainsKey(Me.SelectedObject.Name) Then
                            Me.SelectedObjects.Add(Me.SelectedObject.Name, Me.SelectedObject)
                        Else
                ...
                If Not gObjTo Is Nothing Then
                    ...
                    Else
                        Select Case gObjFrom.ObjectType
                            ...
                            ObjectType.Filter, ObjectType.CustomUO, ObjectType.CapeOpenUO, ObjectType.FlowsheetUO, ObjectType.External, ObjectType.RCT_Conversion, ObjectType.Vessel
                                GoTo 100
                            Case Else
                                Throw New Exception("This connection is not allowed.")
                        End Select
