...
Public Class Settings
    ...
    ''' <summary>The system of units given to a new simulation.</summary>
    Public Shared Property PreferredSystemOfUnits As String = "SI"

    ''' <summary>Whether a user compound replaces the one already loaded under the same name.</summary>
    Public Shared Property ReplaceComps As Boolean = True

    ''' <summary>Whether the extensions and plugins in the application folder are loaded at startup.</summary>
    Public Shared Property LoadExtensionsAndPlugins As Boolean = True

    ''' <summary>Where the backup copies are written. Empty means the default folder.</summary>
    Public Shared Property BackupFolder As String = ""

    ''' <summary>Whether the flowsheet is calculated again after an undo or a redo.</summary>
    Public Shared Property UndoRedoRecalculateFlowsheet As Boolean = False
    ...
    Shared Sub LoadExcelSettings(Optional ByVal configfile As String = "")
        ...
        If configfile = "" Then configfile = AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + "dwsim.ini"

        If Not File.Exists(configfile) Then File.Copy(AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + "default.ini", configfile)
    ...
    Shared Sub SaveExcelSettings(Optional ByVal configfile As String = "")
        ...
        If configfile = "" Then
            configfile = AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + "dwsim.ini"
            File.Copy(AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + "default.ini", configfile, True)
        Else
            File.Copy(AppDomain.CurrentDomain.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar + "excelcompat.ini", configfile, True)
        End If
    ...
    ''' <summary>
    ''' Turns the argument of LoadSettings and SaveSettings into a path. An empty string
    ''' means the default file; a bare name is taken from the application data folder; a
    ''' rooted path is used as it is.
    ''' </summary>
    Private Shared Function ResolveConfigFile(configfile As String) As String

        If configfile = "" Then Return Path.Combine(GetConfigFileDir(), "dwsim.ini")
        If Path.IsPathRooted(configfile) Then Return configfile

        Return Path.Combine(GetConfigFileDir(), configfile)
    ...
    Shared Sub LoadSettings(Optional ByVal configfile As String = "")
        ...
        DrawingAntiAlias = source.Configs("Misc").GetBoolean("DrawingAntiAlias", True)
        SolverBreakOnException = source.Configs("Misc").GetBoolean("SolverBreakOnException", False)
        ResultsReportFontSize = source.Configs("Misc").GetInt("ResultsReportFontSize", 10)
        DarkMode = source.Configs("Misc").GetBoolean("DarkMode", False)

        PreferredSystemOfUnits = source.Configs("Misc").Get("PreferredSystemOfUnits", "SI")
        ReplaceComps = source.Configs("Misc").GetBoolean("ReplaceComps", True)
        LoadExtensionsAndPlugins = source.Configs("Misc").GetBoolean("LoadExtensionsAndPlugins", True)
        BackupFolder = source.Configs("Misc").Get("BackupFolder", "")
        UndoRedoRecalculateFlowsheet = source.Configs("Misc").GetBoolean("UndoRedoRecalculateFlowsheet", False)

        ' the culture the interface starts in is the one the user picked last
    ...
    Shared Sub SaveSettings(Optional ByVal configfile As String = "")
        ...
        source.Configs("Misc").Set("HideSolidPhase_COInterface", HideSolidPhaseFromCAPEOPENComponents)
        source.Configs("Misc").Set("DrawingAntiAlias", DrawingAntiAlias)
        source.Configs("Misc").Set("SolverBreakOnException", SolverBreakOnException)
        source.Configs("Misc").Set("ResultsReportFontSize", ResultsReportFontSize)
        source.Configs("Misc").Set("DarkMode", DarkMode)

        source.Configs("Misc").Set("PreferredSystemOfUnits", PreferredSystemOfUnits)
        source.Configs("Misc").Set("ReplaceComps", ReplaceComps)
        source.Configs("Misc").Set("LoadExtensionsAndPlugins", LoadExtensionsAndPlugins)
        source.Configs("Misc").Set("BackupFolder", BackupFolder)
        source.Configs("Misc").Set("UndoRedoRecalculateFlowsheet", UndoRedoRecalculateFlowsheet)
