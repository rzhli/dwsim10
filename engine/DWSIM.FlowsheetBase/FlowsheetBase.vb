...
''' <summary>
''' Abstract base class for all DWSIM flowsheet implementations.
''' Provides core flowsheet functionality including simulation object management,
''' property package handling, calculation orchestration, undo/redo, scripting,
''' and XML serialization support.
''' </summary>
<System.Runtime.InteropServices.ComVisible(True)> Partial Public MustInherit Class FlowsheetBase
    ...
    ''' <summary>
    ''' UI-supplied launcher for the data regression utility. Wired once at
    ''' app startup by each UI host (WinForms FormFlowsheet / Eto application
    ''' entry); FlowsheetBase.CallDataRegressionUtility delegates here so the
    ''' base class doesn't take a UI dependency. Returns Nothing if no host
    ''' has registered a launcher (e.g. headless contexts).
    ''' </summary>
    Public Shared Property DataRegressionLauncher As Func(Of String, String, String, IInteractionParameter)

    Public Function CallDataRegressionUtility(comp1 As String, comp2 As String, model As String) _
        As IInteractionParameter Implements IFlowsheet.CallDataRegressionUtility
        Dim launcher = DataRegressionLauncher
        If launcher Is Nothing Then Return Nothing
        Return launcher(comp1, comp2, model)
    End Function

    ''' <summary>Gets or sets a value indicating whether data loading is suppressed during deserialization.</summary>
    Public Property SupressDataLoading As Boolean = False

    ''' <summary>Gets or sets the weather data provider used for ambient condition lookups.</summary>
    ...
    ''' <summary>Gets or sets a value indicating whether the flowsheet is operating in dynamic simulation mode.</summary>
    Public Property DynamicMode As Boolean = False Implements IFlowsheet.DynamicMode

    ''' <summary>Gets or sets the dynamics manager responsible for dynamic simulation scheduling and event handling.</summary>
    Public Property DynamicsManager As IDynamicsManager = New DynamicsManager.Manager Implements IFlowsheet.DynamicsManager

    ''' <summary>Gets or sets a dynamic property bag for attaching arbitrary extra data to the flowsheet at runtime.</summary>
    Public Property ExtraProperties As New ExpandoObject Implements IFlowsheet.ExtraProperties

    ''' <summary>Flowsheet configuration variables including units of measure, selected compounds, property packages, and solver settings.</summary>
    Public WithEvents Options As New SharedClasses.DWSIM.Flowsheet.FlowsheetVariables

    ''' <summary>Stack of serialized snapshots used to support undo operations.</summary>
    Public UndoStack As New Stack(Of Tuple(Of Enums.SnapshotType, XDocument))
    ''' <summary>Stack of serialized snapshots used to support redo operations.</summary>
    Public RedoStack As New Stack(Of Tuple(Of Enums.SnapshotType, XDocument))

    ''' <summary>Gets or sets the list of informational and diagnostic messages generated during simulation.</summary>
    ...
    ''' <summary>Collection of optimization cases defined for this flowsheet.</summary>
    ...
    Public Shared Property AvailableUnitOperationExtensions As New Dictionary(Of String, IUnitOperationExtension)

    ''' <summary>Raised just before the flowsheet state is serialized to XML.</summary>
    Public Event FlowsheetSavingToXML(sender As Object, e As EventArgs)

    ''' <summary>Raised after the flowsheet state has been successfully serialized to XML.</summary>
    Public Event FlowsheetSavedToXML(sender As Object, e As EventArgs)

    ''' <summary>Raised just before the flowsheet state is deserialized from XML.</summary>
    Public Event FlowsheetLoadingFromXML(sender As Object, e As EventArgs)

    ''' <summary>Raised after the flowsheet state has been successfully deserialized from XML.</summary>
    Public Event FlowsheetLoadedFromXML(sender As Object, e As EventArgs)

    ''' <summary>Shared list of extender collections that add custom menu items or initialization scripts to flowsheet windows.</summary>
    Public Shared Extenders As New List(Of IExtenderCollection)

    ''' <summary>Gets or sets the dictionary of property packages available for selection in this flowsheet instance.</summary>
    ...
    ''' <summary>Gets or sets the list of unit-of-measure systems available for this flowsheet.</summary>
    Public Property AvailableSystemsOfUnits As New List(Of IUnitsOfMeasure) Implements IFlowsheet.AvailableSystemsOfUnits

    ''' <summary>Gets or sets the dictionary of external (plugin-defined) unit operations registered with this flowsheet.</summary>
    Public Property ExternalUnitOperations As New Dictionary(Of String, IExternalUnitOperation)

    ''' <summary>Gets or sets the dictionary of internal unit operations registered with this flowsheet.</summary>
    Public Property InternalUnitOperations As New Dictionary(Of String, IUnitOperation)

    ''' <summary>Gets or sets the dictionary of GHG emission compositions associated with streams in this flowsheet.</summary>
    ...
    ''' <summary>Delegate invoked to load spreadsheet data from the simulation XML document.</summary>
    Public LoadSpreadsheetData, SaveSpreadsheetData As Action(Of XDocument)

    ''' <summary>
    ''' Called while reading and writing the simulation file so a host can keep its own
    ''' section in it, the way the WinForms UI stores its panel layout. Each host is
    ''' responsible for a distinctly named element, since layouts are not interchangeable.
    ''' </summary>
    Public LoadLayoutData, SaveLayoutData As Action(Of XDocument)

    ''' <summary>Function that returns the active spreadsheet object for scripting access.</summary>
    Public GetSpreadsheetObjectFunc As Func(Of Object)

    ''' <summary>Function that retrieves cell values from the spreadsheet by range address.</summary>
    Public RetrieveSpreadsheetData As Func(Of String, List(Of String()))

    ''' <summary>Function that retrieves cell formatting information from the spreadsheet by range address.</summary>
    Public RetrieveSpreadsheetFormat As Func(Of String, List(Of String()))

    ''' <summary>Gets or sets custom script keyword definitions for flowsheet-level scripts.</summary>
    Public Property ScriptKeywordsF As String = ""

    ''' <summary>Gets or sets custom script keyword definitions for user-defined unit operation scripts.</summary>
    Public Property ScriptKeywordsU As String = ""

    ''' <summary>Optional translation function used to localize strings returned by <see cref="GetTranslatedString"/>.</summary>
    ...
    ''' </summary>
    ''' <param name="sender">The object that triggered the data change.</param>
    ''' <param name="e">Event arguments carrying details about the newly loaded data.</param>
    ...
    ''' <summary>Sets the resource manager used for resolving localised strings in this flowsheet.</summary>
    ''' <param name="_rm">A <see cref="Resources.ResourceManager"/> targeting the flowsheet string resources.</param>
    Public Sub SetResourcesManager(_rm As Resources.ResourceManager)
        rm = _rm
    End Sub

    ''' <summary>Sets the resource manager used for resolving localised property names in this flowsheet.</summary>
    ''' <param name="_prm">A <see cref="Resources.ResourceManager"/> targeting the property name resources.</param>
    ...
    ''' </summary>
    ''' <param name="ms">The material stream to populate with the selected compounds.</param>
    ...
    ''' <summary>Adds a graphic object to both the drawing surface and the <see cref="GraphicObjects"/> dictionary.</summary>
    ''' <param name="obj">The graphic object to add.</param>
    ...
    ''' <summary>Creates a simulation object of the specified type, places its graphic on the surface, and returns the new object.</summary>
    ...
    ''' <param name="tag">A user-visible label for the new object.</param>
    ''' <returns>The newly created <see cref="ISimulationObject"/>.</returns>
    ...
    ''' <param name="xcoord">The X coordinate on the drawing surface.</param>
    ''' <param name="ycoord">The Y coordinate on the drawing surface.</param>
    ''' <param name="id">The unique identifier to assign to the new object.</param>
    ''' <param name="tag">A user-visible label for the new object.</param>
    ''' <returns>The newly created <see cref="ISimulationObject"/>.</returns>
    ...
    ''' <summary>Registers a property package with the flowsheet, assigning a unique ID and tag if not already set.</summary>
    ''' <param name="obj">The property package to add.</param>
    ...
    ''' <summary>Adds a simulation object directly to the <see cref="SimulationObjects"/> dictionary without creating a graphic counterpart.</summary>
    ''' <param name="obj">The simulation object to add.</param>
    ...
    ''' <summary>Checks whether the flowsheet solver should pause or abort the current calculation, e.g. on user request.</summary>
    Public Sub CheckStatus() Implements IFlowsheet.CheckStatus
        FlowsheetSolver.FlowsheetSolver.CheckCalculatorStatus()
    End Sub

    ''' <summary>Connects two graphic objects via their connector ports and registers an undo snapshot.</summary>
    ''' <param name="gobjfrom">The source graphic object.</param>
    ''' <param name="gobjto">The target graphic object.</param>
    ''' <param name="fromidx">The output connector index on the source object.</param>
    ''' <param name="toidx">The input connector index on the target object.</param>
    ...
    ''' <summary>Disconnects two previously connected graphic objects and registers an undo snapshot.</summary>
    ''' <param name="gobjfrom">The source graphic object to disconnect from.</param>
    ''' <param name="gobjto">The target graphic object to disconnect.</param>
    ...
    ''' </summary>
    ''' <param name="objFrom">The upstream simulation object.</param>
    ''' <param name="objTo">The downstream simulation object.</param>
    ''' <param name="fromidx">The output connector index on the upstream object.</param>
    ''' <param name="toidx">The input connector index on the downstream object.</param>
    Public Sub Connect(objFrom As ISimulationObject, objTo As ISimulationObject, fromidx As Integer, toidx As Integer) Implements IFlowsheet.Connect
        If TypeOf objFrom Is IUnitOperation And TypeOf objTo Is IUnitOperation Then
            Dim x = (objTo.GraphicObject.X + objFrom.GraphicObject.X + objFrom.GraphicObject.Width) / 2
            Dim y = (objTo.GraphicObject.Y + objFrom.GraphicObject.Y) / 2
            Dim stream = AddObject(ObjectType.MaterialStream, x, y)
            If objTo.GraphicObject.X < objFrom.GraphicObject.X Then stream.GraphicObject.FlippedH = True
            ConnectObjects(objFrom.GraphicObject, stream.GraphicObject, fromidx, 0)
            ConnectObjects(stream.GraphicObject, objTo.GraphicObject, 0, toidx)
        Else
            ConnectObjects(objFrom.GraphicObject, objTo.GraphicObject, fromidx, toidx)
        End If
    End Sub

    ''' <summary>Disconnects two previously connected simulation objects via their graphic representations.</summary>
    ''' <param name="objFrom">The upstream simulation object.</param>
    ''' <param name="objTo">The downstream simulation object.</param>
    Public Sub Disconnect(objFrom As ISimulationObject, objTo As ISimulationObject) Implements IFlowsheet.Disconnect
        DisconnectObjects(objFrom.GraphicObject, objTo.GraphicObject)
    End Sub
    ...
    ''' </summary>
    ''' <param name="sender">The event sender (can be <c>Nothing</c>).</param>
    ''' <param name="e">Event arguments (can be <see cref="EventArgs.Empty"/>).</param>
    ''' <param name="gobj">The graphic object to delete.</param>
    ''' <param name="confirmation">When <c>True</c>, the implementation may prompt the user for confirmation.</param>
    ''' <param name="triggercalc">When <c>True</c>, triggers a recalculation after deletion.</param>
    ...
    ''' <summary>Displays a form or panel within the flowsheet's docking area. Must be implemented by the concrete UI host.</summary>
    ''' <param name="form">The form or dockable panel to display.</param>
    Public MustOverride Sub DisplayForm(form As Object) Implements IFlowsheet.DisplayForm

    ''' <summary>Gets or sets the full file path of the simulation file on disk.</summary>
    Public Property FilePath As String Implements IFlowsheet.FilePath

    ''' <summary>Gets the flowsheet configuration options (units, compounds, property packages, solver settings).</summary>
    ...
    ''' <summary>Returns the graphic object whose user-visible tag matches the specified value.</summary>
    ''' <param name="tag">The tag string to search for.</param>
    ''' <returns>The matching <see cref="IGraphicObject"/>, or <c>Nothing</c> if not found.</returns>
    ...
    ''' <summary>Returns the simulation object whose user-visible tag matches the specified value.</summary>
    ''' <param name="tag">The tag string to search for.</param>
    ''' <returns>The matching <see cref="ISimulationObject"/>, or <c>Nothing</c> if not found.</returns>
    ...
    ''' <summary>Creates and returns a new empty flowsheet instance of the same concrete type.</summary>
    ...
    ''' </summary>
    ''' <param name="tag">An optional tag to search by. Pass <c>Nothing</c> or an empty string to return the currently selected object.</param>
    ''' <returns>The matching <see cref="ISimulationObject"/>, or <c>Nothing</c> if none is selected or found.</returns>
    ...
    ''' <summary>Returns the underlying <see cref="GraphicsSurface"/> used for drawing and hit-testing flowsheet objects.</summary>
    ''' <returns>The drawing surface object.</returns>
    Public Function GetSurface() As Object Implements IFlowsheet.GetSurface
        Return FlowsheetSurface
    End Function

    ''' <summary>Returns the UI control that hosts the drawing surface, or <c>Nothing</c> in non-UI contexts.</summary>
    ''' <returns>The surface host control, or <c>Nothing</c>.</returns>
    ...
    ''' </summary>
    ''' <param name="text">The resource key or property path to translate.</param>
    ''' <returns>The localised string, or <paramref name="text"/> if no translation exists.</returns>
    Public Function GetTranslatedString(text As String) As String Implements IFlowsheet.GetTranslatedString
        ...
        ' Resolve against the embedded neutral (invariant) resource explicitly. Without a CultureInfo,
        ' ResourceManager honors Thread.CurrentUICulture and tries to load the en-US satellite assembly,
        ' which throws a NullReferenceException (InternalGetSatelliteAssembly) in headless/automation runs.
        ' This path is only reached when no GUI translate function is injected (see _translatefunction above).
        Dim ttext As String = rm.GetString(text, Globalization.CultureInfo.InvariantCulture)
        ...
        Else
            ...
            Try
                If text.Split("/"c).Length = 2 Then
                    ...
                    Dim ttext2 As String = prm.GetString(prop, Globalization.CultureInfo.InvariantCulture)
                    If ttext2 Is Nothing Then Return text Else Return ttext2 + " / " + text.Split("/"c)(1)
                Else
                    Dim ttext2 As String = prm.GetString(text, Globalization.CultureInfo.InvariantCulture)
    ...
    ''' <summary>Overload that ignores <paramref name="locale"/> and delegates to <see cref="GetTranslatedString(String)"/>.</summary>
    ''' <param name="text">The resource key or property path to translate.</param>
    ''' <param name="locale">Unused; reserved for future locale-aware lookup.</param>
    ...
    ''' <summary>Gets or sets the dictionary of all graphic objects currently placed on the flowsheet surface, keyed by unique ID.</summary>
    Public Property GraphicObjects As New Dictionary(Of String, IGraphicObject) Implements IFlowsheet.GraphicObjects

    ''' <summary>Gets or sets the dictionary of property packages attached to this flowsheet, keyed by unique ID.</summary>
    ...
    ''' <summary>Gets or sets the dictionary of reactions defined for this flowsheet, keyed by unique ID.</summary>
    ...
    ''' <summary>Gets or sets the dictionary of reaction sets (groups of reactions) defined for this flowsheet, keyed by name.</summary>
    ...
    ''' <summary>Gets or sets a value indicating whether flowsheet messages should be redirected to an external listener instead of the UI log.</summary>
    Public Property RedirectMessages As Boolean Implements IFlowsheet.RedirectMessages

    ''' <summary>Requests a full flowsheet calculation using the configured solver mode.</summary>
    Public Sub Solve()
    ...
    ''' When <paramref name="sender"/> is provided the object is queued and the solver runs asynchronously.
    ''' </summary>
    ''' <param name="sender">The simulation object that triggered the recalculation, or <c>Nothing</c> for a full solve.</param>
    ''' <param name="ChangeCalculationOrder">When <c>True</c>, recalculates the optimal calculation order before solving.</param>
    ...
    ''' <summary>Requests a full flowsheet calculation, optionally blocking the calling thread until it completes.</summary>
    ''' <param name="Wait">When <c>True</c>, the call blocks until the solver finishes; otherwise it runs on a background task.</param>
    ...
    ''' <summary>Requests a calculation scoped to a single object, or a full solve when <paramref name="Obj"/> is <c>Nothing</c>.</summary>
    ''' <param name="Obj">The simulation object to recalculate, or <c>Nothing</c> for a full flowsheet solve.</param>
    ''' <param name="Wait">When <c>True</c>, blocks until the calculation completes.</param>
    ...
    ''' <summary>Runs a full flowsheet calculation synchronously and returns any exceptions raised during the solve.</summary>
    ''' <returns>A list of exceptions from the calculation pass, or an empty list on success.</returns>
    ...
    ''' <summary>Marks all simulation objects as dirty and uncalculated, forcing a full recalculation on the next solve request.</summary>
    ...
    ''' <summary>Gets or sets the ordered dictionary of compounds currently selected for this flowsheet, sorted according to the configured ordering mode.</summary>
    ...
    ''' <summary>Writes a debug message at the specified verbosity level to the diagnostic output. Must be implemented by the concrete host.</summary>
    ''' <param name="text">The message text.</param>
    ''' <param name="level">The verbosity level (higher values indicate more detail).</param>
    Public MustOverride Sub ShowDebugInfo(text As String, level As Integer) Implements IFlowsheet.ShowDebugInfo

    ''' <summary>Displays a message to the user via the log panel or equivalent output. Must be implemented by the concrete host.</summary>
    ''' <param name="text">The message text.</param>
    ''' <param name="mtype">The message severity/type.</param>
    ...
    ''' </summary>
    Public Overridable Sub UpdateAILabelStatus(text As String, isWorking As Boolean) Implements IFlowsheet.UpdateAILabelStatus
        ' no-op in headless / cross-platform flowsheets
    End Sub

    ''' <summary>Gets or sets the dictionary of all simulation objects in this flowsheet, keyed by unique ID.</summary>
    Public Property SimulationObjects As New Dictionary(Of String, ISimulationObject) Implements IFlowsheet.SimulationObjects

    ''' <summary>Refreshes all open property editors to reflect the current state of simulation objects. Must be implemented by the concrete host.</summary>
    Public MustOverride Sub UpdateOpenEditForms() Implements IFlowsheet.UpdateOpenEditForms

    ''' <summary>Closes all currently open property editors. Must be implemented by the concrete host.</summary>
    Public MustOverride Sub CloseOpenEditForms() Implements IFlowsheet.CloseOpenEditForms

    ''' <summary>Gets or sets the FIFO queue of pending calculation requests consumed by the flowsheet solver.</summary>
    ...
    ''' </summary>
    ''' <param name="typename">The display name of the object type (e.g. "Pump", "Distillation Column").</param>
    ''' <param name="x">The X coordinate on the drawing surface.</param>
    ''' <param name="y">The Y coordinate on the drawing surface.</param>
    ''' <param name="tag">An optional user-visible label. Auto-generated when empty.</param>
    ''' <param name="id">An optional unique ID. A new GUID-based ID is assigned when empty.</param>
    ''' <param name="CreateConnected">When <c>True</c>, automatically creates and connects surrounding stream objects.</param>
    ''' <returns>The newly created <see cref="ISimulationObject"/>, or <c>Nothing</c> if the type name is unrecognised.</returns>
    Public Function AddObject(ByVal typename As String, ByVal x As Integer, ByVal y As Integer, Optional ByVal tag As String = "",
        ...
        Select Case typename
            ...
            Case "Information Carrier"

                Return Me.SimulationObjects(AddObjectToSurface(ObjectType.OT_InformationCarrier, x, y, tag,,, CreateConnected))
            ...
            Case "MPC Controller"
                ...
                Return Me.SimulationObjects(AddObjectToSurface(ObjectType.Controller_MPC, x, y, tag,,, CreateConnected))
    ...
    ''' <summary>Returns a sorted array of all built-in flowsheet object type names that can be passed to <see cref="AddObject(String,Integer,Integer,String,String,Boolean)"/>.</summary>
    ''' <returns>A <see cref="String"/> array of object type display names.</returns>
    ...
    ''' <summary>Creates a simulation object by type name at the default position (50, 50) and assigns the given object name.</summary>
    ''' <param name="typename">The display name of the object type.</param>
    ''' <param name="objname">The tag/name to assign to the new object.</param>
    ''' <returns>The newly created <see cref="ISimulationObject"/>, or <c>Nothing</c> if the type name is unrecognised.</returns>
    Public Function AddFlowsheetObject(typename As String, objname As String) As ISimulationObject Implements IFlowsheet.AddFlowsheetObject
        ...
        Select Case typename
            ...
            Case "MPC Controller"
                ...
                Return AddObject(ObjectType.Controller_MPC, 50, 50, objname)
    ...
    ''' <summary>
    ''' Low-level method that instantiates the graphic and simulation objects for a given <see cref="ObjectType"/>,
    ''' adds them to the drawing surface and the <see cref="SimulationObjects"/> dictionary, and returns the new object's ID.
    ''' </summary>
    ''' <param name="type">The object type enumeration value.</param>
    ''' <param name="x">The X coordinate on the drawing surface.</param>
    ''' <param name="y">The Y coordinate on the drawing surface.</param>
    ''' <param name="tag">An optional user-visible label.</param>
    ''' <param name="id">An optional explicit unique ID.</param>
    ''' <param name="uoobj">An optional external unit operation instance (used when <paramref name="type"/> is <c>ObjectType.External</c>).</param>
    ''' <param name="CreateConnected">When <c>True</c>, auto-creates and connects stream objects around the new object.</param>
    ''' <returns>The unique ID string of the newly created simulation object.</returns>
    Public Function AddObjectToSurface(type As ObjectType, x As Integer, y As Integer,
        ...
        Select Case type
            ...
            Case ObjectType.Controller_MPC

                Dim myGobj As New MPCControllerGraphic(mpx, mpy, 50, 50)
                myGobj.Tag = "MPC-" + objindex
                If tag <> "" Then myGobj.Tag = tag
                gObj = myGobj
                CheckTag(gObj)
                gObj.Name = "MPC-" & Guid.NewGuid.ToString
                If id <> "" Then gObj.Name = id
                GraphicObjects.Add(gObj.Name, myGobj)
                Dim myObj As New MPCController(gObj.Name, "")
                myObj.GraphicObject = myGobj
                SimulationObjects.Add(myGobj.Name, myObj)
            ...
            Case ObjectType.OT_InformationCarrier
                ...
                gObj = myNode
                CheckTag(gObj)
                gObj.Name = "IC-" & Guid.NewGuid.ToString
                If id <> "" Then gObj.Name = id
                GraphicObjects.Add(gObj.Name, myNode)
                Dim myIC As New InformationCarrier(myNode.Name, "Information Carrier")
                myIC.GraphicObject = myNode
                SimulationObjects.Add(myNode.Name, myIC)
    ...
    ''' </summary>
    ''' <param name="obj">The simulation object to wire up.</param>
    ''' <param name="scheme">
    ''' Connection scheme:
    ''' <c>1</c> - create and connect new streams;
    ''' <c>2</c> - reuse nearby unconnected streams where possible.
    ''' </param>
    ...
    Public Sub LoadFromXML(xdoc As XDocument) Implements IFlowsheet.LoadFromXML
        ...
        If xdoc.Element("DWSIM_Simulation_Data").Element("GeneralInfo")?.Element("BuildVersion") IsNot Nothing Then
        ...
        If LoadLayoutData IsNot Nothing Then LoadLayoutData.Invoke(xdoc)
    ...
    Public Function SaveToXML() As XDocument Implements IFlowsheet.SaveToXML
        ...
        If SaveLayoutData IsNot Nothing Then SaveLayoutData.Invoke(xdoc)
        ...
        For Each so As ISimulationObject In SimulationObjects.Values
            Try
                ...
                If TryCast(so, Spec) IsNot Nothing Then
                    ...
                    End If
                End If
                If TryCast(so, InformationCarrier) IsNot Nothing Then
                    Dim so2 As InformationCarrier = so
                    If SimulationObjects.ContainsKey(so2.SourceObjectData.ID) Then
                        so2.SourceObject = SimulationObjects(so2.SourceObjectData.ID)
                        DirectCast(so2.GraphicObject, InformationCarrierGraphic).ConnectedToSv = so2.SourceObject.GraphicObject
                    End If
                    If SimulationObjects.ContainsKey(so2.TargetObjectData.ID) Then
                        so2.TargetObject = SimulationObjects(so2.TargetObjectData.ID)
                        DirectCast(so2.GraphicObject, InformationCarrierGraphic).ConnectedToTv = so2.TargetObject.GraphicObject
                    End If
                    If SimulationObjects.ContainsKey(so2.TargetObjectData2.ID) Then
                        so2.TargetObject2 = SimulationObjects(so2.TargetObjectData2.ID)
                        DirectCast(so2.GraphicObject, InformationCarrierGraphic).ConnectedToTv2 = so2.TargetObject2.GraphicObject
                    End If
                    If SimulationObjects.ContainsKey(so2.TargetObjectData3.ID) Then
                        so2.TargetObject3 = SimulationObjects(so2.TargetObjectData3.ID)
                        DirectCast(so2.GraphicObject, InformationCarrierGraphic).ConnectedToTv3 = so2.TargetObject3.GraphicObject
                    End If
                End If
    ...
    Public Sub Initialize() Implements IFlowsheet.Initialize
        ...
        If ObjectList.Count = 0 Then
            ...
            ' Calculator assembly (DWSIM.Thermodynamics) - filter: implements ISimulationObject
            Try
                Dim calcTypes As Type()
                If GlobalSettings.Settings.RunningPlatform() = Settings.Platform.Linux Then
                    calcTypes = calculatorassembly.GetExportedTypes()
                Else
                    calcTypes = calculatorassembly.GetTypes()
                End If
                aTypeList.AddRange(calcTypes.Where(Function(x) If(x.GetInterface("DWSIM.Interfaces.ISimulationObject") IsNot Nothing, True, False)))
            Catch ex As System.Reflection.ReflectionTypeLoadException
                ' Recover types that loaded successfully; skip the ones that failed
                aTypeList.AddRange(ex.Types.Where(Function(t) t IsNot Nothing AndAlso
                    t.GetInterface("DWSIM.Interfaces.ISimulationObject") IsNot Nothing))
                Logging.Logger.LogError("Initialize - partial type load from " & calculatorassembly.FullName, ex)
            End Try

            ' Unit operations assembly (DWSIM.UnitOperations) - filter: ISimulationObject, not abstract, not external
            Try
                Dim uoTypes As Type()
                If GlobalSettings.Settings.RunningPlatform() = Settings.Platform.Linux Then
                    uoTypes = unitopassembly.GetExportedTypes()
                Else
                    uoTypes = unitopassembly.GetTypes()
                End If
                aTypeList.AddRange(uoTypes.Where(Function(x) If(x.GetInterface("DWSIM.Interfaces.ISimulationObject") IsNot Nothing And
                                                                   Not x.IsAbstract And x.GetInterface("DWSIM.Interfaces.IExternalUnitOperation") Is Nothing, True, False)))
            Catch ex As System.Reflection.ReflectionTypeLoadException
                ' Recover types that loaded successfully; skip the ones that failed
                aTypeList.AddRange(ex.Types.Where(Function(t) t IsNot Nothing AndAlso
                    t.GetInterface("DWSIM.Interfaces.ISimulationObject") IsNot Nothing AndAlso
                    Not t.IsAbstract AndAlso t.GetInterface("DWSIM.Interfaces.IExternalUnitOperation") Is Nothing))
                Logging.Logger.LogError("Initialize - partial type load from " & unitopassembly.FullName, ex)
            End Try
...
Label_00CC:
    ...
    Private Sub AddSystemsOfUnits()
        ...
        With Me.AvailableSystemsOfUnits
            ...
            Try
                Dim ujson = GlobalSettings.Settings.UserUnits
                If ujson IsNot Nothing AndAlso ujson.TrimStart().StartsWith("[") Then
                    ...
                    Dim userunits = Newtonsoft.Json.JsonConvert.DeserializeObject(Of List(Of SystemsOfUnits.Units))(ujson, options)
                    If userunits IsNot Nothing Then
    ...
    Private Sub RunScript_IronPython(scripttext As String)
        ...
        ' The standard streams are built while the language loads, and they take their encoding
        ' from the console. A GUI process reports a legacy code page there, which .NET (Core)
        ' cannot describe, and the language then fails to load. Handing the runtime UTF-8 streams
        ' before asking it for the engine keeps that out of the picture.
        Dim runtime = IronPython.Hosting.Python.CreateRuntime(opts)

        runtime.IO.SetOutput(New FlowsheetLogTextStream(Me), UTF8Encoding.UTF8)
        runtime.IO.SetErrorOutput(New FlowsheetLogTextStream(Me), UTF8Encoding.UTF8)
        runtime.IO.SetInput(New MemoryStream(), UTF8Encoding.UTF8)
    ...
    ''' <summary>
    ''' UI hook for reordering simulation objects in the calculation order. Cross-platform
    ''' hosts (Avalonia / WinForms) assign a delegate that opens the appropriate
    ''' dialog. If nothing is wired up, ChangeCalculationOrder returns the input unchanged.
    ...
    ''' <summary>
    ''' UI hooks for the system clipboard, which copying and pasting flowsheet objects goes
    ''' through. Each host assigns them at startup; without them the text is kept in this
    ''' process only, so copying between two running instances needs the host to wire them up.
    ''' </summary>
    Public Shared Property ClipboardTextWriter As Action(Of String)
        Get
            Return SharedClasses.SystemClipboard.Writer
        End Get
        Set(value As Action(Of String))
            SharedClasses.SystemClipboard.Writer = value
        End Set
    End Property

    ''' <summary>Reads the clipboard text. See <see cref="ClipboardTextWriter"/>.</summary>
    Public Shared Property ClipboardTextReader As Func(Of String)
        Get
            Return SharedClasses.SystemClipboard.Reader
        End Get
        Set(value As Func(Of String))
            SharedClasses.SystemClipboard.Reader = value
        End Set
    End Property

    Private Shared Sub SetClipboardText(text As String)

        SharedClasses.SystemClipboard.SetText(text)

    End Sub

    Private Shared Function GetClipboardText() As String

        Return SharedClasses.SystemClipboard.GetText()
    ...
    Public Function ChangeCalculationOrder(objects As List(Of String)) As List(Of String) Implements IFlowsheet.ChangeCalculationOrder
        ...
        If CustomCalcOrderEditor IsNot Nothing Then
            Dim result As List(Of String) = objects
            RunCodeOnUIThread(Sub()
                                  Try
                                      result = CustomCalcOrderEditor.Invoke(objects)
                                  Catch
                                      result = objects
                                  End Try
                              End Sub)
    ...
    Public Property LastSuccessfulSolutionState As String = "" Implements IFlowsheet.LastSuccessfulSolutionState
    ...
    ''' </summary>
    ''' <param name="name">Reaction name</param>
    ''' <param name="description">Reaction description</param>
    ''' <param name="compounds_and_stoichcoeffs">Compound names and their stoichiometric coefficients</param>
    ''' <param name="basecompound">Base compound</param>
    ''' <param name="reactionphase">reaction phase: mixture, vapor, liquid or solid</param>
    ''' <param name="conversionExpression">Conversion expression as a function of temperature (T) (result in %)</param>
    ''' <returns>IReaction object</returns>
    ...
    ''' <param name="compounds_and_stoichcoeffs">Compound names and their stoichiometric coefficients</param>
    ''' <param name="basecompound">Base compound</param>
    ''' <param name="basis">Reaction basis: activity, fugacity, partial pressure, molar concentration, mass concentration, molar fraction or mass fraction</param>
    ''' <param name="units">basis units</param>
    ''' <param name="Tapproach">temperature approach in K</param>
    ''' <param name="lnKeq_fT"></param>
    ''' <returns>IReaction object</returns>
    ...
    ''' <param name="name">Reaction name</param>
    ''' <param name="description">reaction description</param>
    ''' <param name="compounds_and_stoichcoeffs">Compounds and stoichiometric coefficients</param>
    ''' <param name="directorders">direct order coefficients</param>
    ''' <param name="reverseorders">reverse order coefficients</param>
    ''' <param name="basecompound">base compound</param>
    ''' <param name="reactionphase">reaction phase: mixture, vapor, liquid or solid</param>
    ''' <param name="basis">Reaction basis: activity, fugacity, partial pressure, molar concentration, mass concentration, molar fraction or mass fraction</param>
    ''' <param name="amountunits">amount units</param>
    ''' <param name="rateunits">rate units</param>
    ''' <param name="Aforward">forward reaction A parameter</param>
    ''' <param name="Eforward">forward reaction E parameter</param>
    ''' <param name="Areverse">reverse reaction A parameter</param>
    ''' <param name="Ereverse">reverse reaction E parameter</param>
    ''' <param name="Expr_forward"></param>
    ''' <param name="Expr_reverse"></param>
    ''' <returns>IReaction object</returns>
...
#Region "    Cut/Copy/Paste Objects"
    ...
    Sub CutObjects(Optional ByVal addundo As Boolean = True)
        ...
        Try
            ...
            For Each gobj As GraphicObject In FlowsheetSurface.SelectedObjects.Values
            ...
            For Each s As String In indexes
                ...
                If Not gobj Is Nothing Then
                    DeleteSelectedObject(Me, New EventArgs(), gobj, False)
                    FlowsheetSurface.SelectedObjects.Remove(gobj.Name)
                End If
            Next
    ...
    Sub CopyObjects()
        ...
        Try
            ...
            SetClipboardText(xdoc.ToString())
    ...
    Sub PasteObjects(Optional ByVal addundo As Boolean = True)
        ...
        Try
            ...
            For Each comp In complist
                If Not AvailableCompounds.ContainsKey(comp.Name) Then
                    AvailableCompounds.Add(comp.Name, comp)
                    AddCompound(comp.Name)
                End If
            Next
            ...
            FlowsheetSurface.SelectedObjects.Clear()
            ...
            For Each obj In objlist
                If FlowsheetSurface.SelectedObject Is Nothing Then FlowsheetSurface.SelectedObject = obj.GraphicObject
                FlowsheetSurface.SelectedObjects.Add(obj.Name, obj.GraphicObject)
            Next
    ...
    Public Sub RestoreLastSuccessfulSolutionState() Implements IFlowsheet.RestoreLastSuccessfulSolutionState

        Dim data = Newtonsoft.Json.JsonConvert.DeserializeObject(Of List(Of XElement))(LastSuccessfulSolutionState)
        LoadProcessData(data)

    End Sub

    Public Sub SaveCurrentStateAsLastSuccessfulSolution() Implements IFlowsheet.SaveCurrentStateAsLastSuccessfulSolution

        Try
            LastSuccessfulSolutionState = Newtonsoft.Json.JsonConvert.SerializeObject(GetProcessData())
        Catch ex As Exception
        End Try
        ...
        If Not Settings.AutomationMode Then
            ...
            Using bmp As New SKBitmap(GetFlowsheetSurfaceWidth() * scale, GetFlowsheetSurfaceHeight() * scale)
                Using canvas As New SKCanvas(bmp)
                    ...
                    Using str As New FileStream(pngfilepath, FileMode.CreateNew)
                        d.SaveTo(str)
                    End Using
                End Using
            End Using
