...
Namespace UnitOperations
    ...
    ''' </summary>
    <System.Serializable()> Public Partial Class ShortcutColumn
        ...
        Public Enum CondenserType
            ''' <summary>Total condenser ï¿½ all overhead vapour is condensed and returned as liquid distillate.</summary>
            TotalCond = 0
            ''' <summary>Partial condenser ï¿½ only part of the overhead is condensed; a vapour distillate product is withdrawn.</summary>
            PartialCond = 1
        End Enum

        ''' <summary>Gets or sets the estimated column diameter (m) from the last calculation.</summary>
        Public Property EstimatedDiameter As Double = Double.NaN 'm

        ''' <summary>Gets or sets the estimated column height (m) from the last calculation.</summary>
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="ShortcutColumn"/> class with a name and description.
        ''' </summary>
        ''' <param name="name">The display name of the shortcut column.</param>
        ''' <param name="description">A brief description of the shortcut column.</param>
        ...
        ''' <summary>Gets the list of equipment sub-types available for this column (Tray, Packed).</summary>
        ...
        ''' <summary>Creates the dimensions list (Diameter, Height, NumberOfTrays) used for column sizing.</summary>
        ...
        ''' <summary>Updates the dimension values from the current calculated results.</summary>
        ...
        ''' <summary>Creates a deep copy of this shortcut column via XML serialization.</summary>
        ...
        Public Overrides Sub CreateDynamicProperties()
            ...
            AddDynamicProperty("Reboiler Volume", "Liquid holdup volume in the reboiler sump (m3).", 1.0, UnitOfMeasure.volume, 1.0.GetType())
        ...
        Public Overrides Sub RunDynamicModel()
            ...
            Dim VolCond As Double = GetDynamicProperty("Condenser Volume")
            Dim VolReb As Double = GetDynamicProperty("Reboiler Volume")
            ...
            If Reset Then
                CondenserAccumulationStream = Nothing
                ReboilerAccumulationStream = Nothing
                SetDynamicProperty("Reset Content", 0)
            End If

            Dim feed As MaterialStream = Me.GetInletMaterialStream(0)
            Dim distillate As MaterialStream = Me.GetOutletMaterialStream(0)
            Dim bottoms As MaterialStream = Me.GetOutletMaterialStream(1)

            If CondenserAccumulationStream Is Nothing OrElse ReboilerAccumulationStream Is Nothing Then
                If InitializeFromInlet AndAlso feed IsNot Nothing Then
                    CondenserAccumulationStream = feed.CloneXML
                    ReboilerAccumulationStream = feed.CloneXML
                    Dim density = feed.Phases(0).Properties.density.GetValueOrDefault
                    If density > 0 Then
                        CondenserAccumulationStream.SetMassFlow(density * VolCond)
                        ReboilerAccumulationStream.SetMassFlow(density * VolReb)
                    End If
                    CondenserAccumulationStream.SetPressure(m_condenserpressure)
                    ReboilerAccumulationStream.SetPressure(m_boilerpressure)
                    CondenserAccumulationStream.SpecType = StreamSpec.Temperature_and_Pressure
                    ReboilerAccumulationStream.SpecType = StreamSpec.Temperature_and_Pressure
                    CondenserAccumulationStream.PropertyPackage = PropertyPackage
                    ReboilerAccumulationStream.PropertyPackage = PropertyPackage
                    CondenserAccumulationStream.PropertyPackage.CurrentMaterialStream = CondenserAccumulationStream
                    CondenserAccumulationStream.Calculate()
                    ReboilerAccumulationStream.PropertyPackage.CurrentMaterialStream = ReboilerAccumulationStream
                    ReboilerAccumulationStream.Calculate()
                Else
                    Calculate()
                    Return
                End If
            ...
            CondenserAccumulationStream.SetFlowsheet(FlowSheet)
            ReboilerAccumulationStream.SetFlowsheet(FlowSheet)

            If feed IsNot Nothing AndAlso feed.GetMassFlow() > 0 Then
                ReboilerAccumulationStream = ReboilerAccumulationStream.Add(feed, timestep)
            End If

            If distillate IsNot Nothing AndAlso distillate.GetMassFlow() > 0 Then
                CondenserAccumulationStream = CondenserAccumulationStream.Subtract(distillate, timestep)
                If CondenserAccumulationStream.GetMassFlow() < 0 Then CondenserAccumulationStream.SetMassFlow(0)
            End If

            If bottoms IsNot Nothing AndAlso bottoms.GetMassFlow() > 0 Then
                ReboilerAccumulationStream = ReboilerAccumulationStream.Subtract(bottoms, timestep)
                If ReboilerAccumulationStream.GetMassFlow() < 0 Then ReboilerAccumulationStream.SetMassFlow(0)
            End If

            ReboilerAccumulationStream.SetPressure(m_boilerpressure)
            ReboilerAccumulationStream.SpecType = StreamSpec.Pressure_and_Enthalpy
            ReboilerAccumulationStream.PropertyPackage = PropertyPackage
            ReboilerAccumulationStream.PropertyPackage.CurrentMaterialStream = ReboilerAccumulationStream
            ReboilerAccumulationStream.Calculate()

            Dim vapFromReboiler = ReboilerAccumulationStream.CloneXML()
            vapFromReboiler.SetFlowsheet(FlowSheet)
            vapFromReboiler.PropertyPackage = PropertyPackage

            Dim vapMassFlow = ReboilerAccumulationStream.Vapor.Properties.massflow.GetValueOrDefault()
            If vapMassFlow > 0 Then
                CondenserAccumulationStream = CondenserAccumulationStream.Add(vapFromReboiler, 1.0)
                CondenserAccumulationStream.SetMassFlow(CondenserAccumulationStream.GetMassFlow())
                ReboilerAccumulationStream.SubtractFromPhase(vapMassFlow, PropertyPackages.Phase.Vapor, False)
            End If

            CondenserAccumulationStream.SetPressure(m_condenserpressure)
            CondenserAccumulationStream.SpecType = StreamSpec.Pressure_and_Enthalpy
            CondenserAccumulationStream.PropertyPackage = PropertyPackage
            CondenserAccumulationStream.PropertyPackage.CurrentMaterialStream = CondenserAccumulationStream
            CondenserAccumulationStream.Calculate()

            Dim liqFromCondenser = CondenserAccumulationStream.CloneXML()
            liqFromCondenser.SetFlowsheet(FlowSheet)
            liqFromCondenser.PropertyPackage = PropertyPackage

            Dim liqMassFlow = CondenserAccumulationStream.OverallLiquid.Properties.massflow.GetValueOrDefault()
            Dim refluxFlow = liqMassFlow * m_refluxratio / (1.0 + m_refluxratio)
            If refluxFlow > 0 Then
                ReboilerAccumulationStream = ReboilerAccumulationStream.Add(liqFromCondenser, 1.0)
                ReboilerAccumulationStream.SetMassFlow(ReboilerAccumulationStream.GetMassFlow())
                CondenserAccumulationStream.SubtractFromPhase(refluxFlow, PropertyPackages.Phase.Liquid1, False)
            End If

            If distillate IsNot Nothing Then
                distillate.AssignFromPhase(PhaseLabel.Liquid1, CondenserAccumulationStream, False)
                distillate.SetTemperature(CondenserAccumulationStream.GetTemperature)
                distillate.SetMassEnthalpy(CondenserAccumulationStream.GetMassEnthalpy)
                distillate.SetPressure(m_condenserpressure)
            End If

            If bottoms IsNot Nothing Then
                bottoms.AssignFromPhase(PhaseLabel.Liquid1, ReboilerAccumulationStream, False)
                bottoms.SetTemperature(ReboilerAccumulationStream.GetTemperature)
                bottoms.SetMassEnthalpy(ReboilerAccumulationStream.GetMassEnthalpy)
                bottoms.SetPressure(m_boilerpressure)
            End If

            m_Tc = CondenserAccumulationStream.GetTemperature
            m_Tb = ReboilerAccumulationStream.GetTemperature

        End Sub

        ''' <summary>Restores the shortcut column state from a list of XML elements.</summary>
        ...
        ''' <summary>Serializes the shortcut column state into a list of XML elements.</summary>
