...
Imports Microsoft.Research.Oslo
...
Namespace Reactors
    ...
    <System.Serializable()> Public Class ProfileItem
        ...
        ''' <summary>Gets or sets the compound name.</summary>
        Public Property Compound As String = ""
        ''' <summary>Gets or sets the compound mass flow rate (kg/s).</summary>
        Public Property MassFlow As Double = 0.0
        ''' <summary>Gets or sets the compound molar flow rate (mol/s).</summary>
        Public Property MolarFlow As Double = 0.0
        ''' <summary>Gets or sets the compound mass fraction.</summary>
        Public Property MassFraction As Double = 0.0
        ''' <summary>Gets or sets the compound molar fraction.</summary>
        Public Property MolarFraction As Double = 0.0
        ''' <summary>Gets or sets the compound molar concentration (mol/mï¿½).</summary>
        Public Property MolarConcentration As Double = 0.0
        ''' <summary>Gets or sets the compound mass concentration (kg/mï¿½).</summary>
    ...
    ''' </summary>
    <System.Serializable()> Public Partial Class Reactor_PFR

        Inherits Reactor

        ''' <summary>Defines the sizing calculation mode for the PFR.</summary>
        Public Enum SizingType
            ''' <summary>Reactor length is specified; diameter is calculated from volume.</summary>
            Length = 0
            ''' <summary>Reactor diameter is specified; length is calculated from volume.</summary>
            Diameter = 1
        End Enum
        ...
        ''' <summary>Creates the dimensions list (Volume, Length, Diameter) for this PFR.</summary>
        ...
        ''' <summary>Updates the dimension values from the current reactor geometry.</summary>
        ...
        ''' <summary>Gets or sets the sizing calculation mode for the PFR.</summary>
        ...
        ''' <summary>Gets or sets the number of parallel tubes in the reactor.</summary>
        Public Property NumberOfTubes As Integer = 1

        ''' <summary>Gets or sets the reactor tube length (m).</summary>
        Public Property Length As Double = 1.0

        ''' <summary>Gets or sets the reactor tube internal diameter (m).</summary>
        Public Property Diameter As Double = 0.1

        ''' <summary>Gets or sets the total reactor volume (mï¿½).</summary>
        Public Property Volume As Double = 1.0

        ''' <summary>Gets or sets the volume increment (mï¿½) used as the ODE integration step.</summary>
        Public Property dV As Double = 0.01

        Private Property currentdV As Double

        ''' <summary>Gets or sets the catalyst mass loading (kg/mï¿½ of reactor volume).</summary>
        Public Property CatalystLoading As Double = 0.0#

        ''' <summary>Gets or sets the void fraction of the catalyst bed (0ï¿½1).</summary>
        Public Property CatalystVoidFraction As Double = 0.0#

        ''' <summary>Gets or sets the catalyst particle equivalent diameter (m).</summary>
        Public Property CatalystParticleDiameter As Double = 0.0#

        ''' <summary>Gets or sets the catalyst particle sphericity (0-1).</summary>
        Public Property CatalystParticleSphericity As Double = 1.0#

        ''' <summary>Gets or sets the calculated residence time (s).</summary>
        Public Property ResidenceTime As Double = 0.0#

        ''' <summary>Gets or sets whether a user-defined total pressure drop is used instead of computing it from the Ergun equation.</summary>
        Public Property UseUserDefinedPressureDrop As Boolean = False

        ''' <summary>Gets or sets the user-defined total pressure drop (Pa) when <see cref="UseUserDefinedPressureDrop"/> is <c>True</c>.</summary>
        ...
        ''' <summary>Gets or sets the internal ODE solver type index (0 = Oslo/RK, 1 = manual Euler).</summary>
        Public Property InternalSolver As Integer = 0

        ''' <summary>Initializes a new default instance of the <see cref="Reactor_PFR"/> class.</summary>
        Public Sub New()
        ...
        ''' <summary>
        ''' Initializes a new instance of the <see cref="Reactor_PFR"/> class with a name and description.
        ''' </summary>
        ''' <param name="name">The display name of the PFR.</param>
        ''' <param name="description">A brief description of the PFR.</param>
        ...
        ''' <summary>Serializes the reactor state, including profile and accumulation streams, to XML.</summary>
        ...
        ''' </summary>
        ''' <param name="x">Current volume coordinate (mï¿½).</param>
        ''' <param name="y">Current molar flows (mol/s) for each compound.</param>
        ''' <returns>An array of derivatives dN/dV for each compound.</returns>
        ...
        Public Overrides Sub RunDynamicModel()
            ...
            Dim voidFactor As Double = 1.0
            If CatalystLoading > 0.0 AndAlso CatalystVoidFraction > 0.0 Then
                voidFactor = CatalystVoidFraction
            End If
            ResidenceTime = Volume * voidFactor / ims1.GetVolumetricFlow()
        ...
        ''' </summary>
        ''' <param name="dVF">Volume fraction scale factor applied to the integration step.</param>
        ''' <param name="args">Optional. If <c>True</c>, indicates a dynamic-mode call.</param>
        ''' <returns><c>True</c> if the calculation converged successfully.</returns>
        Public Function Calculate_Internal(dVF As Double, Optional ByVal args As Object = Nothing) As Boolean
            ...
            If Not dynamics Then
                ...
                If CatalystLoading > 0.0 AndAlso CatalystVoidFraction > 0.0 Then
                    voidFactor = CatalystVoidFraction
                End If
                ResidenceTime = Volume * voidFactor / ims.Phases(0).Properties.volumetric_flow.GetValueOrDefault
            End If
            ...
            ' Heat exchange tracking variables
            Dim _coolantTempProfile As Double() = Nothing
            Dim _totalHeatExchanged As Double = 0.0
            Dim _counterCurrentConverged As Boolean = False
            Dim _counterCurrentIter As Integer = 0
            Dim _maxCounterCurrentIter As Integer = 50
            Dim _counterCurrentTolerance As Double = 0.1 ' K
            Dim _savedImsState As MaterialStream = Nothing

            If ReactorOperationMode = OperationMode.HeatExchange Then
                Dim nSteps As Integer = CInt(1.0 / deltaV) + 2
                _coolantTempProfile = New Double(nSteps - 1) {}

                ' Calculate heat exchange area for results
                If HeatExchangeAreaCalculationMode = HeatExchangeAreaMode.AutoFromGeometry Then
                    CalculatedHeatExchangeArea = Math.PI * Diameter * Length * NumberOfTubes
                Else
                ...
                If HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.ConstantTemperature Then
                    ...
                    For idx = 0 To nSteps - 1
                        _coolantTempProfile(idx) = CoolantInletTemperature
                    Next
                ElseIf HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.CoCurrent Then
                    ' Coolant enters at same end as process fluid
                    _coolantTempProfile(0) = CoolantInletTemperature
                Else
                    ' Counter-current: coolant enters at reactor outlet end
                    ' Initial guess: linear profile between inlet and inlet (will iterate)
                    For idx = 0 To nSteps - 1
                        _coolantTempProfile(idx) = CoolantInletTemperature
                    Next
                    ' Save initial state for counter-current iterations
                    _savedImsState = DirectCast(ims.CloneXML(), MaterialStream)
                    _savedImsState.SetPropertyPackage(PropertyPackage)
                    _savedImsState.SetFlowsheet(Me.FlowSheet)
                End If
            End If

            ' Counter-current outer iteration loop
            Do
                ...
                Do
                    ...
                    For Each ar In Me.ReactionsSequence
                        ...
                        Dim vfrac As Double = 1.0

                        If CatalystLoading > 0 And CatalystVoidFraction > 0 And hasHetCatReaction Then
                            vfrac = CatalystVoidFraction
                        End If
                        ...
                        Do
                            ...
                            If Not negativeflag Then
                                ...
                                For nncounter = 1 To 30
                                    ...
                                    If esolv IsNot Nothing Then
                                        ...
                                        IObj2?.SetCurrent
                                        If dynamics Then
                                            esolv.Solve(vc0, 0.0#, 0.01 * deltaV * vfrac * Volume, deltaV * vfrac * Volume,
                                                        Sub(x As Double, y As Double())
                                                            vc = y
                                                            solutions.Add(vc)
                                                        End Sub, 0.00000001)
                                        Else
                                            esolv.Solve(vc0, 0.0#, 0.01 * deltaV * vfrac * Volume, deltaV * vfrac * Volume,
                                                        ...
                                                        End Sub, 0.00000001)
                                        End If
                                        vc = solutions.Where(Function(r) Not r.HasNegative()).Last()
                                    ...
                                    Else
                                        ...
                                        Select Case InternalSolver
                                            Case 0
                                                ...
                                                IObj2?.SetCurrent
                                                If dynamics Then
                                                    odesolver.Solve(vc0, 0.0#, 0.01 * deltaV * vfrac * Volume, deltaV * vfrac * Volume,
                                                                    Sub(x As Double, y As Double())
                                                                        vc = y
                                                                        solutions.Add(vc)
                                                                    End Sub)
                                                ...
                                                End If
                                                vc = solutions.Where(Function(r) Not r.HasNegative()).Last()
                                                If vc.Equals(solutions.First) Then vc = solutions.Last()
                                            Case 1
                                            ...
                                            Case 4
                                                ...
                                                options.InitialStep = 0.01 * deltaV * Volume * vfrac
                                                options.MinStep = 0.0000000001
                                                options.MaxStep = 0.1 * deltaV * Volume * vfrac
                                                Dim results = Microsoft.Research.Oslo.Ode.RK45(0.0, vc0,
                                                                                              Function(x, yv)
                                                                                                  Dim dy = ODEFunc(x, yv.ToArray())
                                                                                                  Return New Vector(dy)
                                                                                              End Function, options).SolveTo(deltaV * vfrac * Volume).ToArray()
                                                vc = results.Where(Function(r) Not r.X.ToArray().HasNegative()).Last().X.ToArray()
                                                If vc.Equals(results.First().X.ToArray()) Then vc = results.Last().X.ToArray()
                                        End Select
                            ...
                            Select Case Me.ReactorOperationMode
                                ...
                                Case OperationMode.HeatExchange
                                    ...
                                    ' Calculate local heat transfer area for this volume step
                                    Dim dA As Double
                                    If HeatExchangeAreaCalculationMode = HeatExchangeAreaMode.AutoFromGeometry Then
                                        dA = Math.PI * Diameter * Length * deltaV * NumberOfTubes
                                    Else
                                        dA = HeatExchangeArea * deltaV
                                    End If

                                    ' Get coolant temperature at this position
                                    Dim Tc_local As Double
                                    If HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.ConstantTemperature Then
                                        Tc_local = CoolantInletTemperature
                                    ElseIf HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.CoCurrent Then
                                        Tc_local = _coolantTempProfile(counter)
                                    Else ' CounterCurrent
                                        Tc_local = _coolantTempProfile(counter)
                                    End If
                                    ...
                                    If CalculateInternalHTC Then
                                        ...
                                        Dim Cp_p As Double = ims.Phases(0).Properties.heatCapacityCp.GetValueOrDefault * 1000 ' kJ/kg.K -> J/kg.K
                                        Dim k_p As Double = ims.Phases(0).Properties.thermalConductivity.GetValueOrDefault
                                        Dim isHeating As Boolean = (Tc_local > T)
                                        CalculatedInternalHTC = DittusBoelterHTC(Diameter, rho_p, mu_p, Cp_p, k_p, W, NumberOfTubes, isHeating)
                                    End If
                                    If CalculateExternalHTC Then
                                        ...
                                        Dim Cp_c As Double = CoolantSpecificHeat ' already J/kg.K
                                    ...
                                    ' Q = U * A * (Tc - T) in Watts, convert to kW
                                    Dim Q_local As Double = U_eff * dA * (Tc_local - T) / 1000.0

                                    ' Update coolant temperature for co-current flow
                                    If HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.CoCurrent Then
                                        Dim mCp As Double = CoolantMassFlowRate * CoolantSpecificHeat
                                        If UseUtilityStream AndAlso Me.GraphicObject.InputConnectors.Count > 2 Then
                                            Dim utilStream = GetInletMaterialStream(2)
                                            If utilStream IsNot Nothing Then
                                                mCp = utilStream.GetMassFlow() * utilStream.GetPhase("Mixture").Properties.heatCapacityCp.GetValueOrDefault * 1000
                                            End If
                                        End If
                                        If mCp > 0 Then
                                            _coolantTempProfile(counter + 1) = Tc_local - Q_local * 1000.0 / mCp
                                        End If
                                    End If
                                    ...
                                    _totalHeatExchanged += Q_local

                                    IObj2?.SetCurrent()

                                    tmp = Me.PropertyPackage.CalculateEquilibrium2(FlashCalculationType.PressureEnthalpy, P, Hp / W, T)
                                    Dim ToutHX As Double = tmp.CalculatedTemperature.GetValueOrDefault

                                    ims.Phases(0).Properties.temperature = ToutHX
                                    ims.Phases(0).Properties.enthalpy = Hp / W
                    ...
                    Else
                        ...
                        If CatalystLoading > 0.0 And hasHetCatReaction Then

                            'has catalyst, use Ergun equation for pressure drop in reactor beds

                            Dim vel As Double = (Qlin + Qvin + Qsin) / (PI * Diameter ^ 2 / 4)
                            Dim dp As Double = CatalystParticleDiameter * CatalystParticleSphericity
                            Dim ev As Double = CatalystVoidFraction
                ...
                ' Counter-current convergence check and outer iteration
                If ReactorOperationMode = OperationMode.HeatExchange AndAlso
                   HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.CounterCurrent Then

                    ' For counter-current: coolant enters at reactor outlet (last step),
                    ' so _coolantTempProfile(nloops) = CoolantInletTemperature.
                    ' After forward integration, we have reactor T at each step.
                    ' Now compute coolant profile backwards from outlet to inlet.
                    Dim nSteps As Integer = CInt(1.0 / deltaV) + 2
                    Dim mCp As Double = CoolantMassFlowRate * CoolantSpecificHeat
                    If UseUtilityStream AndAlso Me.GraphicObject.InputConnectors.Count > 2 Then
                        Dim utilStream = GetInletMaterialStream(2)
                    ...
                    ' Build new coolant profile from reactor outlet backwards
                    Dim newCoolantProfile As Double() = New Double(nSteps - 1) {}
                    Dim nloopsCC As Integer = CInt(1.0 / deltaV)
                    newCoolantProfile(nloopsCC) = CoolantInletTemperature ' coolant enters at reactor outlet

                    ' We need the reactor temperature at each step - use the profile we just computed
                    ' The reactor profile temperatures are in the Profile list
                    For step_idx = nloopsCC - 1 To 0 Step -1
                        Dim dA As Double
                        If HeatExchangeAreaCalculationMode = HeatExchangeAreaMode.AutoFromGeometry Then
                            dA = Math.PI * Diameter * Length * deltaV * NumberOfTubes
                        Else
                            dA = HeatExchangeArea * deltaV
                        End If
                        ' Reactor T at this step from profile (step_idx corresponds to profile index + 1)
                        Dim T_react As Double
                        If Profile IsNot Nothing AndAlso step_idx + 1 < Profile.Count Then
                            T_react = Profile(step_idx + 1).Item2
                        Else
                            T_react = T0
                        End If
                        ' Auto-calculate HTCs for counter-current pass
                        If CalculateInternalHTC Then
                            Dim rho_p As Double = ims.Phases(0).Properties.density.GetValueOrDefault
                            Dim mu_p As Double = ims.Phases(0).Properties.viscosity.GetValueOrDefault
                            Dim Cp_p As Double = ims.Phases(0).Properties.heatCapacityCp.GetValueOrDefault * 1000
                            Dim k_p As Double = ims.Phases(0).Properties.thermalConductivity.GetValueOrDefault
                            Dim isHeating As Boolean = (newCoolantProfile(step_idx + 1) > T_react)
                            CalculatedInternalHTC = DittusBoelterHTC(Diameter, rho_p, mu_p, Cp_p, k_p, W, NumberOfTubes, isHeating)
                        End If
                        If CalculateExternalHTC Then
                            Dim D_outer_cc As Double = Diameter + 2.0 * WallThickness
                            Dim rho_c_cc As Double = CoolantDensity
                            Dim mu_c_cc As Double = CoolantViscosity
                            Dim Cp_c_cc As Double = CoolantSpecificHeat
                            Dim k_c_cc As Double = CoolantThermalConductivity
                            Dim mFlow_c_cc As Double = CoolantMassFlowRate
                            If UseUtilityStream AndAlso Me.GraphicObject.InputConnectors.Count > 2 Then
                                Dim utilStr_cc = GetInletMaterialStream(2)
                                If utilStr_cc IsNot Nothing Then
                                    rho_c_cc = utilStr_cc.GetPhase("Mixture").Properties.density.GetValueOrDefault
                                    mu_c_cc = utilStr_cc.GetPhase("Mixture").Properties.viscosity.GetValueOrDefault
                                    Cp_c_cc = utilStr_cc.GetPhase("Mixture").Properties.heatCapacityCp.GetValueOrDefault * 1000
                                    k_c_cc = utilStr_cc.GetPhase("Mixture").Properties.thermalConductivity.GetValueOrDefault
                                    mFlow_c_cc = utilStr_cc.GetMassFlow()
                                End If
                            End If
                            CalculatedExternalHTC = AnnularJacketHTC(D_outer_cc, JacketDiameter, rho_c_cc, mu_c_cc, Cp_c_cc, k_c_cc, mFlow_c_cc)
                        End If
                        Dim U_eff_cc As Double = If(UseWallProperties, ComputeEffectiveHTC(T_react), OverallHeatTransferCoefficient)
                        Dim Q_step As Double = U_eff_cc * dA * (newCoolantProfile(step_idx + 1) - T_react)
                        If mCp > 0 Then
                            newCoolantProfile(step_idx) = newCoolantProfile(step_idx + 1) + Q_step / mCp
                        Else
                            newCoolantProfile(step_idx) = newCoolantProfile(step_idx + 1)
                        End If
                    Next

                    ' Check convergence: compare old and new coolant profiles
                    Dim maxDiff As Double = 0.0
                    For step_idx = 0 To nloopsCC
                        Dim diff = Math.Abs(newCoolantProfile(step_idx) - _coolantTempProfile(step_idx))
                        If diff > maxDiff Then maxDiff = diff
                    Next

                    _counterCurrentIter += 1

                    If maxDiff < _counterCurrentTolerance OrElse _counterCurrentIter >= _maxCounterCurrentIter Then
                        _counterCurrentConverged = True
                        _coolantTempProfile = newCoolantProfile
                        CoolantOutletTemperature = _coolantTempProfile(0) ' coolant exits at reactor inlet end
                    Else
                        ' Update coolant profile with relaxation and re-run
                        Dim relaxation As Double = 0.5
                        For step_idx = 0 To nloopsCC
                            _coolantTempProfile(step_idx) = (1 - relaxation) * _coolantTempProfile(step_idx) + relaxation * newCoolantProfile(step_idx)
                        Next
                        ' Reset reactor state for next iteration
                        ims = DirectCast(_savedImsState.CloneXML(), MaterialStream)
                        ims.SetPropertyPackage(PropertyPackage)
                        ims.SetFlowsheet(Me.FlowSheet)
                        PropertyPackage.CurrentMaterialStream = ims
                        currvol = 0.0
            ...
            Loop Until _counterCurrentConverged OrElse ReactorOperationMode <> OperationMode.HeatExchange
            ...
            If Not dynamics Then
                ...
                ElseIf Me.ReactorOperationMode = OperationMode.HeatExchange Then
                    ...
                    Me.DeltaQ = _totalHeatExchanged

                    OutletTemperature = ims.GetTemperature()

                    Me.DeltaT = OutletTemperature - T0

                    ' Set coolant outlet temperature
                    If HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.ConstantTemperature Then
                        CoolantOutletTemperature = CoolantInletTemperature
                    ElseIf HeatExchangeCoolantFlowDirection = HeatExchangeCoolantMode.CoCurrent Then
                        Dim nloopsCO As Integer = CInt(1.0 / deltaV)
                        CoolantOutletTemperature = _coolantTempProfile(nloopsCO)
                    End If
                    ' Counter-current outlet is already set during convergence loop

                    ' Update utility stream outlet if connected
                    If UseUtilityStream AndAlso Me.GraphicObject.OutputConnectors.Count > 1 AndAlso Me.GraphicObject.InputConnectors.Count > 2 Then
                        Dim utilOutCp = Me.GraphicObject.OutputConnectors(1)
        ...
        Public Overrides Function SetPropertyValue(ByVal prop As String, ByVal propval As Object, Optional ByVal su As Interfaces.IUnitsOfMeasure = Nothing) As Boolean
            ...
            Select Case propidx
                ...
                Case 34
                    CatalystParticleSphericity = propval
            End Select
            Return 1
        End Function
        ...
        ''' <summary>Returns an OxyPlot chart model for the specified profile chart name.</summary>
        ...
        ''' <summary>Returns the list of available profile chart names for this PFR.</summary>
